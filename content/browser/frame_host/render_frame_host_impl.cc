...
RenderFrameHostImpl::RenderFrameHostImpl(SiteInstance* site_instance,
    ...
    : render_view_host_(render_view_host),
      ...
      isVideoPopupWindow(false),
      videoPopupWindowInfo(""),
...
bool RenderFrameHostImpl::OnMessageReceived(const IPC::Message &msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameHostImpl, msg)
    ...
    IPC_MESSAGE_HANDLER(FrameHostMsg_JavaScriptAddFavoriteMessage,
                                    OnJavaScriptAddFavoriteMessage)
...
void RenderFrameHostImpl::OnJavaScriptAddFavoriteMessage(const base::string16& url, const base::string16& title) {
  delegate_->JavaScriptAddFavoriteMessage(this, url, title);
}
...
void RenderFrameHostImpl::setVideoPopupWindowStatus(bool is_video_popup_window) {
  isVideoPopupWindow = is_video_popup_window;
}

bool RenderFrameHostImpl::getVideoPopupWindowStatus(void) {
  return isVideoPopupWindow;
}

void RenderFrameHostImpl::setVideoPopupWindowInfo(const std::string& video_popup_window_info) {
  videoPopupWindowInfo = video_popup_window_info;
}

const std::string& RenderFrameHostImpl::getVideoPopupWindowInfo(void) {
  return videoPopupWindowInfo;
}

void RenderFrameHostImpl::RestoreVideoPopupWindow(const std::string& extension_id) {
  Send(new FrameMsg_RestoreVideoPopupWindow(GetRoutingID(), extension_id));
}
...
void RenderFrameHostImpl::SetCompatibleMode(bool mode) {
  Send(new FrameMsg_Reload(routing_id_, true));
}
...
bool RenderFrameHostImpl::CanExecuteJavaScript() {
  return g_allow_injecting_javascript ||
         ...
         frame_tree_node_->current_url().SchemeIs("chrome-extension")  ||  // todo: delete it !
         frame_tree_node_->current_url().SchemeIs("nfsbrowser-extension")  ||         
