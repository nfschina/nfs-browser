...
bool IsPresetExtension(const GURL& url) {
  if (!url.is_valid())
    return false;
...
RenderProcessHostImpl::RenderProcessHostImpl(
    ...
    : fast_shutdown_started_(false),
      ...
      is_preset_extension_render_process_(false),   // added by wangpp
#if defined(OS_ANDROID)
...
RenderProcessHostImpl::RenderProcessHostImpl(
    ...
    StoragePartitionImpl* storage_partition_impl,
    bool is_for_guests_only,
    bool is_preset_extension_render_process)
    : fast_shutdown_started_(false),
      deleting_soon_(false),
#ifndef NDEBUG
      is_self_deleted_(false),
      ...
      channel_connected_(false),
      sent_render_process_ready_(false),
      is_preset_extension_render_process_(is_preset_extension_render_process),
#if defined(OS_ANDROID)
      never_signaled_(base::WaitableEvent::ResetPolicy::MANUAL,
                      base::WaitableEvent::InitialState::NOT_SIGNALED),
#endif
...
void RenderProcessHostImpl::AppendRendererCommandLine(
  ...
  command_line->AppendSwitch(switches::kDisableDirectNPAPIRequests);//[zhangyq] add
...
#else    // 本地网页全部共享一个render process
  if (ChildProcessSecurityPolicyImpl::GetInstance()->HasWebUIBindings(
          host->GetID()) !=
      site_url.SchemeIs(kChromeUIScheme)) {
    return false;
  }
#endif
...
RenderProcessHost* RenderProcessHost::GetExistingProcessHost(
  ...
  if (!suitable_renderers.empty()) {
    ...
    if (!IsPresetExtension(site_url))
      return suitable_renderers[random_index];

    for (auto it : suitable_renderers) {
      if (it->IsPresetExtensionRenderProcess()) {
        return it;
      }
    }
  }
...
// added by wangpp
bool RenderProcessHostImpl::IsPresetExtensionRenderProcess() {
  return is_preset_extension_render_process_;
}
// end
