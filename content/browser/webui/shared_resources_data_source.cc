...
#include "ui/resources/grit/webui_resources_nfs_map.h"
...
const ResourcesMap* CreateResourcesMap() {
  ...
  for (size_t i = 0; i < kWebuiResourcesNfsSize; ++i) {
    const std::string resource_name = kWebuiResourcesNfs[i].name;
    const int resource_id = kWebuiResourcesNfs[i].value;
    AddResource(resource_name, resource_id, result);
  }
