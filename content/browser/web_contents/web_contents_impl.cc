...
bool WebContentsImpl::OnMessageReceived(RenderViewHost* render_view_host,
  ...
  IPC_BEGIN_MESSAGE_MAP(WebContentsImpl, message)
    ...
    //huk added
    IPC_MESSAGE_HANDLER(FrameHostMsg_CollectErrorPageInfo, OnCollectErrorPageInfo)
#if defined(OS_ANDROID)
...
void WebContentsImpl::SaveFrameWithHeaders(const GURL& url,
  ...
  params->set_url_source_from(DownloadUrlParameters::URL_SOURCE_FROM_SAVEIMAGEAS);
...
void WebContentsImpl::RunJavaScriptMessage(
  ...
  //zhangyq suppress arp javascript dialogs
  if (!frame_url.GetOrigin().spec().compare("http://172.31.58.20:9070/") &&
      (base::UTF16ToUTF8(message).find("exception") != std::string::npos ||
      base::UTF16ToUTF8(message).find("Exception") != std::string::npos ||
      base::UTF16ToUTF8(message).find("======") != std::string::npos ||
      base::UTF16ToUTF8(message).find("调用逻辑") != std::string::npos)) {
    suppress_this_message = true;
  }
...
void WebContentsImpl::JavaScriptAddFavoriteMessage(RenderFrameHost* render_frame_host,
                                                   const base::string16& url,
                                                   const base::string16& title) {
  if(!delegate_)
    return;
  delegate_->AddFavorite(url, title);
}
...
void WebContentsImpl::UpdateWebContentsVisibility(bool visible, bool isNemoActive) {
  RenderWidgetHostViewBase* rwhv =
      static_cast<RenderWidgetHostViewBase*>(GetRenderWidgetHostView());
  if (rwhv && !isNemoActive) {
    SendPageMessage(new PageMsg_UpdateWindowVisibility(
        MSG_ROUTING_NONE, visible));
  }
...
void WebContentsImpl::UpdatePluginVisibility(bool visible) {
  RenderWidgetHostViewBase* rwhv =
      static_cast<RenderWidgetHostViewBase*>(GetRenderWidgetHostView());
  if (rwhv) {
    SendPageMessage(new PageMsg_UpdateWindowVisibility(
...
bool WebContentsImpl::CanUseForPrerender() {
  return can_use_for_prerender_;
}

void WebContentsImpl::SetUseForPrerender() {
  can_use_for_prerender_ = true;
}
...
void WebContentsImpl::OnCollectErrorPageInfo(
    const std::string& url, int error_code) {
  std::string error_detail = net::ErrorToString(error_code);
  if (delegate_)  {
      delegate_->RecordErrorPageInfo(url, error_detail);
  }
}

void WebContentsImpl::OpenSearchContent(const base::string16& search_text) {
  if (delegate_)  {
    size_t MAX_LENGTH = 512;

    delegate_->OpenSearchContent(search_text.length() > MAX_LENGTH ? 
          search_text.substr(0, MAX_LENGTH - 1) : search_text);
  }
}
