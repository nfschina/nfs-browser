...
class CONTENT_EXPORT WebContentsImpl
 ...
 public:
  ...
  bool CanUseForPrerender() override;
  void SetUseForPrerender() override;
  ...
  void OpenSearchContent(const base::string16& search_text);
  ...
  void JavaScriptAddFavoriteMessage(RenderFrameHost* render_frame_host,
  ...
  void UpdateWebContentsVisibility(bool visible, bool isNemoActive = false);

  void UpdatePluginVisibility(bool visible);
 ...
 private:
  ...
  // huk
  void OnCollectErrorPageInfo(const std::string& url, int error_code);
