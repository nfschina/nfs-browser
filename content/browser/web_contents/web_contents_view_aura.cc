...
class WebContentsViewAura::WindowObserver
 ...
 public:
  ...
  void OnHostActiveChanged(const aura::WindowTreeHost* host, bool active)  override {
  #if defined(OS_POSIX)
    XID window;
    XID x_root_window = DefaultRootWindow(gfx::GetXDisplay());
    ui::GetXIDProperty(x_root_window, "_NET_ACTIVE_WINDOW", &window);
    XClassHint class_hint;
    Status status = XGetClassHint(gfx::GetXDisplay(), window, &class_hint);
    if (status && !strcmp("pluginloader.exe", class_hint.res_class))
      active = true;
    //XFree(class_hint.res_class);
    view_->web_contents_->UpdatePluginVisibility(active);
  #endif
  }
...
void WebContentsViewAura::StartDragging(
  ...
  // huk drag
  if (result_op == 0 && drop_data.url.is_valid() &&
      web_contents_->GetURL() != GURL("nfsbrowser://newtab/"))  {
    web_contents_->OpenURL(OpenURLParams(drop_data.url, Referrer(),
                                         WindowOpenDisposition::NEW_BACKGROUND_TAB,
                                         ui::PAGE_TRANSITION_TYPED, false));
  } else if (result_op == 0 && !drop_data.text.is_null()) {
    // search the text
    web_contents_->OpenSearchContent(drop_data.text.string());
  }
...
void WebContentsViewAura::OnWindowVisibilityChanged(aura::Window* window,
  ...
  bool isNemoActive = false;
#if defined(OS_POSIX)
  XID window_x;
  XID x_root_window = DefaultRootWindow(gfx::GetXDisplay());
  ui::GetXIDProperty(x_root_window, "_NET_ACTIVE_WINDOW", &window_x);
  XClassHint class_hint;
  Status status = XGetClassHint(gfx::GetXDisplay(), window_x, &class_hint);
  if (status && !strcmp("Nemo", class_hint.res_class)) {
    isNemoActive = true;
  }
#endif

  web_contents_->UpdateWebContentsVisibility(visible, isNemoActive);
