...
#endif
{
  process_.reset(new BrowserChildProcessHostImpl(PROCESS_TYPE_PLUGIN, this, kPluginServiceName));
}
...
bool PluginProcessHost::Init(const WebPluginInfo& info) {
  ...
  cmd_line->AppendSwitch(switches::kDisableDirectNPAPIRequests);//[zhangyq] add
