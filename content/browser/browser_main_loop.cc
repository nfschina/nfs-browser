...
int BrowserMainLoop::PreCreateThreads() {
  ...
  memory_pressure_monitor_.reset(new base::nfs::MemoryPressureMonitor());
#endif
...
#if defined(OS_LINUX)
  // consider the case where user runs "sudo ***"
  if (getuid() == 0)  {
    if (!base::CommandLine::ForCurrentProcess()->HasSwitch("user-data-dir")) {
      base::FilePath exe_path;
      PathService::Get(base::FILE_EXE, &exe_path);
      base::CommandLine::ForCurrentProcess()->AppendSwitchPath("user-data-dir", exe_path);
    }
  }
#endif
