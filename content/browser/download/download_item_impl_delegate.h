...
class CONTENT_EXPORT DownloadItemImplDelegate {
 public:
  ...
  virtual bool ProhibitConcat(std::string url);
  ...
  virtual void ResumeInterruptedDownloadEx(
      DownloadItemImpl* download,
  ...
  // Should user the file save as download mode
  virtual bool ShouldUseSaveAsDownloadMode(GURL url, bool find_delete);
