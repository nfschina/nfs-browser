...
// Maximum download speed step is 3
const uint32_t kMaxDownloadSpeedStep = 3;
...
void SavePackage::SaveFinished(SaveItemId save_item_id,
  ...
  if (download_ && (download_->GetState() == DownloadItem::IN_PROGRESS)) {
   // download_->DestinationUpdate(completed_count(), CurrentSpeed());
    ...
    int64_t total_count = download_->GetTotalBytes();
    int64_t div = total_count / kMaxDownloadSpeedStep;
    if (div == 0)    {
      div = 1;
    }
    if (completed_count() % div == 0) {
      download_->DestinationUpdate(completed_count(), CurrentSpeed());
    }
  }
