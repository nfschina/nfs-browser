...
class CONTENT_EXPORT DownloadItemImpl
 ...
 public:
  ...
  std::string GetPackUrl() const override;
  ...
  void Reset() override;
  void DeleteFileOnDisk(bool delete_file) override;
  ...
  void DestinationTotalSize(int64_t bytes_total) override;
  ...
  void DestinationUpdateFileName(std::string file_name) override;

//  #if defined(OS_LINUX)
  virtual void KeepPause(std::unique_ptr<DownloadFile> download_file,
                         std::unique_ptr<DownloadRequestHandleInterface> req_handle);
//  #endif

  virtual DownloadInternalState GetInternalState() const;

  std::string GetBtRealFileName() override;
  void SetBtRealFileName(std::string file_name) override;

  bool IsSaveAsDownloadMode();
  void SetSaveAsDownloadMode(bool is_save_as);
 ...
 private:
  ...
  // BT real file name
  std::string bt_real_name_;

  DownloadItemImpl* download_parent_;
