...
const char nfsDownloadVideo[] = "&nfs_download_video";
...
void DoMediaConcat(std::string input, base::FilePath download_path, bool prohibit_concat) {
  media::DoMediaConcat(input, download_path.AsUTF8Unsafe(), prohibit_concat);
}
...
DownloadItemImpl::DownloadItemImpl(DownloadItemImplDelegate* delegate,
    ...
    : guid_(base::ToUpperASCII(guid)),
      ...
      etag_(etag),
      bt_real_name_(bt_real_name),
      net_log_(net_log),
      weak_ptr_factory_(this) {
  delegate_->Attach();
  DCHECK(state_ == COMPLETE_INTERNAL || state_ == INTERRUPTED_INTERNAL ||
         state_ == CANCELLED_INTERNAL || state_ == PAUSING_INTERNAL);
  DCHECK(base::IsValidGUID(guid_));
  if(state_ == PAUSING_INTERNAL) {
    is_paused_ = true;
  }
...
DownloadItemImpl::DownloadItemImpl(DownloadItemImplDelegate* delegate,
    ...
    : guid_(base::ToUpperASCII(base::GenerateGUID())),
      download_id_(download_id),
      target_path_(info.save_info->file_path),
      ...
      etag_(info.etag),
      download_parent_(info.download_parent),
      net_log_(net_log),
      weak_ptr_factory_(this) {
  delegate_->Attach();
...
void DownloadItemImpl::Cancel(bool user_cancel) {
  ...
  //确保Cancel函数只执行一次
  //下载管理器和窗口都关闭时，该函数会执行两次，从而导致第二次执行时无observer而崩溃。
  if (!cancel_needed_)
    return;

  cancel_needed_ = false;
...
void DownloadItemImpl::Remove() {
  ...
  // if delete_file_ is true then delete the file
  if (delete_file_ && state_ == COMPLETE_INTERNAL) {
  ...
  NotifyRemoved();
  DownloadFileImplSpeed::ApxDownloadRemove(GetId(), delete_file_);
...
DownloadItemImpl::GetInternalState() const {
  return state_;
}

std::string DownloadItemImpl::GetBtRealFileName() {
  return bt_real_name_;
}

void DownloadItemImpl::SetBtRealFileName(std::string file_name) {
  bt_real_name_ = file_name;
}

bool DownloadItemImpl::IsSaveAsDownloadMode() {
  return is_save_as_;
}

void DownloadItemImpl::SetSaveAsDownloadMode(bool is_save_as) {
  is_save_as_ = is_save_as;
}
...
std::string DownloadItemImpl::GetMethod() const {
  return method_;
}

std::string DownloadItemImpl::GetPackUrl() const {
  return pack_url_;
}
...
bool DownloadItemImpl::CanShowInFolder() {
  ...
  // location. or the downloaded file is bt
...
void DownloadItemImpl::DeleteFileOnDisk(bool delete_file) {
  delete_file_ = delete_file;
  if (delete_file_ && download_file_.get()) {
    download_file_->Delete();
  }
}
...
DownloadItemImpl::ResumeMode DownloadItemImpl::GetResumeMode() const {
  ...
  if (!GetURL().SchemeIsHTTPOrHTTPS() && !GetURL().SchemeIsFtp())
    return RESUME_MODE_INVALID;
...
void DownloadItemImpl::DestinationTotalSize(int64_t bytes_total) {
  SetTotalBytes(bytes_total);
}
...
void DownloadItemImpl::DestinationUpdate(int64_t bytes_so_far,
  ...
  // callbacks.
  DCHECK(state_ == TARGET_PENDING_INTERNAL || state_ == IN_PROGRESS_INTERNAL || state_ == PAUSING_INTERNAL);
...
void DownloadItemImpl::DestinationUpdateFileName(std::string file_name) {
  #if defined(OS_WIN)
  base::FilePath real_path =
     target_path_.DirName().Append(base::UTF8ToWide(file_name));
  #else
  base::FilePath real_path = target_path_.DirName().Append(file_name);
  #endif
  target_path_ = real_path;

  SetBtRealFileName(file_name);
  if (download_parent_) {
    download_parent_->SetBtRealFileName(file_name);
  }
}
...
//#if defined(OS_LINUX)
// keep pasue.
void DownloadItemImpl::KeepPause(
  ...
  DCHECK(state_ == PAUSING_INTERNAL);
  DCHECK(download_file_.get());
...
void DownloadItemImpl::Start(
  ...
  //     successful.
  DCHECK(state_ == INITIAL_INTERNAL || state_ == RESUMING_INTERNAL || state_ == PAUSING_INTERNAL);
  ...
  if (!download_file_.get() || !download_file_.get())  {
    return;
  }
...
void DownloadItemImpl::OnDownloadFileInitialized(
  ...
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  if(state_ == PAUSING_INTERNAL ) return;
...
void DownloadItemImpl::DetermineDownloadTarget() {
  ...
  if (target_path_.empty()) {
    ...
    OnDownloadTargetDetermined(target_path_,
                               TARGET_DISPOSITION_PROMPT,
                               danger_type_,
                               target_path_);
  }
...
void DownloadItemImpl::OnDownloadRenamedToFinalName(
  ...
  printf("%s-----------------------url:%s\n", __FUNCTION__, GetURL().spec().c_str());
  std::string url = GetURL().spec();
  if (url.find(nfsDownloadVideo, url.length() - strlen(nfsDownloadVideo)) != std::string::npos) {
    bool prohibit_concat = delegate_->ProhibitConcat(full_path.AsUTF8Unsafe());
    base::FilePath download_path = DownloadPrefs::FromBrowserContext(GetBrowserContext())->DownloadPath();
    printf("InProgressCount----------------------:%d, download_path:%s\n", prohibit_concat, download_path.AsUTF8Unsafe().c_str());
    BrowserThread::PostTask(
        BrowserThread::FILE, FROM_HERE,
        base::Bind(&DoMediaConcat, full_path.AsUTF8Unsafe(), download_path, prohibit_concat));
  }
}
...
void DownloadItemImpl::InterruptWithPartialState(
  ...
  switch (state_) {
    ...
    case PAUSING_INTERNAL:
      ...
      download_failed = (reason == DOWNLOAD_INTERRUPT_REASON_SERVER_FAILED ||
        reason == DOWNLOAD_INTERRUPT_REASON_FILE_FAILED);

      // before release we should cancel request first
      if (request_handle_)
        request_handle_->CancelRequest();
      ...
      if (download_file_) {
        ...
        ReleaseDownloadFile((resume_mode != RESUME_MODE_IMMEDIATE_CONTINUE &&
                            resume_mode != RESUME_MODE_USER_CONTINUE &&
                            resume_mode != RESUME_MODE_USER_RESTART)/* || download_failed*/);
      }
      break;
  ...
  if (current_path_.empty()) {
    ...
    if (reason == DOWNLOAD_INTERRUPT_REASON_USER_SHUTDOWN) {
      UpdateProgress(received_bytes_, 0);
    } else {
  ...
  if(reason == DOWNLOAD_INTERRUPT_REASON_USER_SHUTDOWN) {
    ...
    is_paused_ = true;
    RecordDownloadCount(PAUSING_COUNT);
    TransitionTo(PAUSING_INTERNAL);
    return;
  }
...
void DownloadItemImpl::UpdateProgress(int64_t bytes_so_far,
  ...
  if ((received_bytes_ > total_bytes_) && (total_bytes_ != 0))
...
//          << "Current output directory must match target directory.";
    ...
    case PAUSING_INTERNAL:
      ...
      DCHECK(is_paused_) << "At the time a download enters IN_PROGRESS state, "
                             "it must be paused.";
      net_log_.AddEvent(
          net::NetLogEventType::DOWNLOAD_ITEM_PAUSING,
          base::Bind(&ItemPausingNetLogCallback, received_bytes_));
      break;
  ...
  bool is_done =
      (state_ == COMPLETE_INTERNAL || state_ == INTERRUPTED_INTERNAL ||
       state_ == RESUMING_INTERNAL || state_ == CANCELLED_INTERNAL || state_ == PAUSING_INTERNAL);
  bool was_done =
      (old_state == COMPLETE_INTERNAL || old_state == INTERRUPTED_INTERNAL ||
       old_state == RESUMING_INTERNAL || old_state == CANCELLED_INTERNAL || state_ == PAUSING_INTERNAL);
...
void DownloadItemImpl::ResumeInterruptedDownload(
  ...
  delegate_->ResumeInterruptedDownloadEx(this, on_started);
...
bool DownloadItemImpl::IsValidSavePackageStateTransition(
  ...
  switch (from) {
    ...
    case IN_PROGRESS_INTERNAL:
      return to == CANCELLED_INTERNAL || to == COMPLETE_INTERNAL || to == PAUSING_INTERNAL;

    case PAUSING_INTERNAL:
      return to == IN_PROGRESS_INTERNAL || to == CANCELLED_INTERNAL || to == TARGET_PENDING_INTERNAL;
  ...
  switch (from) {
    ...
    case TARGET_RESOLVED_INTERNAL:
      return to == IN_PROGRESS_INTERNAL || to == INTERRUPTED_INTERNAL ||
             to == CANCELLED_INTERNAL || to == PAUSING_INTERNAL;

    case IN_PROGRESS_INTERNAL:
      return to == COMPLETING_INTERNAL || to == CANCELLED_INTERNAL ||
             to == INTERRUPTED_INTERNAL || to == PAUSING_INTERNAL;

    case PAUSING_INTERNAL:
      return to == IN_PROGRESS_INTERNAL || to == CANCELLED_INTERNAL ||
             to == INTERRUPTED_INTERNAL || to == TARGET_PENDING_INTERNAL;
