...
DownloadRequestCore::CreateDownloadCreateInfo(DownloadInterruptReason result) {
  ...
  {
     net::UploadDataStream* upload =const_cast<net::UploadDataStream *>(request()->get_upload());
     const int kBlobDataLength = upload->size();
     net::CompletionCallback init_callback;
     if(upload->Init(init_callback,net::NetLogWithSource()) == net::OK && upload->size() > 0)
     {
        scoped_refptr<net::IOBufferWithSize> buffer(new net::IOBufferWithSize(kBlobDataLength));
        upload->Read(buffer.get(), buffer->size(), init_callback);
        create_info->pack_url = std::string(buffer->data(), buffer->size());
     }
  }
  return create_info;
}
