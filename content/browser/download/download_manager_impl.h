...
struct UrlSourceFromData {
public:
  UrlSourceFromData(const GURL& url,
  ...
  GURL source_url;
  DownloadUrlParameters::UrlSourceFrom source_from;
};
...
class CONTENT_EXPORT DownloadManagerImpl : public DownloadManager,
 ...
 public:
  ...
  base::FilePath GetDefaultDownloadPath() override;
 ...
 private:
  ...
  bool ProhibitConcat(std::string url) override;
  void ResumeInterruptedDownload(
      std::unique_ptr<content::DownloadUrlParameters> params,
      uint32_t id) override;
  void ResumeInterruptedDownloadEx(
  ...
  void DownloadCompleted(DownloadItemImpl* download) override;
  bool ShouldUseSaveAsDownloadMode(GURL url, bool find_delete) override;
  bool ShouldUseSpeedDownloadMode(DownloadItemImpl* download);

  void AddUrlDownloader(
      std::unique_ptr<UrlDownloader, BrowserThread::DeleteOnIOThread>
          downloader);

  std::string GetCookies(const GURL& tab_url);

  // add the new download info to the list above
  void AddDownloadToDataList(const GURL& url,
      DownloadUrlParameters::UrlSourceFrom from);

  // Check is the save type download, if it is, we delete the element
  // and return true. Otherwise, return false
  bool CheckIsSaveTypeDownload(const GURL& url, bool find_delete);

  // Clear all element when the browser shutdown
  void ClearUrlSourceFromList();
  ...
  DownloadMap SavePackage_downloads_;
  ...
  std::vector<UrlSourceFromData*> url_source_from_data_list_;
