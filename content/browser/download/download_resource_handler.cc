...
void DownloadResourceHandler::OnStart(
  ...
  std::map<std::string, net::AuthCredentials>::iterator iter = net::FtpAuthCache::g_auths_map.find(request()->url().GetOrigin().spec());
  if (iter != net::FtpAuthCache::g_auths_map.end())  {
      create_info->auth_credential = (iter->second);      
  }
