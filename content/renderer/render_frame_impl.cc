...
#include "content/public/browser/resource_dispatcher_host.h"//add by wangn
...
bool RenderFrameImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameImpl, msg)
    IPC_MESSAGE_HANDLER(FrameMsg_FromJsSendMsg, OnJsReceived)//add by wangn
    ...
    IPC_MESSAGE_HANDLER(FrameMsg_RestoreVideoPopupWindow,
                        OnRestoreVideoPopupWindow)
...
blink::WebPlugin* RenderFrameImpl::CreatePlugin(
  ...
  if (info.type == WebPluginInfo::PLUGIN_TYPE_NPAPI) {
    ...
    return new WebPluginImpl(frame, params, info.path, render_view_, this, gbkEncoding_);
  }
#endif
#endif
  return NULL;
...
void RenderFrameImpl::loadErrorPage(int reason) {//add by wangn
...
blink::WebURLError RenderFrameImpl::CreateError() {//add by wangn
//  printf("RenderFrameImpl::CreateError\n");
//  printf("copyu:%s\n",GetLoadingUrl().spec().c_str());
  int error_code = net::ERR_BLOCKED_BY_CLIENT;
  bool has_has_copy_in_cache = true;
  blink::WebURLError error =
      CreateWebURLError(GetLoadingUrl(), has_has_copy_in_cache, error_code);
//  printf("###########CreateError_error.reason:%d\n",error.reason);
  return error;
}


void RenderFrameImpl::OnJsReceived(int flag) {//add by wangn
  //printf("=======RenderFrameImpl::OnJsReceived:flag:%d\n",flag);
  if (!this->IsMainFrame()) {
    return;
  }
  ...
  item.initialize();
  long long seqnum = 100;
  item.setItemSequenceNumber(seqnum);
//  printf("OnJs:item.itemSequenceNumber():%lld\n",item.itemSequenceNumber());

  blink::WebURLError errjs = CreateError();

  int error_code = net::ERR_BLOCKED_BY_CLIENT;
  if(item.itemSequenceNumber() == 100)//add by wangn
    loadErrorPage(error_code);
  this->didCommitProvisionalLoad(this->GetWebFrame(),item,blink::WebStandardCommit);
}
...
void RenderFrameImpl::runModalAddFavorite(const blink::WebString& url, const blink::WebString& title) {
  Send(new FrameHostMsg_JavaScriptAddFavoriteMessage(routing_id_, url, title));
}
...
void RenderFrameImpl::OnRestoreVideoPopupWindow(const std::string& extension_id) {
    const std::string javascript = "var extensionId = \"" + extension_id + "\"; chrome.runtime.sendMessage(extensionId, {isVideoPopupWindow: false})";
    this->ExecuteJavaScript(base::UTF8ToUTF16(javascript));
    //printf("[%s]\n", javascript.c_str());
}
...
/*void RenderFrameImpl::RegisterPluginDelegate(WebPluginDelegateProxy* delegate) {
...
void RenderFrameImpl::UnregisterPluginDelegate(
...
void RenderFrameImpl::ConnectErrorPageInfo(const blink::WebURLError& error_info) {
  std::string error_url = error_info.unreachableURL.string().utf8();
  int error_code = error_info.reason;

  Send(new FrameHostMsg_CollectErrorPageInfo(routing_id_, error_url, error_code));
