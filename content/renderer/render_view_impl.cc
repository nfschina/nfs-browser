...
void RenderView::ApplyWebPreferences(const WebPreferences& prefs,
  ...
  settings->setClickPrerenderEnabled(prefs.click_prerender_enabled);
  settings->setMouseOverPrerenderEnabled(prefs.mouse_over_prerender_enabled);
  ...
  base::SysInfo::SetMacAddresNfs(prefs.mac_address);
#endif

#if defined(OS_WIN)
  WebRuntimeFeatures::enableMiddleClickAutoscroll(true);
#endif
  //add by wdg  for browsering mode
  settings->setDefalutBrowserMode(prefs.defalut_browser_mode);

#if defined(OS_LINUX)
  base::SysInfo::SetOperatingSystemName(prefs.operating_system_name);
#endif

  settings->setCompatibleMode(prefs.compatible_mode);
}
...
bool RenderViewImpl::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderViewImpl, message)
    ...
    IPC_MESSAGE_HANDLER(PageMsg_UpdateWindowVisibility,
                        OnUpdateWindowVisibility)
...
void RenderViewImpl::OnUpdateWindowScreenRect(gfx::Rect window_screen_rect) {
  ...
  for (plugin_it = plugin_delegates_.begin();
    ...
    (*plugin_it)->UpdateWindowScreenRect();
  }
}

void RenderViewImpl::UpdatePluginWindowScreenRect() {
  std::set<WebPluginDelegateProxy*>::iterator plugin_it;
  for (plugin_it = plugin_delegates_.begin();
       plugin_it != plugin_delegates_.end(); ++plugin_it) {
    (*plugin_it)->UpdateWindowScreenRect();
...
void RenderViewImpl::OnUpdateWindowVisibility(bool visibility) {
  std::set<WebPluginDelegateProxy*>::iterator plugin_it;
  for (plugin_it = plugin_delegates_.begin();
       plugin_it != plugin_delegates_.end(); ++plugin_it) {
    (*plugin_it)->UpdateVisibility(visibility);
  }
}
