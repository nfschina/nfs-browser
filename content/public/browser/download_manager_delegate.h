...
class CONTENT_EXPORT DownloadManagerDelegate {
 public:
  ...
  virtual int GetDownloadMaxSpeed();
  virtual base::FilePath GetUserDataPath();
  virtual bool ShouldNoteDownloadQuit() const;
  virtual bool ShouldContinueLastDownload() const;
  virtual bool ShouldPromptForDownload() const;
  ...
  virtual std::string GetReferrerCharset() const;
  virtual std::string GetDownloadDefaultFileName() const;

  // Handle any delegate of a state complete operation on the downloadItem.
  virtual void DownloadCompleted(DownloadItem* download) {}

  // Show the new download windows for create download
  virtual void ShowCreateDownload(content::WebContents* web_contents) {}

  // Show the right menu
  virtual void ShowDownloadRightMenu(DownloadItem* download, 
    content::WebContents* web_contents, int point_x, int point_y) {}

  // Should update the download progressing
  virtual bool ShouldDownloadUpdate();
