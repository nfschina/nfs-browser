...
std::string BuildOSCpuInfo() {
  ...
  base::StringAppendF(
      ...
      cputype.c_str(),    // e.g. i686
      (base::SysInfo::GetOperatingSystemName() &&
       strlen(base::SysInfo::GetOperatingSystemName())) ?
          base::SysInfo::GetOperatingSystemName() : unixinfo.sysname
#endif
  );  // NOLINT
...
std::string BuildUserAgentFromOSAndProduct(const std::string& os_info,
  ...
  std::string additional_mark = "open";
  base::StringAppendF(
      &user_agent,
      "Mozilla/5.0 (%s) AppleWebKit/%d.%d (KHTML, like Gecko) Chrome/%d.%d.%d.%d Safari/%d.%d %s %s",
      os_info.c_str(),
      WEBKIT_VERSION_MAJOR,
      WEBKIT_VERSION_MINOR,
