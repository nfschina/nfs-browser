...
class URL_EXPORT GURL {
 public:
  ...
  bool SchemeIsData() const {
    return SchemeIs(url::kDataScheme);
  }

  // Returns true if the scheme is "bt".
  bool SchemeIsBt() const {
    bool bt_enabled = base::CommandLine::ForCurrentProcess()->HasSwitch(
      "enable_bt");
    return bt_enabled && SchemeIs(url::kBtScheme);
  }

  bool SchemeIsFtp() const {
    return SchemeIs(url::kFtpScheme);
  }
