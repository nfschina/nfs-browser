...
ParserBase<Impl>::ParseHoistableDeclaration(
  ...
  if (default_export && peek() == Token::LPAREN) {
    ...
    if (peek() == Token::PERIOD) {
      char tmp[50];
      strcpy(tmp, (char*)name->raw_data());
      while (peek() == Token::PERIOD || peek() == Token::IDENTIFIER) {
          scanner()->Next();
          if (peek() == Token::PERIOD) {
            strcat(tmp, ".");
          }
          if (scanner()->current_token() == Token::IDENTIFIER) {
            const unsigned char* tt = scanner()->CurrentSymbol(ast_value_factory())->raw_data();
            strcat(tmp, (char*)tt);
          }
      }
      impl()->updateName(&name, ast_value_factory()->GetOneByteString(tmp));
    }
