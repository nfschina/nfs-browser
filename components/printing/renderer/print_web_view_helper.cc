...
void PrintWebViewHelper::PrintHeaderAndFooter(
  ...
  const std::string shielding = std::string("nfsbrowser");
  const std::string url = base::UTF16ToASCII(params.url);
  if(url.find(shielding) != std::string::npos)
    options->SetString("url", std::string());
  else
