...
void PasswordStoreDefault::ResetLoginsDB(
...
void PasswordStoreDefault::PostAllPasswordsToServer() {
  base::Time begin_time = base::Time();
  base::Time end_time = base::Time();
  ScopedVector<autofill::PasswordForm> forms;
  if(login_db_->GetLoginsCreatedBetween(begin_time, end_time, &forms)) {
    for (ScopedVector<autofill::PasswordForm>::iterator it = forms.begin();
        it != forms.end(); ++it) {
      const autofill::PasswordForm password= **it;
      PostData(password, "add");
    }
  }
}
