...
void PasswordStore::AddLogin(const PasswordForm& form, bool from_sync) {
  CheckForEmptyUsernameAndPassword(form);
  ScheduleTask(base::Bind(&PasswordStore::AddLoginInternal, this, form, from_sync));
}

void PasswordStore::UpdateLogin(const PasswordForm& form, bool from_sync) {
  CheckForEmptyUsernameAndPassword(form);
  ScheduleTask(base::Bind(&PasswordStore::UpdateLoginInternal, this, form, from_sync));
}
...
void PasswordStore::RemoveLogin(const PasswordForm& form, bool from_sync) {
  ScheduleTask(base::Bind(&PasswordStore::RemoveLoginInternal, this, form, from_sync));
}
...
PasswordStoreChangeList PasswordStore::AddLoginSync(const PasswordForm& form) {
  ...
  return AddLoginImpl(form, false);
}
...
PasswordStoreChangeList PasswordStore::UpdateLoginSync(
  ...
  return UpdateLoginImpl(form, false);
}

PasswordStoreChangeList PasswordStore::RemoveLoginSync(
    const PasswordForm& form) {
  return RemoveLoginImpl(form, false);
}
...
void PasswordStore::AddLoginInternal(const PasswordForm& form, bool from_sync) {
  PasswordStoreChangeList changes = AddLoginImpl(form, from_sync);
  NotifyLoginsChanged(changes);
}

void PasswordStore::UpdateLoginInternal(const PasswordForm& form, bool from_sync) {
  PasswordStoreChangeList changes = UpdateLoginImpl(form, from_sync);
  NotifyLoginsChanged(changes);
}

void PasswordStore::RemoveLoginInternal(const PasswordForm& form, bool from_sync) {
  PasswordStoreChangeList changes = RemoveLoginImpl(form, from_sync);
  NotifyLoginsChanged(changes);
}
...
void PasswordStore::UpdateLoginWithPrimaryKeyInternal(
  ...
  PasswordStoreChangeList all_changes = RemoveLoginImpl(old_primary_key, false);
  PasswordStoreChangeList changes = AddLoginImpl(new_form, false);
...
void PasswordStore::UpdateAffiliatedWebLoginsImpl(
  ...
  for (const std::string& affiliated_web_realm : affiliated_web_realms) {
    ...
    for (auto& web_login : web_logins) {
      ...
      PasswordStoreChangeList changes = UpdateLoginImpl(*web_login, false);
      all_changes.insert(all_changes.end(), changes.begin(), changes.end());
    }
  }
  NotifyLoginsChanged(all_changes);
