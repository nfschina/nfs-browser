...
class TranslateScript {
 public:
  ...
  const std::string& data() { return local_data; } //disable by wdg
//  // Used by unit-tests to override some defaults:
 ...
 private:
  ...
  //static const char kFakeScriptURL[];  //add by wdg  for the translate flow
 ...
 //  static const char kJavascriptLoaderCallbackQueryValue[];
  ...
  // void OnScriptFetchComplete_Google(int id, bool success, const std::string& data);
