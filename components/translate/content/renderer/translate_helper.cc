...
bool TranslateHelper::StartTranslation(const std::string& script) {
  // std::string script = "cr.googleTranslate.translate('" +
  //                      source_lang_ +
  //                      "','" +
  //                      target_lang_ +
  //                      "')";
  // printf(" the  helper  StartTranslation:  %s \n", script.c_str());
...
bool TranslateHelper::ExecuteScriptAndGetBoolResult(const std::string& script,
  ...
  WebVector<v8::Local<v8::Value> > results;
   //WebScriptSource source = WebScriptSource(ASCIIToUTF16(script)); 
   WebScriptSource source = WebScriptSource(base::string16(script.begin(), script.end()));
  main_frame->executeScriptInIsolatedWorld(
...
// void TranslateHelper::OnTranslatePage(  //old
void TranslateHelper::Translate(
  ...
  // }
   TranslatePageImpl(0,translate_script);
}
...
void TranslateHelper::TranslatePageImpl(int count,const std::string& script) {
  ...
  if (!IsTranslateLibReady()) {
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
        ...
        base::Bind(&TranslateHelper::TranslatePageImpl,
                   weak_method_factory_.GetWeakPtr(), count,script),
