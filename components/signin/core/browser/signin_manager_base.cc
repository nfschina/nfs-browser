  ...
  if (!account_id.empty()) {
    if (account_tracker_service_->GetMigrationState() ==
      ...
      AccountInfo account_info =
          account_tracker_service_->FindAccountInfoByEmail(GetAccountEmail());
...
void SigninManagerBase::SetAuthenticatedAccountId(
  ...
  // chromium的机制是不同的用户分别创建不同的配置文件并保存账号信息
  // 退出时记录最近关闭的帐号的信息
  // 启动时检查两个帐号信息是否一致
  // 目前NFSBrowser采用的是多个用户共用配置文件的策略
  // 所以这段DCHECK可以暂时屏蔽
...
std::string SigninManagerBase::GetAccountID() {
  if (!client_->GetPrefs()) {
    return "";
  }

  const base::ListValue* list =
      client_->GetPrefs()->GetList("account_info");
  for (size_t i = 0; i < list->GetSize(); ++i) {
    const base::DictionaryValue* dict;
    if (list->GetDictionary(i, &dict)) {
      std::string account_id;
      if (dict->GetString("account_id", &account_id)) {
        return account_id;
      }
    }
  }
...
std::string SigninManagerBase::GetAccountEmail() {
  if (!client_->GetPrefs()) {
    return "";
  }
