...
// const char kUploadURL[] = "http://172.30.28.43/testphp.php";
const char kUploadURL[] = "http://180.167.10.100/breakpad/index.php";
#endif
...
const char g_dump_msg[] = "upload_file_minidump\"; filename=";
#if defined(ADDRESS_SANITIZER)
...
void EnableNonBrowserCrashDumping() {
  ...
  base::FilePath dumps_path(tmp_path);
  GetCrashReporterClient()->GetCrashDumpLocation(&dumps_path);

  g_breakpad = new ExceptionHandler(
      MinidumpDescriptor(dumps_path.value()),  // Unused but needed or Breakpad will assert.
      nullptr,
      nullptr,
      nullptr,
      true,
...
void ExecUploadProcessOrTerminate(const BreakpadInfo& info,
  ...
  const std::string user_agent = "--user-agent=Linux/" +
  ...
  const char* args[] = {
    ...
    post_file,
    kUploadURL,
    "--timeout=100",  // Set a timeout so we don't hang forever.
    "--tries=2",     // Don't retry if the upload fails.
...
void HandleCrashDump(const BreakpadInfo& info) {
  ...
  // format the dump file name use guid
  const char* dump_file_path = info.filename;
  const char* basename = my_strrchr(dump_file_path, '/');
  basename = basename == NULL ? dump_file_path : (basename + 1);

  // get the base name
  char dump_file_name[NAME_MAX];
  my_strlcpy(dump_file_name, basename, sizeof(dump_file_name));

  const char tmp_flag[] = "\"";
  const size_t file_name_msg_size = my_strlen(dump_file_name) + sizeof(tmp_flag) * 2;
  char* file_name_msg = reinterpret_cast<char*>(allocator.Alloc(file_name_msg_size));
  my_strlcpy(file_name_msg, tmp_flag, file_name_msg_size);
  my_strlcat(file_name_msg, dump_file_name, file_name_msg_size);
  my_strlcat(file_name_msg, tmp_flag, file_name_msg_size);

  const size_t dump_msg_size = sizeof(g_dump_msg) + my_strlen(file_name_msg);
  char* dump_msg_content = reinterpret_cast<char*>(allocator.Alloc(dump_msg_size));
  my_strlcpy(dump_msg_content, g_dump_msg, dump_msg_size);
  my_strlcat(dump_msg_content, file_name_msg, dump_msg_size);

  //writer.AddFileContents(g_dump_msg, dump_data, dump_size);
  writer.AddFileContents(dump_msg_content, dump_data, dump_size);
#if defined(ADDRESS_SANITIZER)
