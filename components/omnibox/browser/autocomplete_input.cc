...
bool AutocompleteInput::prevent_inline_autocomplete() const {
  static Profile* profile = g_browser_process->profile_manager()->GetActiveUserProfile();
  return prevent_inline_autocomplete_ || !profile->GetPrefs()->GetBoolean(prefs::kAddressAutofill);
}
