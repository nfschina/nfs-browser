...
const TemplateURL* SearchProvider::Providers::GetBaiduProviderURL() const {
  return default_provider_.empty() ? NULL :
      template_url_service_->GetTemplateURLForKeyword(base::ASCIIToUTF16("baidu.com"));
}
...
void SearchProvider::Run(bool query_is_private) {
  ...
  if (!query_is_private) {
    ...
    default_fetcher_ =
     ...
     #else
          CreateSuggestFetcher(kDefaultProviderURLFetcherID,
                             providers_.GetBaiduProviderURL(), input_);
     #endif
  }
  keyword_fetcher_ =
...
void SearchProvider::DoHistoryQuery(bool minimal_changes) {
  ...
  if (!profile->GetPrefs()->GetBoolean(prefs::kAddressHistory))  {
    return;
  }
