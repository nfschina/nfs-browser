...
std::string GetShortUrl(const std::string& url_str) {
  GURL url(url_str);
...
void OmniboxEditModel::Revert() {
  ...
  //[zhangyq]
  if (!has_focus()) {
    base::string16 short_url = base::UTF8ToUTF16(GetShortUrl(base::UTF16ToUTF8(permanent_text_)));
    view_->SetWindowTextAndCaretPos(short_url,
...
bool OmniboxEditModel::OnEscapeKeyPressed() {
  if (dropdown_model() && dropdown_model()->IsOpen()) {
    RevertTemporaryText(true);
    dropdown_model()->OnEscapeKeyPressed();
  }
...
void OmniboxEditModel::RevertTemporaryText(bool revert_popup) {
  ...
  if (revert_popup && popup_model() && PopupIsOpen())
