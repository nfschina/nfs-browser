...
void LoadCallback(const base::FilePath& path,
  ...
  if (bookmark_file_exists) {
    ...
    if (command_line->HasSwitch(chrome::kDisableEncrypt_Bookmark))
      root = deserializer.Deserialize(NULL, NULL);
    else {
      std::string json_string;
      bool read = base::ReadFileToString(path, &json_string);
      if (read) {
        crypto::Encryptor encryptor;
        encryptor.Init(chrome::key_Bookmark.get(), crypto::Encryptor::CBC,
            chrome::iv_Bookmark);
        std::string decrypted;
        encryptor.Decrypt(json_string, &decrypted);
        json_string = decrypted;
      }
      ///end by wdg for descrypt the bookmark json file
      base::JSONReader  reader;
      root  = reader.ReadToValue(json_string);
    } //encry
    ...
    if (root.get()) {
      ...
      codec.Decode(details->bb_node(), &max_node_id, *root.get());
...
bool BookmarkStorage::SerializeData(std::string* output) {
  ...
  if (command_line->HasSwitch(chrome::kDisableEncrypt_Bookmark))
    ...
    //add for the bookmark_crypt
    bool temp = serializer.Serialize(*(value.get()));
    crypto::Encryptor encryptor;
    encryptor.Init(chrome::key_Bookmark.get(), crypto::Encryptor::CBC,
        chrome::iv_Bookmark);
