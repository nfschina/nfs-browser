...
const PreBookmarks kCommonUsage[] = {
  {"腾讯", "http://www.qq.com/"},
  {"京东", "http://www.jd.com/"},
...
void LoadPreBookmarks(const BookmarkNode* node, BookmarkModel* model) {
  for (size_t i = 0; i < arraysize(kCommonUsage); ++i) {
    model->AddURL(node, i,
                  base::UTF8ToUTF16(kCommonUsage[i].title),
                  GURL(kCommonUsage[i].url));
  }
}
...
void BookmarkModel::DoneLoading(std::unique_ptr<BookmarkLoadDetails> details) {
  ...
  // Load pre bookmakrs
  if (first_run::ShouldShowBookmarkBar()) {
    BrowserThread::PostTask(BrowserThread::UI, FROM_HERE,
        base::Bind(&LoadPreBookmarks, bookmark_bar_node_, this));
  }
...
std::unique_ptr<BookmarkLoadDetails> BookmarkModel::CreateLoadDetails() {
  ...
  BookmarkPermanentNode* bb_node =
      CreatePermanentNode(BookmarkNode::BOOKMARK_BAR);
  return std::unique_ptr<BookmarkLoadDetails>(new BookmarkLoadDetails(
      bb_node, client_->GetLoadExtraNodesCallback(),
      new BookmarkIndex(client_.get()), next_node_id_));
#endif
}
