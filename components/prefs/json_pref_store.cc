...
// #include "chrome/common/chrome_key.h"  //wdg
#include "base/json/json_reader.h" //wdg
#include "base/command_line.h"//wdg
#include "components/prefs/components_key.h"  //wdg
#include "chrome/common/chrome_switches.h"  //wdg

// const std::string iv_json("the iv: 16 bytes");
// const std::string iv_json = chrome::iv_json;
...
std::unique_ptr<JsonPrefStore::ReadResult> ReadPrefsFromDisk(
 ...
 base::CommandLine::ForCurrentProcess();
  if (command_line->HasSwitch(components::kDisableEncrypt))
 // if (1)
 {
  ...
  {
    error_code = base::ReadFileToString(path, &json_string);
     if(error_code)
   {
  // scoped_ptr<base::Value> root  = reader.ReadToValue(json_string);
  crypto::Encryptor encryptor;
  encryptor.Init(components::key_json.get(), crypto::Encryptor::CBC, components::iv_json);
  std::string decrypted;
  encryptor.Decrypt(json_string, &decrypted);
  json_string = decrypted;  
  read_result->value =   reader.ReadToValue(json_string);
   JSONFileValueDeserializer deserializer(path);
  read_result->error =
    HandleReadErrors(read_result->value.get(), path, error_code, error_msg);
  read_result->no_dir = !base::PathExists(path.DirName());
...
if (command_line->HasSwitch(components::kDisableEncrypt))
 // if(1)
  ...
  { 
    bool  serializer_success =  serializer.Serialize(*prefs_);   
    crypto::Encryptor encryptor;
    encryptor.Init(components::key_json.get(), crypto::Encryptor::CBC, components::iv_json);
    std::string encrypted;
    encryptor.Encrypt(*output,&encrypted);
    *output = encrypted;
