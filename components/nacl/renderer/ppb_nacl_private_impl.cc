...
const char* kPNaClTranslatorBaseUrl = "nfsbrowser://pnacl-translator/";
...
PP_Bool PPBNaClPrivate::GetPnaclResourceInfo(PP_Instance instance,
  ...
  static const char kFilename[] = "nfsbrowser://pnacl-translator/pnacl.json";
  NexeLoadManager* load_manager = GetNexeLoadManager(instance);
  DCHECK(load_manager);
  if (!load_manager)
    return PP_FALSE;
  ...
  if (!file.IsValid()) {
    load_manager->ReportLoadError(
        ...
        "installed. Please consult nfsbrowser://components for more "
        "information.");
    return PP_FALSE;
  }
