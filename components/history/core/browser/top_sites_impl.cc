...
struct PrepopulatedCapture {
  int capture_id;
  const char* url;
} kPrepopulatedCapture[] = {
  {
    IDR_CDOS_THUMB_BAIDU,
    "https://www.baidu.com/"
  },
  {
    IDR_CDOS_THUMB_IQIYI,
    "http://www.iqiyi.com/",
  },
  {
    IDR_CDOS_THUMB_JD,
  ...
  {
    IDR_CDOS_THUMB_QQZONE,
    "http://www.qzone.com/",
  },
  {
    IDR_CDOS_THUMB_TAOBAO,
    "https://www.taobao.com/"
  },
  {
    IDR_CDOS_THUMB_WEIBO,
    "http://www.weibo.com/",
  },
  {
    IDR_CDOS_THUMB_XINLANG,
    "http://www.sina.com.cn/"
  },
  {
    IDR_CDOS_THUMB_YOUKU,
    "http://www.youku.com/",
  },
};
...
bool TopSitesImpl::SetPageCapture(const GURL& url,
                                  const base::string16& title,
                                  int index,
                                  const gfx::Image& capture) {
  DCHECK(thread_checker_.CalledOnValidThread());
  ...
  if (!EncodeBitmap(capture, &capture_data))
    return false;

  return SetPageCaptureEncoded(url, title, index, capture_data);
}
...
// added by wangpp
bool TopSitesImpl::GetPageCapture(
    const GURL& url,
    scoped_refptr<base::RefCountedMemory>* bytes) {
  for (size_t i = 0; i < arraysize(kPrepopulatedCapture); ++i) {
    if (url.spec() == std::string(kPrepopulatedCapture[i].url)) {
      *bytes =
          ResourceBundle::GetSharedInstance().LoadDataResourceBytes(
              kPrepopulatedCapture[i].capture_id);
      return true;
    }
  }
  ...
  {
    base::AutoLock lock(lock_);
    if (thread_safe_cache_->GetPageCapture(url, bytes))
      return true;
  }
...
// added by wangpp
bool TopSitesImpl::SetPageCaptureEncoded(
    const GURL& url,
    const base::string16& title,
    int index,
    scoped_refptr<base::RefCountedBytes> capture_data) {
  Images thumbnail;
  thumbnail.thumbnail = capture_data.get();
  // Update Captures.
  {
    base::AutoLock lock(lock_);
    if (thread_safe_cache_->IsCaptureExist(url, thumbnail))
      return false;
  }

  backend_->SetPageCapture(url, title, index, capture_data);

  return true;
}

void TopSitesImpl::OnCaptureChanged(int index, const GURL& url) {
  LOG(ERROR) << "OnCaptureChanged";
}
...
void TopSitesImpl::ResetThreadSafeImageCache() {
  ...
  thread_safe_cache_->SetCaptures(cache_->captures());
}
...
void TopSitesImpl::OnGotMostVisitedThumbnails(
    ...
    const scoped_refptr<MostVisitedThumbnails>& captures) {
  DCHECK(thread_checker_.CalledOnValidThread());
  ...
  cache_->SetCaptures(captures->url_to_images_map);
...
void TopSitesImpl::OnURLVisited(HistoryService* history_service,
  ...
  if(IsBlacklisted(row.url())) {
    RemoveBlacklistedURL(row.url());
  }

  StartQueryForMostVisited();
}

void TopSitesImpl::OnURLsModified(HistoryService* history_service,
