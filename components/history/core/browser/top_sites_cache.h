...
class TopSitesCache {
 public:
  ...
  // The Captures.
  void SetCaptures(const URLToImagesMap& captures);
  const URLToImagesMap& captures() const { return captures_; }
  ...
  // Returns true if |url| is exist.
  bool IsCaptureExist(const GURL& url, Images image);
