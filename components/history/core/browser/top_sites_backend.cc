...
void TopSitesBackend::GetMostVisitedThumbnails(
  ...
  scoped_refptr<MostVisitedThumbnails> captures = new MostVisitedThumbnails();
  tracker->PostTaskAndReply(
      db_task_runner_.get(), FROM_HERE,
      base::Bind(&TopSitesBackend::GetMostVisitedThumbnailsOnDBThread, this,
                 thumbnails, captures),
      base::Bind(callback, thumbnails, captures));
}
...
// added by wangpp
void TopSitesBackend::SetPageCapture(const GURL& url,
                                     const base::string16& title,
                                     int index,
                                     scoped_refptr<base::RefCountedBytes> capture_data) {
  db_task_runner_->PostTask(
      FROM_HERE, base::Bind(&TopSitesBackend::SetPageCaptureOnDBThread, this,
                            url, title, index, capture_data));
}
...
void TopSitesBackend::GetMostVisitedThumbnailsOnDBThread(
    ...
    scoped_refptr<MostVisitedThumbnails> captures) {
  DCHECK(db_task_runner_->BelongsToCurrentThread());

  if (db_) {
    db_->GetPageThumbnails(&(thumbnails->most_visited),
                           &(thumbnails->url_to_images_map));
    db_->GetPageCaptures(&(captures->most_visited),
                         &(captures->url_to_images_map));
  }
}
...
void TopSitesBackend::SetPageCaptureOnDBThread(
    const GURL& url,
    const base::string16& title,
    int index,
    scoped_refptr<base::RefCountedBytes> capture_data) {
  ...
  db_->SetPageCapture(url, title, index, capture_data);
}
