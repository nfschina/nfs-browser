...
base::CancelableTaskTracker::TaskId HistoryService::QueryActiveHistoryDate(const QueryVisitDateCallback& callback,
                                                     base::CancelableTaskTracker* tracker) {
  DCHECK(thread_checker_.CalledOnValidThread());
  std::vector<int>* query_dates = new std::vector<int>();
  return tracker->PostTaskAndReply(
      thread_->task_runner().get(),
      FROM_HERE,
      base::Bind(&HistoryBackend::QueryActiveHistoryDate,
                 history_backend_.get(),
                 base::Unretained(query_dates)),
      base::Bind(callback, base::Owned(query_dates)));
}
