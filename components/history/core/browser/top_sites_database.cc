...
bool InitTables(sql::Connection* db) {
  ...
  return db->Execute(kThumbnailsSql) && db->Execute(kCapturesSql);
}
...
// Encodes redirects into a string.
// std::string GetCaptureRedirects(const MostVisitedURL& url) {
//   std::vector<std::string> redirects;
//   for (size_t i = 0; i < url.redirects.size(); i++)
//     redirects.push_back(url.redirects[i].spec());
//   return base::JoinString(redirects, " ");
...
// Decodes redirects from a string and sets them for the url.
void SetCaptureRedirects(const std::string& redirects, MostVisitedURL* url) {
  for (const std::string& redirect : base::SplitString(
           redirects, base::kWhitespaceASCII,
           base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
    GURL redirect_url(redirect);
...
// added by wangpp ------------------------------------------------------------
void TopSitesDatabase::GetPageCaptures(MostVisitedURLList* urls,
                                      URLToImagesMap* thumbnails) {
  sql::Statement statement(db_->GetCachedStatement(
      SQL_FROM_HERE,
      "SELECT url, title, id, capture "
      "FROM captures ORDER BY url"));
  ...
  while (statement.Step()) {
    ...
    GURL gurl(statement.ColumnString(0));
    url.url = gurl;
    url.title = statement.ColumnString16(1);
    urls->push_back(url);
...
// added by wangpp
void TopSitesDatabase::SetPageCapture(
    const GURL& url,
  ...
  AddPageCapture(url, title, index, capture_data);
...
// added by wangpp
void TopSitesDatabase::AddPageCapture(
    const GURL& url,
    const base::string16& title,
    int index,
    scoped_refptr<base::RefCountedBytes> capture_data) {
  sql::Statement statement(db_->GetCachedStatement(
      ...
      "(url, title, id, capture) "
      "VALUES (?, ?, ?, ?)"));

  statement.BindString(0, url.spec());
  statement.BindString16(1, title);
  statement.BindInt(2, index);
  if (capture_data.get() || capture_data.get()->front()) {
    statement.BindBlob(3, capture_data.get()->front(),
                       static_cast<int>(capture_data.get()->size()));
  }
  if (!statement.Run())
    return ;
}
...
// Added by wangpp ------------------------------------------------------------
bool TopSitesDatabase::GetPageCapture(const GURL& url, Images* thumbnail) {
  sql::Statement statement(db_->GetCachedStatement(
      SQL_FROM_HERE,
      "SELECT capture FROM captures WHERE url=?"));
  statement.BindString(0, url.spec());
  if (!statement.Step())
    return false;
