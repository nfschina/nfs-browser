...
void TopSitesCache::SetCaptures(const URLToImagesMap& captures) {
  captures_ = captures;
}
...
bool TopSitesCache::GetPageCapture(
    const GURL& url,
    scoped_refptr<base::RefCountedMemory>* bytes) const {
  std::map<GURL, Images>::const_iterator found =
      captures_.find(GetCanonicalURL(url));
...
bool TopSitesCache::IsCaptureExist(const GURL& url,
                                   Images image) {
  std::map<GURL, Images>::const_iterator found =
      captures_.find(GetCanonicalURL(url));
  if (found != captures_.end()) {
  ...
  captures_.insert(std::pair<GURL, Images>(url, image));
  return false;
}
