...
bool VisitDatabase::InitVisitTable() {
  ...
  // huk 增加每天的访问次数
  if (!GetDB().DoesTableExist("daily_visit")) {
    if (!GetDB().Execute("CREATE TABLE daily_visit("
                         "date INTEGER PRIMARY KEY,visit_num INTEGER)"))
        return false;
   }
...
bool VisitDatabase::DropVisitTable() {
  ...
  return
      ...
      GetDB().Execute("DROP TABLE IF EXISTS daily_visit") &&      
...
int VisitDatabase::GetDate(const base::Time& visit_time) {
   base::Time::Exploded explod;
   visit_time.LocalExplode(&explod);
   int year = explod.year;
   int month = explod.month;
   int day = explod.day_of_month;

   // something like 20160809
   return year * 10000 + month * 100 + day;
}

void VisitDatabase::AddDailyVisit(VisitRow* visit, VisitSource source) {
  int date = GetDate(visit->visit_time);

  sql::Statement query_statement(GetDB().GetCachedStatement(SQL_FROM_HERE,
      "SELECT "  " date, visit_num"  " FROM daily_visit "
      " WHERE date = ? "));

  query_statement.BindInt64(0, date);
  int old_num = 0;
  bool found = false;
  if (query_statement.Step()) {
    old_num = query_statement.ColumnInt64(1);
    found = true;
  }

  if (!found)  {
    // insert 
    sql::Statement insert_statement(GetDB().GetCachedStatement(SQL_FROM_HERE,
         "INSERT INTO daily_visit (date, visit_num)  VALUES (?, ?)"));
    insert_statement.BindInt64(0, date);
    insert_statement.BindInt64(1, 1);
    ...
    sql::Statement update_statement(GetDB().GetCachedStatement(SQL_FROM_HERE,
         "UPDATE  daily_visit SET visit_num = visit_num + 1 WHERE date = ?"));

    update_statement.BindInt64(0, date);
...
void VisitDatabase::RemoveDailyVisit(const VisitRow& visit) {
  int date = GetDate(visit.visit_time);

  sql::Statement update_statement(GetDB().GetCachedStatement(SQL_FROM_HERE,
       "UPDATE  daily_visit SET visit_num = visit_num - 1 WHERE date = ? AND visit_num > 0"));
...
void VisitDatabase::GetAllVisitDays(std::vector<int>* visit_dates) {
  DCHECK(visit_dates);
  visit_dates->clear();

  sql::Statement statement(GetDB().GetCachedStatement(SQL_FROM_HERE,
      "SELECT date FROM daily_visit WHERE visit_num > 0"));

  if (!statement.is_valid())
    return;

  while (statement.Step()) {
    int date = statement.ColumnInt64(0);
    visit_dates->push_back(date);
  }
}
