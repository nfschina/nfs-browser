...
bool DownloadDatabase::InitDownloadTable() {
  const char kSchema[] =
      ...
      "pack_url VARCHAR NOT NULL,"          // http body.
      ...
      "bt_real_name VARCHAR NOT NULL,"      // BT Real Name
...
void DownloadDatabase::QueryDownloads(std::vector<DownloadRow>* results) {
  ...
  sql::Statement statement_main(GetDB().GetCachedStatement(
      ...
      "site_url, tab_url, tab_referrer_url, http_method, pack_url, by_ext_id, "
      "by_ext_name, etag, last_modified, bt_real_name FROM downloads ORDER BY start_time"));
  ...
  while (statement_main.Step()) {
    ...
    info->pack_url = statement_main.ColumnString(column++);
    ...
    info->bt_real_name = statement_main.ColumnString(column++);
...
bool DownloadDatabase::UpdateDownload(const DownloadRow& data) {
  ...
  sql::Statement statement(GetDB().GetCachedStatement(
      ...
      "opened=?, by_ext_id=?, by_ext_name=?, etag=?, last_modified=?, bt_real_name=? "
      "WHERE id=?"));
  int column = 0;
  ...
  statement.BindString(column++, data.bt_real_name);
...
bool DownloadDatabase::CreateDownload(const DownloadRow& info) {
...

  {
    sql::Statement statement_insert(GetDB().GetCachedStatement(
        ...
        " site_url, tab_url, tab_referrer_url, http_method, pack_url, "
        " by_ext_id, by_ext_name, etag, last_modified, bt_real_name) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        "        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        "        ?, ?, ?, ?, ?, ?)"));
    ...
    statement_insert.BindString(column++, info.pack_url);
    ...
    statement_insert.BindString(column++, info.bt_real_name);
