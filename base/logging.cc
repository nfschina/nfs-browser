...
bool ShouldDeleteLog(const PathString& log_name) {
  int64_t file_size = 0;
#if defined(OS_WIN)
  if (!base::GetFileSize(base::FilePath(log_name.c_str()), &file_size)) {
    return true;
  }
#elif defined(OS_POSIX)
  struct stat statbuff;
  if (stat(log_name.c_str(), &statbuff) < 0) {
    return true;
  } else {
  ...
  file_size = static_cast<int64_t>(file_size / (1024 * 1024));
  if (file_size >= 3) {
    return true;
  }
