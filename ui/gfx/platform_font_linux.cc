...
sk_sp<SkTypeface> CreateSkTypeface(bool italic,
  ...
  if (!typeface) {
    ...
    // DCHECK(typeface) << "Could not find any font: " << *family << ", "
...
PlatformFontLinux::PlatformFontLinux() {
  if (!g_default_font.Get()) {
    ...
    // huk
    // bool default_font_exist = !!CreateSkTypeface(false, weight, &family);
    const LinuxFontDelegate* delegate = LinuxFontDelegate::instance();
    if (delegate /*&& !default_font_exist*/) {
      delegate->GetDefaultFontDescription(&family, &size_pixels, &style,
                                          &weight, &params);
    }

   // huk (In Ubuntu, the font size is 15, we prefer 12) ;
   size_pixels = 14;
