  ...
  Menu.prototype = {
    ...
    handleKeyDown: function(e) {
      ...
      switch (e.key) {
        ...
        // Add a shortcut key operation with rigth
        case 'O':
        case 'o':
        case 'N':
        case 'n':
        ...
        case 'G':
        case 'g':
          var menu = this.getMenuByKeyshort(e.key);
    ...
    getMenuByKeyshort: function(key) {
      var items = this.menuItems;
      var result = null;
      if(this.shortKey) {
        result = this.shortKey[key.toUpperCase()];
        if(result && !result.hidden && !result.disabled) {
          return result;
        } else {
          return null;
        }
      ...
      for(var i = 0, len = items.length; i < len; i++) {
        ...
        if(item.dataset.keyshort) {
          this.shortKey[item.dataset.keyshort] = item;
        }

        if(item.dataset.keyshort === key.toUpperCase()) {
          result = this.shortKey[key.toUpperCase()];
