...
void SetActivationVisibility(aura::Window* window, bool visible) {
  TransientWindowManager* manager =TransientWindowManager::Get(window);
  if (manager) {
    manager->set_parent_controls_visibility(visible);
  }
}
