...
void Widget::Init(const InitParams& in_params) {
  ...
  //[zhangyq] Remove title bar and border in custom frame view.
  remove_custom_frame_ = params.remove_custom_frame;
...
void Widget::Restore() {
  ...
  int max_width = display::Screen::GetScreen()->GetPrimaryDisplay().bounds().width();
  if (max_width - native_widget_->GetRestoredBounds().width() < 5)  {
    SetSize(gfx::Size(max_width/2, native_widget_->GetRestoredBounds().height()));
  }
}
...
NonClientFrameView* Widget::CreateNonClientFrameView() {
  ...
  custom_frame_view->Init(this, remove_custom_frame_);
...
void Widget::OnNativeFocus() {
  widget_delegate_->RecordUsingNow(); // it impls the browser is active
