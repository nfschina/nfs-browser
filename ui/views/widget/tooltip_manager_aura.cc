...
// static
void TooltipManagerAura::ResetTooltipText(aura::Window* source) {
  if (!source) {
    return;
  }

  Widget* target_widget = Widget::GetWidgetForNativeView(source);
  if (target_widget && target_widget->GetTooltipManager()) {
    target_widget->GetTooltipManager()->ClearTooltip();
  }
}
...
void TooltipManagerAura::ClearTooltip() {
  tooltip_text_.clear();
 }
