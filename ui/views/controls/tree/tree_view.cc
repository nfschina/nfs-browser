...
#else
  closed_icon_ = *ui::ResourceBundle::GetSharedInstance().GetImageNamed(
      (base::i18n::IsRTL() ? IDR_CDOS_FOLDER_CLOSED_RTL
                           : IDR_CDOS_FOLDER_CLOSED)).ToImageSkia();
  open_icon_ = *ui::ResourceBundle::GetSharedInstance().GetImageNamed(
      (base::i18n::IsRTL() ? IDR_CDOS_FOLDER_OPEN_RTL
                           : IDR_CDOS_FOLDER_OPEN)).ToImageSkia();
#endif
...
void TreeView::PaintRow(gfx::Canvas* canvas,
  ...
  if (!PlatformStyle::kTreeViewSelectionPaintsEntireRow &&
      node == selected_node_) {
    canvas->FillRect(text_bounds, gfx::kGoogleBlue300);
    if (HasFocus())
