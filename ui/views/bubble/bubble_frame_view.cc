...
void BubbleFrameView::UpdateCloseButton(gfx::ImageSkia* images) {
  if (ui::MaterialDesignController::IsSecondaryUiMaterial()) {
    // close_->SetIcon(gfx::VectorIconId::BAR_CLOSE);
    // close_->SetSize(close_->GetPreferredSize());
  } else {
    LabelButton* close =static_cast<LabelButton*>(close_);
    close->SetImage(CustomButton::STATE_NORMAL, images[0]);
    close->SetImage(CustomButton::STATE_HOVERED, images[1]);
    close->SetImage(CustomButton::STATE_PRESSED, images[2]);
    close->SetBorder(nullptr);
    close->SetSize(close->GetPreferredSize());
  }
  ...
  // Windows will automatically create a tooltip for the close button based on
  // the HTCLOSE result from NonClientHitTest().
  close_->SetTooltipText(l10n_util::GetStringUTF16(IDS_APP_CLOSE));
#endif
}
