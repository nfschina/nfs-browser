...
void CustomFrameView::Init(Widget* frame, bool remove_frame) {
  remove_frame_ = remove_frame;
  Init(frame);
}
...
int CustomFrameView::FrameBorderThickness() const {
  return (frame_->IsMaximized() || remove_frame_) ? 0 : kFrameBorderThickness;
}
...
bool CustomFrameView::ShouldShowTitleBarAndBorder() const {
  if (frame_->IsFullscreen() || remove_frame_)
    return false;
