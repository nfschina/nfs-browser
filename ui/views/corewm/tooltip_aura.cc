...
void TooltipAura::SetText(aura::Window* window,
  ...
  gfx::Size last_size = tooltip_view_->GetPreferredSize(); //[zhangyq] add
  tooltip_window_ = window;
  tooltip_view_->SetMaxWidth(GetMaxWidth(location));
  tooltip_view_->SetText(tooltip_text);
  //[zhangyq] Work around to  "0032216--鼠标在夜间模式和广告拦截两个图标中移动，黄色浮动字标显示错误."
  //It's not a ideal way to fix this issue, it's a unfixed chrome issue on some computer with non-integrated graphic card.
  //This work-around should be removed when a better way comes out.
  gfx::Size current_size = tooltip_view_->GetPreferredSize();
