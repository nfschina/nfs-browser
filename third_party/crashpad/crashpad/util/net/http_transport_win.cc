...
class HTTPTransportWin final : public HTTPTransport {
 public:
  HTTPTransportWin();
  HTTPTransportWin(const std::string& user_agent);
...
HTTPTransportWin::HTTPTransportWin(const std::string& user_agent)
    : HTTPTransport()
...
bool HTTPTransportWin::ExecuteSynchronously(std::string* response_body) {
  ...
  //                 0));
  ScopedHINTERNET session(
      WinHttpOpen(base::UTF8ToUTF16(user_agent_).c_str(),
                  WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                  WINHTTP_NO_PROXY_NAME,
                  WINHTTP_NO_PROXY_BYPASS,
                  0));
...
// static
std::unique_ptr<HTTPTransport>
HTTPTransport::Create(const std::string& user_agent) {
  return std::unique_ptr<HTTPTransportWin>(new HTTPTransportWin(user_agent));
}
