...
void CrashReportUploadThread::SetUserAgent(const std::string& user_agent) {
  ...
  if (rate_limit_) {
    ...
    if (settings->GetLastUploadAttemptTime(&last_upload_attempt_time)) {
      ...
      if (now >= last_upload_attempt_time) {
        ...
        const int kUploadAttemptIntervalSeconds = 60;  // 1 minutes
...
CrashReportUploadThread::UploadResult CrashReportUploadThread::UploadReport(
  ...
  std::unique_ptr<HTTPTransport> http_transport(HTTPTransport::Create(user_agent_));
