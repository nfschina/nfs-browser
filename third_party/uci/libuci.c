...
/* exported functions */
struct uci_context *uci_alloc_context(const char* filename)
{
	...
	ctx->confdir = (char *) uci_confdir;
        if(filename)
        {
          char *uci_savedir1 = (char*)malloc(strlen(filename)+6);
          strcpy(uci_savedir1,filename);
          #if !defined(_WIN32)
          strcat(uci_savedir1,"/.uci");
          #else
          strcat(uci_savedir1,"\\.uci");
          #endif
          strcat(uci_savedir1,"\0");
          ctx->savedir = uci_savedir1;
        }
        else
...
uci_get_errorstr(struct uci_context *ctx, char **dest, const char *prefix)
{
	...
	if (dest) {
		...
		#else
		if (prefix && ctx->func) {
			*dest = (char*)malloc(strlen(prefix)+strlen(ctx->func)+6+strlen(uci_errstr[err])+strlen(error_info));
			memset(*dest,0,strlen(prefix)+strlen(ctx->func)+6+strlen(uci_errstr[err])+strlen(error_info));
		} else if (prefix && !ctx->func) {
			*dest = (char*)malloc(strlen(prefix)+6+strlen(uci_errstr[err])+strlen(error_info));
			memset(*dest,0, strlen(prefix)+6+strlen(uci_errstr[err])+strlen(error_info));
		} else if (!prefix && ctx->func) {
			*dest = (char*)malloc(strlen(ctx->func)+6+strlen(uci_errstr[err])+strlen(error_info));
			memset(*dest,0,strlen(ctx->func)+6+strlen(uci_errstr[err])+strlen(error_info));
		} else {
			*dest = (char*)malloc(7+strlen(uci_errstr[err])+strlen(error_info));
			memset(*dest,0,7+strlen(uci_errstr[err])+strlen(error_info));
		}
		err = sprintf(*dest, format, (prefix ? prefix : ""), (prefix ? ": " : ""), 
			            (ctx->func ? ctx->func : ""), (ctx->func ? ": " : ""),
						uci_errstr[err],
						error_info);
		if (err < 0)
