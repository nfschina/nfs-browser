...
int uci_load_delta(struct uci_context *ctx, struct uci_package *p, bool flush)
{
	...
	uci_foreach_element(&ctx->delta_path, e) {
		...
		#else
		if (e->name && p->e.name)
			filename = (char*)malloc(strlen(e->name)+strlen(p->e.name)+2);
		if(!filename)
			UCI_THROW(ctx, UCI_ERR_MEM);	
		memset(filename,0,strlen(e->name)+strlen(p->e.name)+2);
		if ((sprintf(filename, "%s\\%s", e->name, p->e.name) < 0))
			UCI_THROW(ctx, UCI_ERR_MEM);
		#endif
	...
	if ((asprintf(&filename, "%s/%s", ctx->savedir, p->e.name) < 0) || !filename)
		UCI_THROW(ctx, UCI_ERR_MEM);
	#else
	if (ctx->savedir && p->e.name)
		filename = (char*)malloc(strlen(ctx->savedir)+strlen(p->e.name)+2);
	if(!filename)
		UCI_THROW(ctx, UCI_ERR_MEM);	
	memset(filename,0,strlen(ctx->savedir)+strlen(p->e.name)+2);
	if ((sprintf(filename, "%s\\%s", ctx->savedir, p->e.name) < 0))
		UCI_THROW(ctx, UCI_ERR_MEM);
	#endif
