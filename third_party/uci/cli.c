...
static void cli_perror(const char *prefix)
{
	char err_info[128] = {0};

	snprintf(err_info, sizeof(err_info) - 1, "[%s] %s", appname, prefix);

	if (flags & CLI_FLAG_QUIET)
		return;

	uci_perror(ctx, err_info);
}
...
static int package_cmd(int cmd, char *tuple)
{
	...
	strncpy(tuple_o, tuple, sizeof(tuple_o));
...
static int uci_do_section_cmd(int cmd, int argc, char **argv)
{
	...
	strncpy(op_name, argv[1], sizeof(op_name));
