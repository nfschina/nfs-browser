...
String CSSStyleSheet::cssText() {
  String cssText = emptyString();
  CSSRuleList* cssRuleList = rules();
  if (cssRuleList) {
    unsigned ruleCount = cssRuleList->length();
    CSSRule* cssRule = NULL;
    for(unsigned i = 0; i < ruleCount; i++) {
      cssRule = cssRuleList->item(i);
      if (cssRule) {
        cssText.append(StringView(cssRule->cssText()));
        cssText.append(StringView(" "));
      }
    }
  }
  return cssText;
...
void CSSStyleSheet::setCSSText(const String& cssText, ExceptionState& exceptionState) {
#if 1
  //fix bug: mantis id 0233183 0232993
  // FIXME: This way causes owner Node to have no text Node.
  setText(cssText);
#else
  //fix bug: mantis id 0232993
  Document* document = ownerDocument();
  if (m_ownerNode && document) {
    Text* textNode = document->createTextNode(cssText);
    if (textNode) {
      m_ownerNode->appendChild(textNode);
    } else {
      exceptionState.throwDOMException(
      InvalidStateError, "Failed to create text node");
    }
  } else {
    exceptionState.throwDOMException(
        InvalidStateError, "Failed to set CSS text: " + cssText + "");
  }
#endif
}
