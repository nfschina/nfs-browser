...
Element* Document::createElement(const AtomicString& name,
                                 ExceptionState& exceptionState) {
  //==>the flowing code-changing are limited to some situations such as
      //==>document.createElement("<div(or other label) style(or other attributes)="color:#FF0099;PADDING-RIGHT: 0px;">"innerHTML(some standard html web elements)"</div(or other label)>");
  //==>implementation framework
      //1. parse name & attrs
      //2. create element using name
      //3. set attributes for <name>
      //4. return element ptr.
  if (url().getString().contains("172.31.58.20:9070")) {
    unsigned length = name.length();
    if (!length || !name.is8Bit())
        return nullptr;

    const LChar* char_form= name.characters8();
    LChar *characters = const_cast<LChar*>(char_form);
    if(name[0]=='<')    {
        String label;
        int i=0;
        if(characters[i++]=='<') {
            do{
                label.append(characters[i++]);
            }while(characters[i]!=' '&&characters[i]!='>');
        }

        const AtomicString atomicLabel(label);
        HTMLElement* newTagElement= HTMLElementFactory::createHTMLElement(convertLocalName(atomicLabel), *this, 0, CreatedByCreateElement);

        Vector<String> attributes_list;
        for(i=0;i<(int)name.length();i++){
            if(characters[i]=='='){
                String attributes_name,attributes_value;
                int equal_prior,equal_following;
                equal_prior=equal_following=i;
                do{
                    equal_prior--;
                }while(characters[equal_prior]!=' ');
                for(int a=equal_prior+1;a<i;a++)    attributes_name.append(characters[a]);

                if(attributes_name.contains("style")){
                    equal_following++;
                    do{
                        attributes_value.append(characters[++equal_following]);
                    }while(characters[equal_following]!='"');
                }
                else{
                    do{
                        attributes_value.append(characters[++equal_following]);
                    }while(characters[equal_following+1]!=' '&&characters[equal_following+1]!='>');
                }
                    attributes_value.replace("\'","");
                    attributes_value.replace("\"","");
                    if (Document::isValidName(AtomicString(attributes_name)))
                        newTagElement->setAttribute(AtomicString(attributes_name),AtomicString(attributes_value),exceptionState);
            }
        }
  ...
  AtomicString sub_name = name;
  while(sub_name.endsWith(0x20))
    sub_name = AtomicString(sub_name.getString().substring(0, sub_name.length() - 1));

  if (!isValidName(sub_name)) {
    exceptionState.throwDOMException(
        InvalidCharacterError,
        "The tag name provided ('" + sub_name + "') is not a valid name.");
    return nullptr;
  }

  if (isXHTMLDocument() || isHTMLDocument()) {
    if (CustomElement::shouldCreateCustomElement(sub_name))
      return CustomElement::createCustomElementSync(*this, sub_name,
                                                    exceptionState);
    return HTMLElementFactory::createHTMLElement(convertLocalName(sub_name), *this,
                                                 0, CreatedByCreateElement);
  }

  return Element::create(QualifiedName(nullAtom, sub_name, nullAtom), this);
}
...
StyleSheet* Document::createStyleSheet(ExceptionState& exceptionState) {
  if (!documentElement() || !documentElement()->firstChild()) {
    exceptionState.throwDOMException(
        InvalidStateError, "documentElement() or documentElement()->firstChild() is NULL");
    return nullptr;
  }

  Text* text = createTextNode("");
  Element* styleTag = createElement("style", exceptionState);
  styleTag->setInnerHTML("CSS Style Sheet", exceptionState);
  styleTag->appendChild(text);
  documentElement()->firstChild()->appendChild(styleTag);

  StyleSheetList& styleSheetList = styleSheets();
  for (unsigned i = 0; i < styleSheetList.length(); i++) {
    if (styleSheetList.item(i)->ownerNode() == styleTag) {
      return styleSheetList.item(i);
    }
  }
...
StyleSheet* Document::createStyleSheet(const AtomicString& cssURL, ExceptionState& exceptionState) {
  if (cssURL.isEmpty())
    return createStyleSheet(exceptionState);
  ...
  Text* text = createTextNode("");
  HTMLLinkElement* linkTag = (HTMLLinkElement*) createElement("link", exceptionState);
  linkTag->setSyncLoadSheet(true);
  linkTag->setAttribute(hrefAttr, cssURL);
  linkTag->setAttribute(relAttr, "stylesheet");
  linkTag->setAttribute(typeAttr, "text/css");
  linkTag->appendChild(text);
  documentElement()->firstChild()->appendChild(linkTag);

  StyleSheetList& styleSheetList = styleSheets();
  for (unsigned i = 0; i < styleSheetList.length(); i++) {
    if (styleSheetList.item(i)->ownerNode() == linkTag) {
      return styleSheetList.item(i);
    }
  }
...
StyleSheet* Document::createStyleSheetCdos(ExceptionState& exceptionState) {
  return createStyleSheet(exceptionState);
}

StyleSheet* Document::createStyleSheetCdos(const AtomicString& cssURL, ExceptionState& exceptionState) {
  return createStyleSheet(cssURL, exceptionState);
}
...
bool Document::callApplication(const String& app_name,
                                 ExceptionState& exceptionState) {
  // huk: todo(make an IPC to browser process)
  #if defined(OS_WIN)
    return false;
  #else 
    pid_t pid = fork();
    std::string cmd = app_name.utf8().data();
    if(pid == 0) {
