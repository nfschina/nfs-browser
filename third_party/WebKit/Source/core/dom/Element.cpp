...
inline void Element::updateName(const AtomicString& oldName,
  ...
  if (!isInTreeScope() && !isInDocumentTree())
    return;

  updateName(containingTreeScope(), oldName, newName);
}

inline void Element::updateName(TreeScope& scope,
                              const AtomicString& oldName,
                              const AtomicString& newName) {
  if (oldName == newName)
    return;
