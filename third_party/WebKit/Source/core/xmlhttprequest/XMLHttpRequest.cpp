...
void XMLHttpRequest::didReceiveData(const char* data, unsigned len) {
  ...
  if (m_responseTypeCode == ResponseTypeDocument && responseIsHTML()) {
    ...
    String text = m_decoder->decode(data, len);


    if (m_url.host().find("perbank.abchina.com") != kNotFound && 
        text.find("window.top.ABCPrintCtl4RA1.getAllMediaIDCounts") != kNotFound)  {
      text.replace("window.top.ABCPrintCtl4RA1.getAllMediaIDCounts()","1");
    }
