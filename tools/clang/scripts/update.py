...
clang_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
def UnpackClang(url, output_dir):
  EnsureDirExists(output_dir)
  if url.endswith('.zip'):
    zipfile.ZipFile(url, 'r').extractall(path=output_dir)
  else:
    tarfile.open(url, 'r').extractall(path=output_dir)
...
def UpdateClang(args):
  ...
  if not args.force_local_build:
    ...
    if sys.platform == 'win32' or sys.platform == 'cygwin':
      ...
      cds_full_url = clang_dir + '/win/' + cds_file
    elif sys.platform == 'darwin':
      cds_full_url = clang_dir + '/mac/' + cds_file
    else:
      assert sys.platform.startswith('linux')
      cds_full_url = clang_dir + '/linux/' + cds_file
    ...
    try:
      ...
      UnpackClang(cds_full_url, LLVM_BUILD_DIR)
