...
void FirefoxImporter::LoadRootNodeID(sql::Connection* db,
  ...
  std::string query;
  std::string kToolbarFolderName;
  std::string kMenuFolderName;
  std::string kUnsortedFolderName;

  if (db->DoesTableExist("moz_bookmarks_roots")) {
    query = "SELECT root_name, folder_id FROM moz_bookmarks_roots";
    kToolbarFolderName = "tookbar",
    kMenuFolderName = "menu";
    kUnsortedFolderName = "unfiled";
  } else {
    query = "SELECT guid, id FROM moz_bookmarks";
    kToolbarFolderName = "toolbar_____";
    kMenuFolderName = "menu________";
    kUnsortedFolderName = "unfiled_____";
  }
...
void FirefoxImporter::GetWholeBookmarkFolder(sql::Connection* db,
  ...
  const char select_favicon_id[] = "PRAGMA table_info([moz_places])";
  bool result = false;

  sql::Statement temp(db->GetUniqueStatement(select_favicon_id));
  while (temp.Step()) {
    if (temp.ColumnString(1) == "favicon_id") {
      result = true;
      break;
    }
  }
