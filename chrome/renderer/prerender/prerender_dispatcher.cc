...
void PrerenderDispatcher::OnPrerenderCancel(int prerender_id) {
  std::map<int, WebPrerender>::iterator it = prerenders_.find(prerender_id);
  if (it == prerenders_.end())
    return;
...
bool PrerenderDispatcher::OnControlMessageReceived(
  ...
  IPC_BEGIN_MESSAGE_MAP(PrerenderDispatcher, message)
    ...
    IPC_MESSAGE_HANDLER(PrerenderMsg_OnPrerenderCancel, OnPrerenderCancel)
...
bool PrerenderDispatcher::addFromMouseOver(const WebPrerender& prerender) {
  base::TimeDelta elapsed_time = base::TimeTicks::Now() - last_prerender_start_time_;
  if (elapsed_time >= base::TimeDelta::FromMilliseconds(kMinTimeBetweenMouseOver)) {
    add(prerender);
