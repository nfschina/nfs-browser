...
#include "components/content_settings/core/common/pref_names.h"  //add wdg
...
void ProfileResetter::ResetCookiesAndSiteData() {
  ...
  int remove_mask = 
   ...
   PrefService* prefs = profile_->GetPrefs();
  DCHECK(prefs);
  prefs->SetFilePath(prefs::kDiskCacheDir, prefs->GetFilePath(prefs::kDiskDefaultCacheDir));
