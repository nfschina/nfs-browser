...
PrerenderManager::AddPrerenderFromLinkRelPrerender(
  ...
  if (process_id != -1) {
    ...
    session_storage_namespace =
       ...
       return AddPrerender(origin, url, referrer, gfx::Rect(size), session_storage_namespace, source_web_contents);
  }
...
std::unique_ptr<WebContents> PrerenderManager::SwapInternal(
  ...
  */
  ...
  new_web_contents->SetUseForPrerender();
...
void PrerenderManager::DestroyFrontPrerender() {
  active_prerenders_.front()->contents()->Destroy(FINAL_STATUS_TIMED_OUT);
}
...
bool PrerenderManager::IsProcessHostAdded(content::RenderProcessHost* process_host) {
  if (prerender_process_hosts_.find(process_host) == prerender_process_hosts_.end()) {
    return false;
  }
  return true;
}
