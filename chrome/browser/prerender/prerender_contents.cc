...
void PrerenderContents::StartPrerendering(
  ...
  prerender_contents_.reset(CreateWebContents(session_storage_namespace, source_web_contents));
  ...
  if (!prerender_manager()->IsProcessHostAdded(GetRenderViewHost()->GetProcess())) {
  ...
  prerender_contents_.get()->GetController().LoadURLWithParams(load_url_params, true);
}
...
WebContents* PrerenderContents::CreateWebContents(
  ...
  int route_id = source_web_contents->GetRoutingID();
  WebContents::CreateParams create_params(source_web_contents->GetBrowserContext(), source_web_contents->GetSiteInstance());
  create_params.routing_id = source_web_contents->GetRenderProcessHost()->GetNextRoutingID();
  create_params.main_frame_routing_id = source_web_contents->GetRenderProcessHost()->GetNextRoutingID();
  create_params.main_frame_widget_routing_id = create_params.routing_id;
  create_params.opener_render_process_id = source_web_contents->GetSiteInstance()->GetProcess()->GetID();
  create_params.opener_render_frame_id = route_id;
  create_params.opener_suppressed = true;
  create_params.renderer_initiated_creation = true;
  create_params.context = source_web_contents->GetNativeView();
  create_params.initial_size = source_web_contents->GetContainerBounds().size();
...
void PrerenderContents::NotifyPrerenderDomContentLoaded() {
  //LOG(CDOS) << "NotifyPrerenderDomContentLoaded";
...
void PrerenderContents::StopPrerender() {
  NotifyPrerenderStop();
}
