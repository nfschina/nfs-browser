...
void ChromeContentBrowserClient::OverrideWebkitPrefs(
  ...
  web_prefs->click_prerender_enabled = prefs->GetBoolean(prefs::kEnableClickPrerender);

  web_prefs->mouse_over_prerender_enabled = prefs->GetBoolean(prefs::kEnableMouseOverPrerender);
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
          switches::kEnableSafeBrowsermode))
  {
  	web_prefs->defalut_browser_mode =  2;
  }
  else
  	web_prefs->defalut_browser_mode =  prefs->GetInteger(prefs::kBrowseringMode);
// web_prefs->defalut_browser_mode =  prefs->GetInteger(prefs::kBrowseringMode);
}
