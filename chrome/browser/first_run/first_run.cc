...
bool g_should_show_bookmark_bar = false;  // added by wangpp
...
// added by wangpp
void SetShouldShowBookmarkBar() {
  g_should_show_bookmark_bar = true;
}

bool ShouldShowBookmarkBar() {
  bool retval = g_should_show_bookmark_bar;
  g_should_show_bookmark_bar = false;
  return retval;
}
// end
...
void AutoImport(
  ...
  if (importer_list->count() > 0) {
    ...
    //               importer::HISTORY,
    //               &items);
    //disable by wdg , don't auto import the history
    SetImportItem(user_prefs,
