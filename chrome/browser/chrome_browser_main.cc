...
int ChromeBrowserMainParts::PreMainMessageLoopRunImpl() {
  ...
  if (parsed_command_line().HasSwitch(switches::kPreLoad)) {
    return static_cast<int>(content::RESULT_CODE_NORMAL_EXIT);
  }
  ...
  #if defined(OS_WIN)
  if (parsed_command_line().HasSwitch(switches::kRegisterBrowser)) {
    return shell_integration::RegisterChromeBrowser()
               ? static_cast<int>(content::RESULT_CODE_NORMAL_EXIT)
               : static_cast<int>(chrome::RESULT_CODE_SHELL_INTEGRATION_FAILED);
  }
  #endif
  ...
  if (first_run::IsChromeFirstRun()) {
    ...
    if ( false && auto_import) { 
      // huk disable this #https://bugs.chromium.org/p/chromium/issues/detail?id=366116
...
bool ChromeBrowserMainParts::MainMessageLoopRun(int* result_code) {
  ...
  *result_code = result_code_;
  ...
  metrics::MetricsService::SetExecutionPhase(
      ...
      g_browser_process->local_state());
  #if defined(OS_LINUX)
  if (profile_->GetPrefs()->GetBoolean(prefs::kContinueLastDownload)) {
    BrowserThread::PostDelayedTask(
      BrowserThread::UI,
      FROM_HERE,
      base::Bind(&content::BrowserContext::CreateDownloadManager,profile_),
