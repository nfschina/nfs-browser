...
#ifndef CHROME_BROWSER_DOWNLOAD_DOWNLOAD_ITEM_MODEL_SPEED_H_
#define CHROME_BROWSER_DOWNLOAD_DOWNLOAD_ITEM_MODEL_SPEED_H_
...
// constructor outlives this |DownloadItemModelSpeed|. In addition, multiple
// DownloadItemModelSpeed objects could be wrapping the same DownloadItem.
class DownloadItemModelSpeed {
 public:
  // Constructs a DownloadItemModelSpeed. The caller must ensure that |download|
  // outlives this object.
  explicit DownloadItemModelSpeed(content::DownloadItem* download);
  ~DownloadItemModelSpeed();
  ...
  base::string16 GetTabProgressSpeedText() const;
  base::string16 GetTabProgressFileSizeText() const;
  base::string16 GetTabProgressTimeRemainText() const;
 ...
 private:
  ...
  base::string16 GetPauseStatusString() const; 

  // The DownloadItem that this model represents. Note that DownloadItemModelSpeed
  // itself shouldn't maintain any state since there can be more than one
  // DownloadItemModelSpeed in use with the same DownloadItem.
  content::DownloadItem* download_;

  DISALLOW_COPY_AND_ASSIGN(DownloadItemModelSpeed);
};

#endif  // CHROME_BROWSER_DOWNLOAD_DOWNLOAD_ITEM_MODEL_SPEED_H_
