...
DownloadCommands::DownloadCommands(content::DownloadItem* download_item, content::DownloadManager* download_manager)
  ...
  switch (command) {
    ...
    case PAUSE:
      return !download_item_->IsDone() && !download_item_->IsPaused() &&
             !(download_item_->GetState() == content::DownloadItem::PAUSING);
    case RESUME:
    ...
    case DELETE_TASK_DOWNLOAD:
    case MOVE_FOLDER:
      return !download_item_->GetFileExternallyRemoved();
    case DOWNLOAD_AGAIN:
      //return download_item_->GetFileExternallyRemoved();
      return true;

    case COPY_DOWNLOAD_LINK:
        GURL url = download_item_->GetURL();
        if(url.is_empty())
          return false;
        return true;
  }
...
void DownloadCommands::ExecuteCommand(Command command) {
  switch (command) {
    ...
    case DELETE_TASK:
      ...
      break;
    case DELETE_TASK_DOWNLOAD:
      download_item_->DeleteFileOnDisk(true);
      download_item_->Remove();
      break;
    case MOVE_FOLDER:
      break;
    case COPY_DOWNLOAD_LINK:
      if(download_item_) {
        GURL url = download_item_->GetURL();
        ui::ScopedClipboardWriter scoped_clipboard_writer(ui::CLIPBOARD_TYPE_COPY_PASTE);
        scoped_clipboard_writer.WriteURL(base::ASCIIToUTF16(url.spec().c_str()));
      }
      break;
    case DOWNLOAD_AGAIN:
      if (download_item_) {
        ...
        //info->download_id = download_item_->GetId();
        info->url_chain.push_back(download_item_->GetURL());
        info->referrer_url = download_item_->GetReferrerUrl();

        info->force_speedy = true;

        info->tab_url = download_item_->GetTabUrl();
        info->method = download_item_->GetMethod();
        info->pack_url = download_item_->GetPackUrl();
        info->save_info->suggested_name = base::UTF8ToUTF16(download_item_->GetSuggestedFilename());
        info->save_info->file_path = download_item_->GetTargetFilePath();
        info->result = content::DOWNLOAD_INTERRUPT_REASON_NONE;
        info->start_time = base::Time::Now();

        content::DownloadUrlParameters::OnStartedCallback on_started;
        download_manager_->CreateDownload(std::move(info), std::move(stream), on_started);
        //chrome::CloseDownloadNewTask(web_ui()->GetWebContents());

        DownloadVector downloads;
        downloads.push_back(download_item_);
