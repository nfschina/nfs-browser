...
DownloadPrefs::DownloadPrefs(Profile* profile) : profile_(profile) {
  ...
  original_download_path_.Init(prefs::kOriginalDownloadDirectoty, prefs);  //add by wdg 
  save_file_type_.Init(prefs::kSaveFileType, prefs);
  prompt_download_.Init(prefs::kPromptDownload, prefs);
...
void DownloadPrefs::RegisterProfilePrefs(
  ...
  registry->RegisterListPref(prefs::kDownloadDirectorySelected);
  registry->RegisterFilePathPref(prefs::kSaveFileDefaultDirectory,
                                 default_download_path);
   registry->RegisterFilePathPref(prefs::kOriginalDownloadDirectoty,
                                 default_download_path);
#if defined(OS_WIN) || defined(OS_LINUX) || defined(OS_MACOSX)
  registry->RegisterBooleanPref(prefs::kOpenPdfDownloadInSystemReader, false);
#endif
    //add by wdg  for the new download setting  add the download options
  registry->RegisterIntegerPref(prefs::kDownloadMaxSpeed,9999,user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);
  registry->RegisterIntegerPref(prefs::kDownloadNeedLimitSpeed, 0, user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);
  registry->RegisterBooleanPref(prefs::kNoteDownloadQuit,true,user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);
  registry->RegisterBooleanPref(prefs::kPromptDownload,true, user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);
  //registry->RegisterBooleanPref(prefs::kVoiceNoteDownload,false,user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);
  registry->RegisterBooleanPref(prefs::kContinueLastDownload,false,user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);
  //end
}
...
bool DownloadPrefs::PromptDownload() const {
  return prompt_download_.GetValue();
}

void DownloadPrefs::SetPromptDownload(bool prompt) {
  prompt_download_.SetValue(prompt);
}
