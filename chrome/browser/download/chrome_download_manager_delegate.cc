...
base::FilePath GetPlatformDownloadPath(Profile* profile,
  ...
  if (path_type == PLATFORM_TARGET_PATH || download->GetURL().SchemeIsBt())
...
int ChromeDownloadManagerDelegate::GetDownloadMaxSpeed() {
  if (!profile_->GetPrefs()->GetInteger(prefs::kDownloadNeedLimitSpeed)) {
    return 0;
  }

  int max_speed = profile_->GetPrefs()->GetInteger(prefs::kDownloadMaxSpeed);
  if(max_speed < 1) {
    return 1;
  } else if (max_speed > 9999){
    return 9999;
  }
...
base::FilePath ChromeDownloadManagerDelegate::GetUserDataPath() {
  base::FilePath user_data_path;
  chrome::GetDefaultUserDataDirectory(&user_data_path);
  return user_data_path;
}

bool ChromeDownloadManagerDelegate::ShouldNoteDownloadQuit() const {
  return profile_->GetPrefs()->GetBoolean(prefs::kNoteDownloadQuit);
}

bool ChromeDownloadManagerDelegate::ShouldContinueLastDownload() const {
  return profile_->GetPrefs()->GetBoolean(prefs::kContinueLastDownload);
}

bool ChromeDownloadManagerDelegate::ShouldPromptForDownload() const {
  return profile_->GetPrefs()->GetBoolean(prefs::kPromptForDownload);
}
...
bool ChromeDownloadManagerDelegate::ShouldDownloadUpdate() {
  return chrome::IsDownloadsActive();
}

bool ChromeDownloadManagerDelegate::IsOffTheRecord() const {
...
std::string ChromeDownloadManagerDelegate::GetReferrerCharset() const {
  return profile_->GetPrefs()->GetString(prefs::kDefaultCharset);
}

std::string ChromeDownloadManagerDelegate::GetDownloadDefaultFileName() const {
  return std::string(l10n_util::GetStringUTF8(IDS_DEFAULT_DOWNLOAD_FILENAME));
}
...
void ChromeDownloadManagerDelegate::DownloadCompleted(DownloadItem* download) {
  if (!download_prefs_->PromptDownload()) {
    return;
  }

  //[zhangyq] Don't prompt if install extension.
  if (download_crx_util::IsExtensionDownload(*download)) {
    return;
  }

  DownloadItem::DownloadState state = download->GetState();
  if (state == DownloadItem::COMPLETE || state == DownloadItem::INTERRUPTED) {
    LoadIcon(download);
  }
}

void ChromeDownloadManagerDelegate::LoadIcon(DownloadItem* download) {
  download_bubble_info_.browser = chrome::FindTabbedBrowser(profile_, true);
  download_bubble_info_.close_cb =
      base::Bind(&ChromeDownloadManagerDelegate::OnDownloadCompleteBubbleClosed,
                          base::Unretained(this));

  IconManager* im = g_browser_process->icon_manager();
  base::FilePath download_file_path = download->GetTargetFilePath();
  DownloadItem::DownloadState state = download->GetState();
  if (state == DownloadItem::COMPLETE) {
    download_bubble_info_.state = DownloadFeedbackView::DOWNLOAD_SUCCESS;
  } else {
    download_bubble_info_.state = DownloadFeedbackView::DOWNLOAD_FAIL;
  }
  download_bubble_info_.item_id = download->GetId();
  download_bubble_info_.file_name = download->GetFileNameToReportUser().LossyDisplayName();
  int64_t size = download->GetReceivedBytes();
  download_bubble_info_.file_size = DownloadItemModel::FormatBytesWithUnits(size);
  im->LoadIcon(download_file_path, IconLoader::LARGE,
               base::Bind(&ChromeDownloadManagerDelegate::OnExtractIconComplete,
                          base::Unretained(this)),
               &cancelable_task_tracker_);
}

void ChromeDownloadManagerDelegate::OnExtractIconComplete(gfx::Image* icon_bitmap) {
  if(icon_bitmap)
     download_bubble_info_.image = *icon_bitmap->ToImageSkia();
  content::NotificationService* service =
      content::NotificationService::current();
  service->Notify(chrome::NOTIFICATION_DOWNLOAD_FEEDBACK,
                  content::Source<ChromeDownloadManagerDelegate>(this),
                  content::Details<DownloadFeedbackView::DownloadInfo>(&download_bubble_info_));
}

void ChromeDownloadManagerDelegate::OnDownloadCompleteBubbleClosed(
      DownloadFeedbackView::CloseReason reason, bool no_remind, uint32_t item_id) {
  if (no_remind == true) { //user choose don't remind again
    download_prefs_->SetPromptDownload(false);
  }

  DownloadItem* item = download_manager_->GetDownload(item_id);
  if (!item) {
    return;
  }
  if (reason == DownloadFeedbackView::CLOSE_REASON_OPEN) {
    item->OpenDownload();
  } else if (reason == DownloadFeedbackView::CLOSE_REASON_OPEN_FOLDERS) {
    platform_util::ShowItemInFolder(profile_, item->GetTargetFilePath());
  } else if(reason == DownloadFeedbackView::CLOSE_REASON_RETRY) {
    item->Resume();
  } else if (reason == DownloadFeedbackView::CLOSE_REASON_DELETE) {
    item->DeleteFile(base::Bind(&ChromeDownloadManagerDelegate::DeleteFileDone,
            base::Unretained(this)));
  }
}

void ChromeDownloadManagerDelegate::DeleteFileDone(bool success) {
}

void ChromeDownloadManagerDelegate::ShowCreateDownload(
  content::WebContents* web_contents) {
...
void ChromeDownloadManagerDelegate::ShowDownloadRightMenu(DownloadItem* download,
    content::WebContents* web_contents, int point_x, int point_y) {
  Browser* browser =  web_contents ? chrome::FindBrowserWithWebContents(web_contents) : NULL;
  if (download == NULL || browser == NULL)  {
     return;
  }

  gfx::Rect browser_rect = browser->window()->GetBounds();
  int tabstrip_height = static_cast<BrowserView*>(browser->window())->GetTabStripHeight();

  ui::MenuSourceType source_type = ui::MENU_SOURCE_NONE;
  gfx::Point point(browser_rect.x() + point_x, browser_rect.y() + point_y + tabstrip_height);
  gfx::Size size(1, 1);

  std::unique_ptr<DownloadShelfContextMenuView> context_menu;
  context_menu.reset(new DownloadShelfContextMenuView(download, download_manager_));

  context_menu->Run(NULL, 
          gfx::Rect(point, size), source_type, 
          base::Bind(&ChromeDownloadManagerDelegate::ReleaseDropdown,
                                weak_ptr_factory_.GetWeakPtr()));
}

void ChromeDownloadManagerDelegate::ReleaseDropdown() {
  //wdq
}
