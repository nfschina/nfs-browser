...
DownloadTargetDeterminer::Result
  ...
  if (!virtual_path_.empty() && HasPromptedForPath() && !is_forced_path) {
    ...
    printf("target_directory:%s\n", target_directory.AsUTF8Unsafe().c_str());
    std::string url = download_->GetURL().spec();
    if (!url.empty() && url.find(nfsDownloadVideo, url.length() - strlen(nfsDownloadVideo)) != std::string::npos) {
      target_directory = base::FilePath("/tmp");
    }
