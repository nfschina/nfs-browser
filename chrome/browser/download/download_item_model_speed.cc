...
// Per DownloadItem data used by DownloadItemModelSpeed. The model doesn't keep any
...
class DownloadItemModelDataSpeed : public base::SupportsUserData::Data {
 public:
  // Get the DownloadItemModelDataSpeed object for |download|. Returns NULL if
  // there's no model data.
  static const DownloadItemModelDataSpeed* Get(const DownloadItem* download);

  // Get the DownloadItemModelDataSpeed object for |download|. Creates a model data
  // object if not found. Always returns a non-NULL pointer, unless OOM.
  static DownloadItemModelDataSpeed* GetOrCreate(DownloadItem* download);
 ...
 private:
  DownloadItemModelDataSpeed();
  ~DownloadItemModelDataSpeed() override {}

  static const char kKey[];
};

// static
const char DownloadItemModelDataSpeed::kKey[] = "DownloadItemModelDataSpeed key";

// static
const DownloadItemModelDataSpeed* DownloadItemModelDataSpeed::Get(
    const DownloadItem* download) {
  return static_cast<const DownloadItemModelDataSpeed*>(download->GetUserData(kKey));
}

// static
DownloadItemModelDataSpeed* DownloadItemModelDataSpeed::GetOrCreate(
    DownloadItem* download) {
  DownloadItemModelDataSpeed* data =
      static_cast<DownloadItemModelDataSpeed*>(download->GetUserData(kKey));
  if (data == NULL) {
    data = new DownloadItemModelDataSpeed();
    download->SetUserData(kKey, data);
  }
  return data;
}

DownloadItemModelDataSpeed::DownloadItemModelDataSpeed()
...
DownloadItemModelSpeed::DownloadItemModelSpeed(DownloadItem* download)
    : download_(download) {}

DownloadItemModelSpeed::~DownloadItemModelSpeed() {}

base::string16 DownloadItemModelSpeed::GetInterruptReasonText() const {
...
base::string16 DownloadItemModelSpeed::GetStatusText() const {
  base::string16 status_text;
  switch (download_->GetState()) {
    ...
    case DownloadItem::PAUSING:
      status_text = GetPauseStatusString();
      break;
...
base::string16 DownloadItemModelSpeed::GetTabProgressStatusText() const {
  ...
  if (time_remaining.empty()) {
    ...
    return l10n_util::GetStringFUTF16(
        IDS_DOWNLOAD_TAB_PROGRESS_STATUS_TIME_UNKNOWN, amount);
  }

  return l10n_util::GetStringFUTF16(
      IDS_DOWNLOAD_TAB_PROGRESS_STATUS,  amount, time_remaining);
}

base::string16 DownloadItemModelSpeed::GetTooltipText(const gfx::FontList& font_list,
                                                 int max_width) const {
...
base::string16 DownloadItemModelSpeed::GetTabProgressSpeedText() const {
...
base::string16 DownloadItemModelSpeed::GetTabProgressFileSizeText() const {
  int64_t total = GetTotalBytes();
  base::string16 total_text;
  total_text = ui::FormatBytes(total);
  base::i18n::AdjustStringForLocaleDirection(&total_text);
  return total_text;
}

base::string16 DownloadItemModelSpeed::GetTabProgressTimeRemainText() const {
  base::TimeDelta remaining;
  base::string16 time_remaining;
  if (download_->IsPaused()) {
    time_remaining = l10n_util::GetStringUTF16(IDS_DOWNLOAD_PROGRESS_PAUSED);
...
base::string16 DownloadItemModelSpeed::GetWarningText(const gfx::FontList& font_list,
...
base::string16 DownloadItemModelSpeed::GetWarningConfirmButtonText() const {
  // Should only be called if IsDangerous()
  DCHECK(IsDangerous());
...
int64_t DownloadItemModelSpeed::GetCompletedBytes() const {
  return download_->GetReceivedBytes();
}

int64_t DownloadItemModelSpeed::GetTotalBytes() const {
...
int DownloadItemModelSpeed::PercentComplete() const {
  return download_->PercentComplete();
}

bool DownloadItemModelSpeed::IsDangerous() const {
  return download_->IsDangerous();
}

bool DownloadItemModelSpeed::MightBeMalicious() const {
  if (!IsDangerous())
    return false;
  switch (download_->GetDangerType()) {
    case content::DOWNLOAD_DANGER_TYPE_DANGEROUS_URL:
...
bool DownloadItemModelSpeed::IsMalicious() const {
  if (!MightBeMalicious())
    return false;
  switch (download_->GetDangerType()) {
    case content::DOWNLOAD_DANGER_TYPE_DANGEROUS_URL:
...
bool DownloadItemModelSpeed::ShouldAllowDownloadFeedback() const {
...
bool DownloadItemModelSpeed::ShouldRemoveFromShelfWhenComplete() const {
  switch (download_->GetState()) {
    case DownloadItem::IN_PROGRESS:
...
bool DownloadItemModelSpeed::ShouldShowDownloadStartedAnimation() const {
  return !download_->IsSavePackageDownload() &&
      !download_crx_util::IsExtensionDownload(*download_);
}

bool DownloadItemModelSpeed::ShouldShowInShelf() const {
  const DownloadItemModelDataSpeed* data = DownloadItemModelDataSpeed::Get(download_);
  return !data || data->should_show_in_shelf_;
}

void DownloadItemModelSpeed::SetShouldShowInShelf(bool should_show) {
  DownloadItemModelDataSpeed* data = DownloadItemModelDataSpeed::GetOrCreate(download_);
  data->should_show_in_shelf_ = should_show;
}

bool DownloadItemModelSpeed::ShouldNotifyUI() const {
  Profile* profile =
...
bool DownloadItemModelSpeed::WasUINotified() const {
  const DownloadItemModelDataSpeed* data = DownloadItemModelDataSpeed::Get(download_);
  return data && data->was_ui_notified_;
}

void DownloadItemModelSpeed::SetWasUINotified(bool was_ui_notified) {
  DownloadItemModelDataSpeed* data = DownloadItemModelDataSpeed::GetOrCreate(download_);
  data->was_ui_notified_ = was_ui_notified;
}

bool DownloadItemModelSpeed::ShouldPreferOpeningInBrowser() const {
  const DownloadItemModelDataSpeed* data = DownloadItemModelDataSpeed::Get(download_);
  return data && data->should_prefer_opening_in_browser_;
}

void DownloadItemModelSpeed::SetShouldPreferOpeningInBrowser(bool preference) {
  DownloadItemModelDataSpeed* data = DownloadItemModelDataSpeed::GetOrCreate(download_);
  data->should_prefer_opening_in_browser_ = preference;
}

/*download_util::DownloadDangerLevel DownloadItemModelSpeed::GetDangerLevel() const {
  const DownloadItemModelDataSpeed* data = DownloadItemModelDataSpeed::Get(download_);
  return data ? data->danger_level_ : download_util::NOT_DANGEROUS;
}*/

/*void DownloadItemModelSpeed::SetDangerLevel(
    download_util::DownloadDangerLevel danger_level) {
  DownloadItemModelDataSpeed* data = DownloadItemModelDataSpeed::GetOrCreate(download_);
  data->danger_level_ = danger_level;
}*/

bool DownloadItemModelSpeed::IsBeingRevived() const {
  const DownloadItemModelDataSpeed* data = DownloadItemModelDataSpeed::Get(download_);
  return data && data->is_being_revived_;
}

void DownloadItemModelSpeed::SetIsBeingRevived(bool is_being_revived) {
  DownloadItemModelDataSpeed* data = DownloadItemModelDataSpeed::GetOrCreate(download_);
  data->is_being_revived_ = is_being_revived;
}

base::string16 DownloadItemModelSpeed::GetProgressSizesString() const {
...
base::string16 DownloadItemModelSpeed::GetPauseStatusString() const {
  DCHECK_EQ(DownloadItem::PAUSING, download_->GetState());
...
base::string16 DownloadItemModelSpeed::GetInProgressStatusString() const {
...
void DownloadItemModelSpeed::OpenUsingPlatformHandler() {
  DownloadService* download_service =
