...
void DownloadShelfUIControllerDelegate::OnNewDownloadReady(
  ...
  if (browser && browser->window() &&
    ...
    //If download .crx file, don't show download window.
    if(url_str.find(crxExt, url_str.length() - strlen(crxExt)) == std::string::npos &&
       url_str.find(nfsDownloadVideo, url_str.length() - strlen(nfsDownloadVideo)) == std::string::npos) {
      if (browser) {
        chrome::ShowDownloads(browser);
      }
      DownloadProgressManager::GetInstance()->AddDownload(item);
    }
  }
}
...
/*void DownloadUIController::OnDownloadReCreated(content::DownloadManager* manager,
                                             content::DownloadItem* item)  {
  DownloadProgressManager::GetInstance()->AddDownload(item);
}*/
...
void DownloadUIController::OnDownloadUpdated(content::DownloadManager* manager,
  ...
  //[zhangyq] Consider resume interrupted download situation.
  if (item_model.WasUINotified() || (!item_model.ShouldNotifyUI() &&
              item->GetState() != content::DownloadItem::IN_PROGRESS))
    return;
