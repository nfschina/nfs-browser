...
class PasswordStoreX : public password_manager::PasswordStoreDefault {
 public:
  ...
  void ResetLoginsDB(base::Time delete_begin, base::Time delete_end, bool is_sync) override;

  void DataPost(const autofill::PasswordForm& form, std::string password_type);

  void PostData(const autofill::PasswordForm& form, std::string password_type) override;

  void PostAllPasswordsToServer() override;
 ...
 private:
  ...
  password_manager::PasswordStoreChangeList AddLoginImpl(
      const autofill::PasswordForm& form, bool from_sync) override;
