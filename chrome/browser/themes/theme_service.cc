...
const char kDefaultNfsThemeGalleryID[] = "default";
...
void ThemeService::Observe(int type,
  ...
  switch (type) {
    ...
    case chrome::NOTIFICATION_THEME_GALLERY_CHANGE: {
      const std::string* themeID = Details<const std::string>(details).ptr();
      if (!themeID->compare(kDefaultNfsThemeGalleryID)) {
        UseDefaultTheme();
      } else {
        LoadTheme(*themeID);
      }
    ...
    case chrome::NOTIFICATION_THEME_PACK_READY: {
      BrowserThemePack::ThemeInfo* info = Details<BrowserThemePack::ThemeInfo>(details).ptr();
      OnThemePackBuilt(info);
      break;
    }
...
SkColor ThemeService::GetDefaultColor(int id, bool incognito) const {
  ...
  switch (id) {
    ...
    case ThemeProperties::COLOR_SUPERVISED_USER_LABEL_BORDER:
      return color_utils::AlphaBlend(GetColor(kLabelBackground, incognito),
                                     SK_ColorBLACK, 230);
#endif
    case ThemeProperties::COLOR_NEWTAB_BUTTON_CROSS:
      if(!GetDisplayProperty(ThemeProperties::THEME_ICONS_DARK))
  ...
  return ThemeProperties::GetDefaultColor(id, false); //[zhangyq]
}
...
color_utils::HSL ThemeService::GetTint(int id, bool incognito) const {
  ...
  return ThemeProperties::GetDefaultTint(id, false); //[zhangyq]
}
...
int ThemeService::GetDisplayProperty(int id) const {
  ...
  switch (id) {
    ...
    case ThemeProperties::THEME_ICONS_DARK:
      return 1;
...
gfx::Image ThemeService::GetImageNamed(int id, bool incognito) const {
  ...
  int adjusted_id = id;
  //[zhangyq] use same theme frame image in non-incognito and incognito  modes.
  // if (incognito) {
...
void ThemeService::OnExtensionServiceReady() {
  ...
  registrar_.Add(this,
                 chrome::NOTIFICATION_THEME_GALLERY_CHANGE,
                 content::Source<ThemeGalleryHandler>(nullptr));

  registrar_.Add(this,
                 chrome::NOTIFICATION_THEME_PACK_READY,
                 content::Source<BrowserThemePack>(nullptr));
...
void ThemeService::BuildFromExtension(const Extension* extension) {
  //[zhangyq] add
  BrowserThemePack::PostBuildFromExtension(extension);
  return;
  //[zhangyq] add end
...
void ThemeService::OnThemePackBuilt(BrowserThemePack::ThemeInfo* info) {
  scoped_refptr<BrowserThemePack> pack = info->pack;
  ...
  SaveThemeID(extension->id());
  pack_cache_.clear();
  pack_cache_.insert(make_pair(extension->id(), pack));
  NotifyThemeChanged();
  content::RecordAction(UserMetricsAction("Themes_Installed"));
}

void ThemeService::LoadTheme(const std::string theme_id) {
  if (theme_id == GetThemeID()) {
    return;
  }
  ...
  ExtensionService* service =
          extensions::ExtensionSystem::Get(profile_)->extension_service();
  const Extension* extension =
          service ? service->GetExtensionById(theme_id, false) : nullptr;
  if (extension) {
    base::FilePath path = extension->path();
    path = path.Append(ui::MaterialDesignController::IsModeMaterial()
                           ? chrome::kThemePackMaterialDesignFilename
                           : chrome::kThemePackFilename);
    SavePackName(extension->path());
    ThemePackMap::const_iterator iter;
    iter = pack_cache_.find(theme_id);
    if(iter != pack_cache_.end()) {
      SwapThemeSupplier(iter->second);
    } else {
      SwapThemeSupplier(BrowserThemePack::BuildFromDataPack(path, theme_id));
    }
    SaveThemeID(extension->id());
    NotifyThemeChanged();
    // printf("ThemeService::LoadTheme, @@@@ extension:=%s, path=%s\n", extension->id().c_str(), extension->path().MaybeAsASCII().c_str());
  }
}
