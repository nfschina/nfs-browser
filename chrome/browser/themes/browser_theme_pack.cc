...
const int PRS_THEME_FRAME_DOWNLOAD = 15;
...
PersistingImagesTable kPersistingImages[] = {
    ...
    {PRS_THEME_WINDOW_CONTROL_BACKGROUND, IDR_THEME_WINDOW_CONTROL_BACKGROUND,
     ...
     {PRS_THEME_FRAME_DOWNLOAD, IDR_THEME_FRAME_DOWNLOAD,
     "theme_frame_download"},
};
...
StringToIntTable kColorTable[] = {
  ...
  { "tab_blur_active", ThemeProperties::COLOR_TAB_BLUR_ACTIVE},
  { "tab_blur_inactive", ThemeProperties::COLOR_TAB_BLUR_INACTIVE},
  { "new_tab_button_blur_normal", ThemeProperties::COLOR_NEW_TAB_BUTTON_BLUR_NORMAL},
  { "new_tab_button_blur_press", ThemeProperties::COLOR_NEW_TAB_BUTTON_BLUR_PRESS},
  { "new_tab_button_blur_hover", ThemeProperties::COLOR_NEW_TAB_BUTTON_BLUR_HOVER},
  { "toolbar_blur", ThemeProperties::COLOR_TOOLBAR_BLUR},
};
...
StringToIntTable kDisplayProperties[] = {
  ...
  { "theme_icons_dark", ThemeProperties::THEME_ICONS_DARK },
};
...
IntToIntTable kFrameTintMap[] = {
  ...
  { PRS_THEME_FRAME_INCOGNITO_INACTIVE,
    ThemeProperties::TINT_FRAME_INCOGNITO_INACTIVE },
  { PRS_THEME_FRAME_DOWNLOAD, ThemeProperties::TINT_FRAME },
};
...
struct CropEntry kImagesToCrop[] = {
  ...
  { PRS_THEME_FRAME_DOWNLOAD, 120, true },
};
  ...
  for (base::DictionaryValue::Iterator iter(*display_properties_value);
    ...
    switch (property_id) {
      ...
      case ThemeProperties::THEME_ICONS_DARK: {
        int val = 0;
        if (iter.value().GetAsInteger(&val))
          temp_properties[ThemeProperties::THEME_ICONS_DARK] = val;
        break;
      }
    }
  }
...
BrowserThemePack::ThemeInfo::ThemeInfo() {}

BrowserThemePack::ThemeInfo::~ThemeInfo() {}

void BrowserThemePack::PostBuildFromExtension(const Extension* extension) {
  content::BrowserThread::PostTask(
      content::BrowserThread::FILE, FROM_HERE,
      base::Bind(&BrowserThemePack::BuildFromExtensionOnFileThread, extension));
}


void BrowserThemePack::BuildFromExtensionOnFileThread(
    const Extension* extension) {
  DCHECK(extension);
  DCHECK(extension->is_theme());
  ...
  content::BrowserThread::PostTask(
      ...
      base::Bind(&BrowserThemePack::NotifyThemeInfo, pack, extension));
}

void BrowserThemePack::NotifyThemeInfo(scoped_refptr<BrowserThemePack> pack,
     const Extension* extension) {
  ThemeInfo info;
  info.pack = pack;
  info.extension = extension;
  content::NotificationService* service = content::NotificationService::current();
  service->Notify(chrome::NOTIFICATION_THEME_PACK_READY,
                  content::Source<BrowserThemePack>(pack.get()),
                  content::Details<ThemeInfo>(&info));
}