...
class ThemeService : public base::NonThreadSafe,
 ...
 private:
  ...
  void OnThemePackBuilt(BrowserThemePack::ThemeInfo* info);
  void LoadTheme(const std::string theme_id);
  ...
  typedef std::map<std::string, scoped_refptr<BrowserThemePack> > ThemePackMap;
  ThemePackMap pack_cache_;
