...
const int kSmallBrowserMemoryUsageThreshold = 35;

const int kMiddleBrowserMemoryUsageThreshold = 30;

const int kLargeBrowserMemoryUsageThreshold = 25;
...
TabManager::TabManager()
    : TaskManagerObserver(base::TimeDelta::FromMilliseconds(1000),
          task_manager::REFRESH_TYPE_MEMORY),
      discard_count_(0),
      recent_tab_discard_(false),
      discard_once_(false),
      //minimum_protection_time_(base::TimeDelta::FromMinutes(10)),
      //[zhangyq] change minimum_protection_time_ to 0.
      minimum_protection_time_(base::TimeDelta::FromMinutes(0)),
...
void TabManager::Start() {
  //[zhangyq] Set minimum_protection_time_ to 0 on Windows and Linux.
...
#if defined(OS_LINUX)
     auto monitor = base::MemoryPressureMonitor::Get();
     //if no tab can be discard.
    if (monitor && !manager->HasDiscardableTab()) {
      monitor->CooldownMemPressureTimer();
      return;
    }
#endif
    manager->DiscardTab();
...
bool TabManager::CanOnlyDiscardOnce() {
  //[zhangyq] Return false on Windows and Linux.
  // #if defined(OS_WIN) || defined(OS_MACOSX)
  #if defined(OS_MACOSX)
...
void TabManager::PurgeMemoryNfs() {
  if (g_browser_process && g_browser_process->GetTabManager()) {
    TabManager* manager = g_browser_process->GetTabManager();
    manager->PurgeBrowserMemory();
    manager->DiscardAllPossibleTabs();
  }
}

void TabManager::DiscardAllPossibleTabs() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  TabStatsList stats = GetTabStats();
  if (stats.empty())
    return;
  ...
  for (TabStatsList::const_reverse_iterator stats_rit = stats.rbegin();
    stats_rit != stats.rend(); ++stats_rit) {
    int64_t tab_contents_id = stats_rit->tab_contents_id;
    if (CanDiscardTab(tab_contents_id)) {
      DiscardTabById(tab_contents_id);
    }
  }
}

bool TabManager::HasDiscardableTab() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  TabStatsList stats = GetTabStats();
...
//[zhangyq add]Take browser memory usage into account.
int64_t TabManager::GetBrowserMemoryUsage() const {
  int64_t memory_bytes = 0;
  std::vector<int64_t> tasks = observed_task_manager()->GetTaskIdsList();
  std::map<base::ProcessId, int64_t> tasks_by_proc_id;
  for (auto id : tasks) {
    base::ProcessId proc_id = observed_task_manager()->GetProcessId(id);
    tasks_by_proc_id[proc_id] = id;
  }
  for (const auto& itr : tasks_by_proc_id) {
    memory_bytes += observed_task_manager()->GetPhysicalMemoryUsage(itr.second);
  }
  return memory_bytes / 1024;
}

bool TabManager::OverBrowserMemUsageThreshold() {
  base::SystemMemoryInfoKB meminfo;
  base::GetSystemMemoryInfo(&meminfo);
#if defined(OS_LINUX)
  int sys_mem_usage  = meminfo.total - meminfo.free - meminfo.buffers - meminfo.cached;
#else
  int sys_mem_usage  = meminfo.total - meminfo.free;
#endif
  int mem_usage_threshold = kLargeBrowserMemoryUsageThreshold;
  if (meminfo.total < 2 * 1024 * 1024) {
    mem_usage_threshold = kSmallBrowserMemoryUsageThreshold;
  } else if (meminfo.total < 4 * 1024 * 1024) {
    mem_usage_threshold = kMiddleBrowserMemoryUsageThreshold;
  } else {
    mem_usage_threshold = kLargeBrowserMemoryUsageThreshold;
  }
  int browser_mem_usage = GetBrowserMemoryUsage();
  return (browser_mem_usage * 100 / sys_mem_usage) > mem_usage_threshold;
}
//[zhangyq add end]Take browser memory usage into account.
