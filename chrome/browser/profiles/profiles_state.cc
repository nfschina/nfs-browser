...
//add by  dg
void RemoveBrowsingDataForProfile_RemoveMask(const base::FilePath& profile_path,const int period_selected,const int remove_mask,const int original_mask) {
  // The BrowsingDataRemover relies on the ResourceDispatcherHost, which is
  // null in unit tests.
  if (!content::ResourceDispatcherHost::Get())
    return;
