...
void ProfileImpl::RegisterProfilePrefs(
  ...
  registry->RegisterFilePathPref(prefs::kDiskCacheDir, base::FilePath());
  registry->RegisterFilePathPref(prefs::kDiskDefaultCacheDir, base::FilePath());
  registry->RegisterIntegerPref(prefs::kDiskCacheSize, 0);
  registry->RegisterIntegerPref(prefs::kMediaCacheSize, 0);

  // registry->RegisterStringPref(prefs::kUserID, std::string(),
  //                              user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);
  registry->RegisterBooleanPref(prefs::kShouldShowImportDialog, true,
                                user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);

  registry->RegisterStringPref(prefs::kBrowserLastUpdateVersion, "1.1.1.1",
                                user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);

  registry->RegisterIntegerPref(prefs::kBrowserShouldShowWelcome, 0,
                                user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);

  registry->RegisterBooleanPref(prefs::kEnableSyncBookmark, true,
                                user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);

  registry->RegisterBooleanPref(prefs::kEnableSyncSkin, true,
                                user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);

  registry->RegisterBooleanPref(prefs::kEnableSyncPassword, true,
                                user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);

  registry->RegisterBooleanPref(prefs::kFirstSyncPassword, true,
                                user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);
}
...
std::vector<base::FilePath> ProfileImpl::download_selected_directorys() {
  std::vector<base::FilePath> selected_directorys;
  const base::ListValue* directorys =
      GetPrefs()->GetList(prefs::kDownloadDirectorySelected);
  for (size_t i = 0; i < directorys->GetSize(); ++i) {
    #if defined(OS_WIN)
    std::wstring directory;
    #else
    std::string directory;
    #endif
    if (directorys->GetString(i, &directory)) {
      selected_directorys.push_back(base::FilePath(directory));
    }
  }

  return selected_directorys;
}

void ProfileImpl::set_download_selected_directory(const base::FilePath& path) {
  std::unique_ptr<base::ListValue> directorys =
      GetPrefs()->GetList(prefs::kDownloadDirectorySelected)->CreateDeepCopy();
  for (size_t i = 0; i < directorys->GetSize(); ++i) {
    #if defined(OS_WIN)
    std::wstring directory;
    #else
    ...
    if (directorys->GetString(i, &directory)) {
      if (directory == path.value()) {
        ...
        directorys->Remove(i, &value);
      }
    }
  }

  if (directorys->GetSize() >= kMaxSelectedDownloadDirectorys) {
    std::unique_ptr<base::Value> value;
    directorys->Remove(0, &value);
  }

  directorys->AppendString(path.value());
  GetPrefs()->Set(prefs::kDownloadDirectorySelected, *directorys.get());
}
...
GURL ProfileImpl::GetHomePage() {
  ...
  return home_page;
#else
  if (GetPrefs()->GetInteger(prefs::kNewTabPage) != 1)
    return GURL(chrome::kChromeUINewTabURL);
  GURL home_page(url_formatter::FixupURL(
      GetPrefs()->GetString(prefs::kHomePage), std::string()));
  if (!home_page.is_valid())
...
void ProfileImpl::GetCacheParameters(bool is_media_context,
  ...
  bool contains_nfs = false;
  #if defined(OS_WIN)
    base::string16 value = path.value();
    contains_nfs = value.find(base::UTF8ToUTF16("nfsbrowser")) != base::string16::npos;
  #else
    std::string value = path.value();
    contains_nfs = value.find("nfsbrowser") != std::string::npos;
  #endif

  if (!path.empty() && !contains_nfs)   // the cache is not empty
      *cache_path = path;
    else {
      if(!is_media_context)
        {
           *cache_path = cache_path->Append(chrome::kCacheDirname);
          prefs_->SetFilePath(prefs::kDiskDefaultCacheDir, *cache_path);
          prefs_->SetFilePath(prefs::kDiskCacheDir, *cache_path);
        }
         }
