...
void RenderViewContextMenu::AppendDeveloperItems() {
  ...
  // devtools build.
  if (params_.page_url.SchemeIs(content::kChromeUIScheme) ||
      params_.page_url.SchemeIs(content::kChromeExtentionScheme))
    return;
  ...
  menu_model_.AddSeparator(ui::NORMAL_SEPARATOR);
  encoding_menu_model_.reset(new EncodingMenuModel(GetBrowser()));
  menu_model_.AddSubMenuWithStringId(IDC_ENCODING_MENU, IDS_ENCODING_MENU,
...
void RenderViewContextMenu::AppendPageItems() {
  ...
  // if (params_.page_url.SchemeIs(content::kChromeUIScheme) || params_.page_url.SchemeIs(content::kChromeExtentionScheme))
  //   return;
...
bool RenderViewContextMenu::IsTranslateEnabled() const {
  ...
  return ((params_.edit_flags & WebContextMenuData::CanTranslate) != 0) &&
      ...
      //   !chrome_translate_client->GetLanguageState().IsPageTranslated() &&   //wdg
