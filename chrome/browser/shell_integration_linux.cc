...
bool SetDefaultWebClient(const std::string& protocol) {
  ...
  if (!ran_ok || exit_code != EXIT_SUCCESS)  {
    LOG(ERROR) << "set default browser: xdg setting failed " << ran_ok << "   " << exit_code;
  }
...
DefaultWebClientState GetIsDefaultWebClient(const std::string& protocol) {
 ...
 // argv.push_back(shell_integration_linux::GetDesktopName(env.get()));
  ...
  if (!ran_ok || success_code != EXIT_SUCCESS) {
    LOG(ERROR) << "check default browser: xdg setting failed " << ran_ok << "   " << success_code;

    // xdg-settings failed: we can't determine or set the default browser.
//    return UNKNOWN_DEFAULT;   //disable by wdg  if   like bt download block the reply ,return not default
    return NOT_DEFAULT;
  }

  // Allow any reply that starts with "yes".
  return base::StartsWith(reply, "nfs-browser.desktop", base::CompareCase::SENSITIVE)
             ? IS_DEFAULT
             : NOT_DEFAULT;
#endif
}
...
const char kDirectoryFilename[] = "nfsbrowser-apps.directory";
...
#else
#if defined(ENABLE_APP_LIST)
const char kAppListDesktopName[] = "nfsbrowser-app-list";
#endif
#endif
