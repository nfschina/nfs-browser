...
const char kNemoCmd[] = "nemo";
base::ProcessId email_id = -1;
bool sigaction_changed = false;

void SIGCHLDHandler(int signal) {
  int stat = 0;
  if (waitpid(email_id, &stat, WNOHANG) > 0 && stat > 0) {
    const base::string16& title =
        base::UTF8ToUTF16("nfs-browser");
    const base::string16& message =
        base::UTF8ToUTF16("can not open this url, please make sure mail client is installed\n\n");
...
void ShowFileInNemo(const base::FilePath& working_directory,
                        const std::string& path) {
  RunCommand(kNemoCmd, working_directory, path);
}
...
void XDGEmail(const std::string& email) {
  ...
  email_id = RunCommand("xdg-email", base::FilePath(), email);
}
...
void ShowItem(Profile* profile,
  ...
  if (use_nautilus_file_browser) {
    ...
    OpenItem(profile, full_path, OPEN_FOLDER,
...
void PlatformOpenVerifiedItem(const base::FilePath& path, OpenItemType type) {
  switch (type) {
    ...
    case OPEN_FOLDER:
      ...
      if( !IsDirectoryEmpty(path) || !base::PathExists(path)) {
        XDGOpen(path.DirName(), ".");
      } else {
        ShowFileInNemo(path.DirName(), path.value());
      }
      break;
