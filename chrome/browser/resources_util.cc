...
class ThemeMap {
 public:
  ...
  ThemeMap() {
    ...
    for (size_t i = 0; i < kThemeResourcesNfsSize; ++i)
      id_map_[kThemeResourcesNfs[i].name] = kThemeResourcesNfs[i].value;
    for (size_t i = 0; i < kUiResourcesSize; ++i)
      id_map_[kUiResources[i].name] = kUiResources[i].value;
    for (size_t i = 0; i < kUiResourcesNfsSize; ++i)
      id_map_[kUiResourcesNfs[i].name] = kUiResourcesNfs[i].value;
#if defined(OS_CHROMEOS)
