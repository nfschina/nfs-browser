...
ExtensionService::ExtensionService(Profile* profile,
    ...
    : extensions::Blacklist::Observer(blacklist),
      ...
      preinstall_extension_service_(new PreinstallExtensionService(profile_)) {
  CHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  ...
  if (is_first_run_ || extension_prefs_->GetPreinstallVersion() < extensions::GetVersion()) {
    extension_prefs_->SetPreinstallVersion(extensions::GetVersion());

    extensions::PreInstallExtensionsVector vector;
    extensions::GetPreInstallExtensionsInfo(vector);

    extensions::PreInstallExtensionsVectorIterator iter;
    int ellapse = 0;
    for (iter = vector.begin(); iter != vector.end(); ++iter)  {
      base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
            FROM_HERE, 
            base::Bind(&ExtensionService::PreInstallExtensions,
                AsWeakPtr(), iter->id),
            base::TimeDelta::FromMilliseconds(ellapse));
      ellapse += 500;
    }
  }
...
void ExtensionService::OnExtensionInstalled(
  ...
  if (IsPreInstallExtension(id))  {
    GrantPermissionsAndEnableExtension(extension);
    // pre install extensions are allowed to run in incognito mode
    extension_prefs_->SetIsIncognitoEnabled(id, true);
  }
...
void ExtensionService::PreInstallExtensions(const std::string& id) {  
  base::FilePath exe_dir;
  PathService::Get(base::DIR_MODULE, &exe_dir);
  base::FilePath extension_dir = exe_dir.AppendASCII(chrome::kExtensionDir);
  ...
  for (const auto& i : vector) {
    if (id == i.id) {
      installer->InstallCrxFile(extensions::CRXFileInfo(i.id, extension_dir.AppendASCII(i.name)));
      EnableExtension(id);
    }
  }
}

bool ExtensionService::IsPreInstallExtension(const std::string& id)  {
  extensions::PreInstallExtensionsVector vector;
  extensions::GetPreInstallExtensionsInfo(vector); 
