...
#include "content/common/frame_messages.h"//add by wangn
...
void ExtensionMessagePort::DispatchOnMessage(const Message& message) {
  if(strstr(message.data.c_str(),"testfunc")) {//add by wangn
    for (content::RenderFrameHost* rfh : frames_) {
      int error_code = -20;
      int sendflag= rfh->Send(new FrameMsg_FromJsSendMsg(rfh->GetRoutingID(),error_code));
      LOG(INFO) << "sendflag:"<<sendflag;
    } 
  } 
  else {
    SendToPort(base::MakeUnique<ExtensionMsg_DeliverMessage>(MSG_ROUTING_NONE, port_id_, opener_tab_id_, message));
    //SendToPort(base::WrapUnique(new ExtensionMsg_DeliverMessage(MSG_ROUTING_NONE, port_id_, message)));
  }
}
