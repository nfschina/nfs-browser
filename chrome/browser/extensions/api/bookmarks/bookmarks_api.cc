...
const BookmarkNode* BookmarksFunction::CreateBookmarkNode(
  ...
  if (!details.parent_id.get()) {
    ...
    parentId = model->bookmark_bar_node()->id();
  } else {
...
bool BookmarksNFSImportFunction::RunOnReady() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);

  if (!EditBookmarksEnabled())
    return false;

  ShowImportSelectDialog(GetCurrentBrowser()->window()->GetNativeWindow());

  return true;
}

void BookmarksNFSImportFunction::FileSelected(const base::FilePath& path,
                                           int index,
                                           void* params) {
  // Deletes itself.
  ExternalProcessImporterHost* importer_host = new ExternalProcessImporterHost;
