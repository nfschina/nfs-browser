...
#include "base/strings/utf_string_conversions.h"  // added by wangpp
#include "base/synchronization/waitable_event.h"
#include "base/nix/xdg_util.h"//add by wangn
...
#include "chrome/browser/extensions/api/checkmurl/checkmurl_api.h" //add by wangn
...
#include "sql/connection.h"   //add by wangn
#include "sql/statement.h"   //add by wangn
...
using sql::Statement; //add by wangn
using base::nix::kDotConfigDir; //add by wangn
...
BrowserProcessImpl::BrowserProcessImpl(
    ...
    : created_watchdog_thread_(false),
      ...
      created_checkurl_manager_(false),//add by wangn
...
void BrowserProcessImpl::StartTearDown() {
  ...
  // 隐藏的新标签页进程无法自动析构，需要手动析构
  // 避免关闭浏览器时造成崩溃
  if (g_hide_new_tab_contents) {
    delete g_hide_new_tab_contents;
    g_hide_new_tab_contents = NULL;
  }
...
CheckurlManager* BrowserProcessImpl::check_url_manager() {
  //printf("check_url_manager()\n");
  DCHECK(CalledOnValidThread());
  if (!created_checkurl_manager_) {
    CreateCheckurlManager();
    DBPathTransf();
    check_url_manager_.get()->checkdb = CreateMurl();
  }
  return check_url_manager_.get();
  return NULL;
}


void BrowserProcessImpl::CreateCheckurlManager() {//add by wangn
  DCHECK(!created_checkurl_manager_ && check_url_manager_.get() == NULL);
  created_checkurl_manager_ = true;
  check_url_manager_.reset(new CheckurlManager);
}

static int callback(void *NotUsed, int argc, char **argv, char **azColName) { //add by wangn
  int i;
  for(i=0; i<argc; i++) {
...
bool BrowserProcessImpl::IsTableExist(const char* pTableName,sqlite3* db) { //add by wangn
  char *zErrMsg = 0;
  if (NULL == pTableName) {
    return true;
  }
  char chSQL[1024] = {0};
  snprintf(chSQL, 1024, "select count(*) from sqlite_master where type = 'table' and name = '%s'", pTableName);
  LOG(INFO) << "Table_sql:"<<chSQL<<"\n";
  int bIsTableExist = sqlite3_exec(db,chSQL,callback,0,&zErrMsg);
    return (bIsTableExist == 0) ? false : true;
}




void BrowserProcessImpl::DBPathTransf() { //add by wangn
  #if defined(OS_WIN)
  const wchar_t* kDefault = FILE_PATH_LITERAL("Default\\murl.db");
  #else
  const char* kDefault = "Default/murl.db";
  #endif
  static const int LatestVersion = 38;//the latest version need by manual maintenance
  //printf("++++++++++++ApplyMurlDBUpdate before:%d\n",local_state()->GetInteger(prefs::kMurlUpdate));
  //LOG(ERROR) << __func__;
  if(local_state()->GetInteger(prefs::kMurlUpdate) < LatestVersion) {
    base::FilePath user_config_dir;
    if (PathService::Get(chrome::DIR_USER_DATA, &user_config_dir))
      user_config_dir = user_config_dir.Append(kDefault);

    #if defined(OS_WIN)
    const wchar_t* kMurldb = FILE_PATH_LITERAL("Murl\\murl.db");
    #else
    const char* kMurldb = "Murl/murl.db";
    #endif
    base::FilePath user_db_dir;
    if (PathService::Get(chrome::DIR_APP, &user_db_dir))
      user_db_dir = user_db_dir.Append(kMurldb);

    LOG(ERROR) << "user_config_dir" << user_config_dir.value().c_str();
    LOG(ERROR) << "user_db_dir" << user_db_dir.value().c_str();
    #if defined(OS_WIN)
    //1st Verify File "~/.config/nfsbrowser/Default/murl.db" is exist or not
    //2nd Verify File "/opt/nfsbrowser/Murl/murl.db is exist or not
    if (!base::PathExists(user_config_dir) && base::PathExists(user_db_dir)) {
      if (!base::CopyFile(user_db_dir, user_config_dir)) {
        LOG(INFO) << "DBPathTransf copy file failed";
      }
    }
    #else
    //1st Verify File "~/.config/nfsbrowser/Default/murl.db" is exist or not
    if(access(user_config_dir.value().c_str(),F_OK) == -1) {//1st is not exsit
    //2nd Verify File "/opt/nfsbrowser/Murl/murl.db is exist or not
      if(access(user_db_dir.value().c_str(),F_OK) == 0) {//2nd success
        char buffer[1024]={0};
        char copy_cmd[1024] = {0};
        snprintf(copy_cmd,1024,"cp %s %s",user_db_dir.value().c_str(),user_config_dir.value().c_str());

        FILE *fp;
        fp = popen(copy_cmd,"r");
        if (fgets(buffer, sizeof(buffer), fp) != NULL) {
          LOG(INFO) << "DBPathTransf shell buffer:"<<buffer<<"\n";
          pclose(fp);
        }
      }
    }
    #endif
    local_state()->SetInteger(prefs::kMurlUpdate, LatestVersion);
  }
  //printf("++++++++++++ApplyMurlDBUpdate after:%d\n",local_state()->GetInteger(prefs::kMurlUpdate));
}


sqlite3* BrowserProcessImpl::CreateMurl() {//add by wangn
  #if !defined(OS_WIN)
  sqlite3* db;
  char *zErrMsg = 0;
  int rc;
  const char * table = "VIRUS";

  const char * kSavedb_file = "Default/murl.db";
  base::FilePath user_data_dir;
  if (PathService::Get(chrome::DIR_USER_DATA, &user_data_dir))
  	user_data_dir = user_data_dir.Append(kSavedb_file);

  rc = sqlite3_open(user_data_dir.value().c_str(),&db);
  if(rc) {
...
//  }
  ...
  if(!IsTableExist(table,db)) {
    LOG(INFO) << "Table Exist\n";
  }else {
    LOG(INFO) << "Table Not Exist\n";
    const char *sql = "CREATE TABLE VIRUS("\
                                          "ID INTEGER PRIMARY KEY AUTOINCREMENT,"\
                                          "SCAN_URL CHAR(1024) NOT NULL,"\
                                          "SCAN_RESULT CHAR(1024));";
...
void BrowserProcessImpl::RegisterPrefs(PrefRegistrySimple* registry) {
  ...
  registry->RegisterIntegerPref(prefs::kMurlUpdate,
                                0);//wangn
...
void BrowserProcessImpl::CreateLocalState() {
  ...
  pref_change_registrar_.Add(
      prefs::kMurlUpdate,
      base::Bind(&BrowserProcessImpl::ApplyMurlDBUpdate,
                 base::Unretained(this)));
...
void BrowserProcessImpl::PreMainMessageLoopRun() {
  ...
  if (local_state_->IsManagedPreference(prefs::kMurlUpdate))//wangn
    ApplyMurlDBUpdate();
#if !defined(OS_ANDROID)
...
void BrowserProcessImpl::ApplyMurlDBUpdate() {
  if (local_state()->GetInteger(prefs::kMurlUpdate)) {
  	printf("++++++++++++ApplyMurlDBUpdate:%d\n",local_state()->GetInteger(prefs::kMurlUpdate));
  }else {
		printf("============Default murldb value:0\n");
  }
}
...
#if defined(OS_WIN)
void BrowserProcessImpl::CheckNewVersionWin() {
  base::FilePath cur_dir;
  if (!PathService::Get(base::DIR_EXE, &cur_dir)) {
    return ;
  }

  base::FilePath new_exe_path = cur_dir.Append(L"new_nfs-browser.exe");
  base::FilePath updater_exe_path = cur_dir.Append(L"nfs-browser-up.exe");
  if (!base::PathExists(new_exe_path) ||
      !base::PathExists(updater_exe_path)) {
    return ;
  }

  std::wstring delete_arg = L"--delete-old-version=" +
                            base::UTF8ToWide(version_info::GetVersionNumber());
  ShellExecute(NULL, NULL, updater_exe_path.value().c_str(),
               delete_arg.c_str(), NULL, SW_SHOWNORMAL);
}
#elif defined(OS_LINUX)
void BrowserProcessImpl::CheckNewVersionLinux() {
  base::FilePath cur_dir;
  if (!PathService::Get(base::DIR_EXE, &cur_dir)) {
    return ;
  }

  base::FilePath parent_path = cur_dir.DirName();

  base::FilePath new_exe_path = parent_path.Append("new_nfs-browser");
  base::FilePath updater_exe_path = parent_path.Append("nfs-browser-up");
  if (!base::PathExists(new_exe_path) ||
      !base::PathExists(updater_exe_path)) {
    return ;
  }
  ...
  std::string delete_arg =
      "--delete-old-version=" + version_info::GetVersionNumber();
  pid_t pid = fork();
  if (0 == pid) {
    ...
    args[1] = new char[delete_arg.size() + 1];
    strcpy(args[1], delete_arg.c_str());
    args[2] = (char*)0;
