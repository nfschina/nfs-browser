...
_VULCANIZE_BASE_ARGS = [
  ...
  '--exclude', 'nfsbrowser://resources/html/polymer.html',
  ...
  '--redirect', 'nfsbrowser://resources/cr_elements/|%s' % _CR_ELEMENTS_PATH,
  '--redirect', 'nfsbrowser://resources/css/|%s' % _CSS_RESOURCES_PATH,
  '--redirect', 'nfsbrowser://resources/html/|%s' % _HTML_RESOURCES_PATH,
  '--redirect', 'nfsbrowser://resources/js/|%s' % _JS_RESOURCES_PATH,
  '--redirect', 'nfsbrowser://resources/polymer/v1_0/|%s' % _POLYMER_PATH,
...
def _vulcanize(directory, host, html_in_file, html_out_file='vulcanized.html',
  ...
  output = _run_cmd(['vulcanize'] + _VULCANIZE_BASE_ARGS + extra_args +
                    ['--redirect', 'nfsbrowser://%s/|%s' % (host, target_path),
                     html_in_path])
...
def main():
  ...
  history_extra_args = ['--exclude', 'nfsbrowser://resources/html/util.html',
                        '--exclude', 'nfsbrowser://history/constants.html']
  ...
  lazy_load_extra_args = ['--exclude', 'nfsbrowser://history/app.html']
