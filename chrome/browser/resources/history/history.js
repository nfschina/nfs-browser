...
function Visit(result, continued, model) {
  ...
  var dayFlagE = result.dateTimeOfDay.substr(-2);
  if(dayFlag == "上午" || dayFlag == "下午"){
    if(dayFlag == "上午"){
  ...
  }else if ( dayFlagE== "AM" || dayFlagE == "PM"){
    if(dayFlagE == "AM"){
          var h= result.dateTimeOfDay.substr(0).split(":")[0];
          if(h==12)    h = parseInt(h)-12;
          this.dateTimeOfDay = h+ ":" +result.dateTimeOfDay.split(":")[1].substr(0,2);
    }else{
          var h= result.dateTimeOfDay.substr(0).split(":")[0];
          if(h!=12)    h = parseInt(h)+12;
          this.dateTimeOfDay = h+ ":" +result.dateTimeOfDay.split(":")[1].substr(0,2);
...
Visit.prototype.loadFavicon_ = function(faviconDiv) {
  if (cr.isAndroid) {
    ...
    img.src = 'nfsbrowser://large-icon/' + desiredPixelSize + '/' + this.url_;
  } else {
...
/**
 * Called when the nfsbrowser://large-icon image has finished loading.
...
/** @enum {number} */
HistoryModel.Range = {//yana mark 160905
  ALL_TIME: 0,
  WEEK: 1,
  MONTH: 2,
  DAY:3//yana add 160907
};
...
function expandDay(){
	if(this.getAttribute("expand") == ""){
		...
		var expand = document.querySelector("#date-nav h6[expand]");
		if(expand){
			expand.removeAttribute("expand");
			expand.nextSibling.style.display = "none";
		}
...
function getDayOffset(date){
	    ...
	    var month = tMonth[m - 1];
        var today = new Date();
        var startday = new Date(""+month+" "+d+", "+y+" 00:00 EDT");
        startday.setYear(""+y+"");
        var secsPerDay = 24 * 60 * 60 * 1000;
        var daysLeft = ((today.getTime() - startday.getTime()) / secsPerDay);
		var daysOffset = Math.floor(daysLeft + 0.5);
		return daysOffset;
}

function searchDayHistory(){
	var sld = document.querySelector("#date-nav li[sld]");
	var date = this.getAttribute("date");
	if(sld){
		sld.removeAttribute("sld");
	}
	this.setAttribute("sld","");
	var dayOffset = getDayOffset(date);
	historyView.setPageState('', 0, HistoryModel.Range.DAY, dayOffset);
};
/***********************************yana add 160912***************************************/
function dateLoad(date){
  ...
  for(i=0;i < date.length;i++){
    ...
    var y = date[i].split("-")[0];
    var m = date[i].split("-")[1];
    var d = date [i].split("-")[2];
    if(lasty != y){
    ...
    if(lastm != m || lasty != y){
      ...
      nav.appendChild(h6);
      nav.appendChild(ol);
      h6.addEventListener("click",expandDay);
    }
    if(lastd != d || lastm != m || lasty != y){

      var li = document.createElement("li");
      li.innerHTML = d+"日";
      li.setAttribute("date",y + "-" + m + "-" + d)
      ol.appendChild(li);
      li.addEventListener("click",searchDayHistory);
    }
    lasty = y;
    lastm = m;
    lastd = d;
 ...
 * @private
 */
HistoryModel.prototype.queryDateList_ = function() {
  chrome.send('queryActiveDates');
};
...
function HistoryFocusRow(root, boundary) {
  ...
  this.addItem('star', '.bookmark-section');//yana add 161019
  this.addItem('domain', '[is="action-link"]');
  this.addItem('title', '.title a');
  //this.addItem('menu', '.drop-down');
  this.addItem('menu', '.remove-visit');//yana add 161019
  this.addItem('menu', '.more-from-site');//yana add 161019
}
...
HistoryView.prototype.addTimeframeInterval_ = function(resultsFragment) {
  if ((this.getRangeInDays() == HistoryModel.Range.ALL_TIME)||(this.getRangeInDays() == HistoryModel.Range.DAY))//yana add 160920
    return;
...
HistoryView.prototype.displayResults_ = function(doneLoading) {
  ...
  if (searchText) {
    ...
    //this.addTimeframeInterval_(resultsFragment);//yana mask 160912
 ...
 * @param {HistoryQuery} info An object containing information about the query.
 * @param {Array<HistoryEntry>} results A list of date results.
 * add by yana
 */
function historyDateResult(date) {
  dateLoad(date);
}
