  ...
  BrowserOptions.prototype = {
    ...
    initializePage: function() {
      ...
      if (window.top != window) {
       ...
       // chrome.send('openedAutoClearBrowserData');  //wdg
      ...
      Preferences.getInstance().addEventListener('tabs.newtabpageishomepage',
          this.OnTagPageChanged.bind(this));
      ...
      if (!cr.isChromeOS) {
       ...
       //   $('set-default-browser-section').hidden = true;
          $('set-as-default-browser').disabled = true;
        }
      ...
      $('close_tabs').onchange = function(event) {
        var value = event.target.options[event.target.selectedIndex].value;
        console.log(value)
        Preferences.setIntegerPref(
            'browser.close.swichpagemode',
            value-0 , true);
        chrome.send('defaultCloseTabDirection', [String(value)]);
      };

      $('language-select').onchange = function(event) {
        chrome.send('selectLanguage',
            [String(event.target.options[event.target.selectedIndex].value)]);
      };


  $('open_tabs').onchange = function(event) {
        var value = event.target.options[event.target.selectedIndex].value;
        // alert("open_tabs"+value);
        Preferences.setIntegerPref(
             'browser.opentabs.direction',
             value-0 , true);
       chrome.send('defaultOpenTabDirection', [String(value)]);
      };
      // Languages section.
      ...
      Preferences.getInstance().addEventListener('browser.enable_tab_preview',
          this.onDefaultTabPreviewIndexChange_.bind(this));
      Preferences.getInstance().addEventListener('tab.maxindex',
          this.onDefaultTabPreviewIndexValueChange_.bind(this));
      Preferences.getInstance().addEventListener('download.isLimitSpeed',
          this.onDefaultDownloadSpeedChanged_.bind(this));
      Preferences.getInstance().addEventListener('download.maxspeed',
          this.onDefaultDownloadSpeedValueChanged_.bind(this));
      // cache  section.
      Preferences.getInstance().addEventListener('browser.disk_cache_dir',
          this.onDefaultCacheDirectoryChanged_.bind(this));
      $('cacheLocationChangeButton').onclick = function(event) {
        chrome.send('selectCacheLocation');
      };
       $('resetcacheLocationChangeButton').onclick = function(event) {
        chrome.send('ResetCacheLocation');
      };
      Preferences.getInstance().addEventListener('browser.disk_cache_auto_clear',
          this.OnCacheAutoClear.bind(this));
      Preferences.getInstance().addEventListener('browser.max_disk_cache_size',
          this.OnCacheAutoClearSizeChanged.bind(this));
      if (cr.isChromeOS) {
        $('disable-drive-row').hidden =
            UIAccountTweaks.loggedInAsSupervisedUser();
      }
    ...
    */
    ...
    onDefaultTabPreviewIndexChange_:function(event){
         var  tab_index = $('tab_index');
         if (event.value.value == 2) {
            tab_index.removeAttribute("readonly");
         }else{
            tab_index.setAttribute("readonly","readonly");
         }

    },

    onDefaultTabPreviewIndexValueChange_:function(event){
        $("tab_index").onchange = function(){
...
/**
   modify the download max speed value
       * @param {Event} event Change event.
     * @private
   **/
    onDefaultDownloadSpeedChanged_:function(event){
        /*
         var  download_speed = $('download_speed');
         if (event.value.value) {
            download_speed.removeAttribute("readonly");
         }else{
            download_speed.setAttribute("readonly","readonly");
         }
         */
    },

/**
observer the download max speed changed
    * @param {Event} event Change event.
     * @private
**/

onDefaultDownloadSpeedValueChanged_:function(event){
/*
        $("download_speed").onchange = function(){
              var speeds = parseInt(this.value);
                if(speeds < 1){
                 this.value = 1;
                   }else if(speeds > 9999){
                     this.value = 9999;
...
/**
observer the cache  max size changed
    * @param {Event} event Change event.
     * @private
**/

   OnCacheAutoClearSizeChanged:function(event) {
         $("CacheSize").onchange = function(){
              var cache_size = parseInt(this.value);
                if(cache_size < 100){
                 this.value = 100;
                   }else if(cache_size > 9999){
                     this.value = 9999;
                   }else if(isNaN(cache_size)){
                    this.value = 100;
 }else{
      this.value = cache_size;
    }
    }
...
/**
observer the cache  auto clear need or not  changed
    * @param {Event} event Change event.
     * @private
**/
    OnCacheAutoClear: function(event) {
         var  cache_size = $('CacheSize');
         if (event.value.value) {
            cache_size.removeAttribute("readonly");
            chrome.send('openedAutoClearBrowserData');  //wdg
         }else{
            cache_size.setAttribute("readonly","readonly");
         }
    },
 ...
 /**
     ...
     * @private
     */
    onDefaultCacheDirectoryChanged_: function(event) {
      $('CacheLocationPath').value = event.value.value;

      $('cache-location-label').classList.toggle('disabled',
                                                    event.value.disabled);
      $('cacheLocationChangeButton').disabled = event.value.disabled;
    },



  OnTagPageChanged: function(event){
          var  homepage_id = $('homepage_id');
          if (event.value.value) {
            homepage_id.removeAttribute("readonly");
         }else{
            homepage_id.setAttribute("readonly","readonly");
         }
    ...
    updateDefaultBrowserState_: function(statusString, isDefault,
      ...
      if (!cr.isChromeOS) {
        ...
        if(isDefault) //默认
          $('default-browser-state').style.color = "#666";
          else{
          $('default-browser-state').style.color = "#f00";
          // $('default-browser-state').style.margin-left = 0;
        }
        // $('set-as-default-browser').hidden = !canBeDefault || isDefault;
       $('set-as-default-browser').disabled = !canBeDefault || isDefault;
      }
    },
    ...
    setFontSize_: function(pref) {
      ...
      setOpenTabDiection_: function(pref) {
      var selectCtl = $('open_tabs');
      selectCtl.disabled = pref.disabled;
      // Create a synthetic pref change event decorated as
      // CoreOptionsHandler::CreateValueForPref() does.
      var event = new Event('synthetic-opentab-direction');
      event.value = {
        value: pref.value,
        controlledBy: pref.controlledBy,
        disabled: pref.disabled
      ...
      setCloseTabDiection_: function(pref) {
      var selectCtl_open = $('close_tabs');
      selectCtl_open.disabled = pref.disabled;
      // Create a synthetic pref change event decorated as
      // CoreOptionsHandler::CreateValueForPref() does.
      var event = new Event('synthetic-closetab-direction');
      event.value = {
        value: pref.value,
        controlledBy: pref.controlledBy,
        disabled: pref.disabled
      };
      for (var i = 0; i < selectCtl_open.options.length; i++) {
        if (selectCtl_open.options[i].value == pref.value) {
          selectCtl_open.selectedIndex = i;
          return;
        }
      }
    ...
    setSelectedLanguage_: function(locale) {
      var selectCtl = $('language-select');
      for (var i = 0; i < selectCtl.options.length; i++) {
        if (selectCtl.options[i].value == locale) {
          selectCtl.selectedIndex = i;
          return;
        }
      }
    ...
    toggleExtensionControlledBox_: function(
      ...
      div.style.backgroundImage =
          'url(nfsbrowser://extension-icon/' + extensionId + '/24/1)';
