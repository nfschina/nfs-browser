 ...
 */
function  otherdspSetList(t)
{
  for(i=0;i<5;i++){

            $('settings').children[i].setAttribute("hidden"," ");
             $('sld-nav').children[i].removeAttribute("sld");

          }

         $('sld-nav').children[t].setAttribute('sld',' ');
        $('settings').children[t].removeAttribute('hidden');

}

function dspSetList(t){
 //  if(t==0)
 //  window.location.href = 'nfsbrowser://settings';
 // else
 window.location.href = 'nfsbrowser://settings/#' + t;
for(i=0;i<5;i++){
...
function load() {
  ...
  PageManager.register(ShortCut.getInstance());
  // Overlays.
  ...
  PageManager.registerOverlay(
      ...
      BrowserOptions.getInstance());
    // PageManager.register(ShortCutPage.getInstance());
...
<if expr="is_win">
 ...
 var datahref = window.location.href;
  ...
  dspList =  datahref.split("#")[1];
  if(dspList){
   var n = Number(dspList); 
  if(n>=0&&n<=4){
    // var n = Number(dspList); 
    dspSetList(n);
  }else{
  //  otherdspSetList(0);
    ...
    $("basic-list").onclick = function(){
        dspSetList(0);
    };
    $("tab-list").onclick = function(){
        dspSetList(1);
    };
    $("download-list").onclick = function(){
        dspSetList(2);
    };
    $("shortcut-list").onclick = function(){
        dspSetList(3);
    };
    $("more-list").onclick = function(){
        dspSetList(4);
    };
