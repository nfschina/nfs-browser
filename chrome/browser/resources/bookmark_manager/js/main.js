...
function processHash() {
  ...
  if (valid) {
    $('add-folder').setAttribute('disabled', '');
    $('add-bookmark').setAttribute('disabled', '');

    updateParentId(id);
  } else {
    $('add-folder').removeAttribute('disabled');
    $('add-bookmark').removeAttribute('disabled');
...
function handleImportBookmark() {
  chrome.bookmarks.NFSImport();
}

function handleExportBookmark() {
...
// 处理修改和删除操作
function handleEditAndRemove(e) {
  if (e.target && e.target.className == 'list-remove') {
    recordUserAction('Delete');
  ...
  if (e.target && e.target.className == 'list-edit') {
...
function continueInitializeBookmarkManager(localizedStrings) {
  ...
  bmm.list.addEventListener('click', handleEditAndRemove);
  ...
  $('add-folder').addEventListener('click', handleAddFolder);
  $('add-bookmark').addEventListener('click', handleAddBookmark);
  $('import-bookmark').addEventListener('click', handleImportBookmark);
  $('export-bookmark').addEventListener('click', handleExportBookmark);
