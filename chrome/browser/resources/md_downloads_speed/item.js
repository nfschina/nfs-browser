...
cr.define('downloads', function() {
  ...
  var Item = Polymer({
    ...
    properties: {
      ...
      completelyOnDisk_: {
        computed: 'computeCompletelyOnDisk_(data.state)',
        type: Boolean,
        value: true,
      },
      ...
      showRetry_: {
        computed: 'computeShowRetry_(data.file_externally_removed, isFailed_)',
        type: Boolean,
        value: false,
      },

      showOpen_: {
        computed: 'computeShowOpen_(completelyOnDisk_, showRetry_)',
        type: Boolean,
        value: false,
      },

      showOpenFolder_: {
        computed: 'computeShowOpenFolder_(data.file_externally_removed, completelyOnDisk_, isFailed_)',
        type: Boolean,
        value: false,
      },

      showOperationOpen_: {
        computed: 'computeShowOperationOpen_(data.file_externally_removed, completelyOnDisk_, isFailed_)',
        type: Boolean,
        value: false,
      },

      showResume_: {
        computed: 'computeShowResume_(data.resume, isFailed_)',
        type: Boolean,
        value: false,
      },

      showCompleteInfo_: {
        computed: 'computeShowCompleteInfo_(completelyOnDisk_, isFailed_)',
        type: Boolean,
        value: false,
      },

      isFailed_: {
        computed: 'computeIsFailed_(data.state)',
        type: Boolean,
        value: false,
      },

      isRemoved_: {
        computed: 'computeIsRemoved_(data.file_externally_removed)',
        type: Boolean,
        value: false,
      },
    },
    ...
    listeners: {
      'mousedown': 'onClickTap_',
    },

    /** @private */
    onClickTap_: function() {

      var targe = this.parentNode.children;
      for(var i=1;i<targe.length;i++){
        
        if(targe[i].dataset.sld){
          delete targe[i].dataset.sld;
          delete targe[i].$['content'].dataset.sld
        }

      }
      this.dataset.sld = true;
      this.$['content'].dataset.sld = true;
    ...
    computeControlledBy_: function() {
      ...
      var url = 'nfsbrowser://extensions#' + this.data.by_ext_id;
    ...
    /** @private */
    computeShowOpen_: function() {
      return this.completelyOnDisk_ && !this.showRetry_;
    },

    /** @private */
    computeShowOpenFolder_: function() {
      return (this.data.file_externally_removed || this.completelyOnDisk_) && !this.isFailed_;
    },

    /** @private */
    computeShowOperationOpen_: function() {
      return this.data.file_externally_removed || this.completelyOnDisk_ || this.isFailed_;
    },

    /** @private */
    computeShowRetry_: function() {
      return this.data.file_externally_removed || this.isFailed_;
    },

    /** @private */
    computeShowResume_: function() {
      return this.data.resume && !this.isFailed_;
    },

    /** @private */
    computeShowCompleteInfo_: function() {
      return this.completelyOnDisk_ || this.isFailed_;
    },

    /** @private */
    computeIsFailed_: function() {
      return this.data.state == downloads.States.INTERRUPTED;
    },

    /** @private */
    computeIsRemoved_: function() {
    ...
    observeIsDangerous_: function() {
      if (this.data && !this.isDangerous_) {
        ...
        this.$['file-icon'].src = 'nfsbrowser://fileicon/' + filePath + scaleFactor;
      }
    },
    ...
    onRetryTap_: function() {
      downloads.ActionService.getInstance().retry(this.data.id);
    },
