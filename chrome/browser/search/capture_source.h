...
#ifndef CHROME_BROWSER_SEARCH_CAPTURE_SOURCE_H_
#define CHROME_BROWSER_SEARCH_CAPTURE_SOURCE_H_
...
// CaptureSource is the gateway between network-level chrome: requests for
// thumbnails and the history/top-sites backend that serves these.
class CaptureSource : public content::URLDataSource {
 public:
  CaptureSource(Profile* profile);
 ...
 private:
  ...
  base::WeakPtrFactory<CaptureSource> weak_ptr_factory_;

  DISALLOW_COPY_AND_ASSIGN(CaptureSource);
};

#endif  // CHROME_BROWSER_SEARCH_CAPTURE_SOURCE_H_
