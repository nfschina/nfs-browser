...
void ExternalProcessImporterHost::StartImportSettings(
  ...
  //if (!CheckForFirefoxLock(source_profile)) {  //wdq
  //  Cancel();
  //  return;
  //}
...
void ExternalProcessImporterHost::ShowChromeWarningDialog() {
  base::string16 importer_lock_string;

  if (source_profile_.importer_type == importer::TYPE_CHROME) {
    importer_lock_string = l10n_util::GetStringUTF16(IDS_CHROME_IMPORTER_LOCK_TEXT);
  }

  DCHECK(!headless_);
  importer::ShowImportLockDialog(
      parent_window_,
      base::Bind(&ExternalProcessImporterHost::OnChromeImportLockDialogEnd,
                 weak_ptr_factory_.GetWeakPtr()),
      importer_lock_string);
}
...
void ExternalProcessImporterHost::OnChromeImportLockDialogEnd(bool is_continue) {
  if (is_continue) {
    chrome_lock_->Lock();
