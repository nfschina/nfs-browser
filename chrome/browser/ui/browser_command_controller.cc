...
void EmptyCallback(void) {
  //empty callback for set default browser
}
...
void BrowserCommandController::SetAsDefaultImpl() {
  const base::Closure on_finished_callback = base::Bind(EmptyCallback);
  switch (shell_integration::GetDefaultWebClientSetPermission()) {
    case shell_integration::SET_DEFAULT_NOT_ALLOWED:
      NOTREACHED();
      break;
    case shell_integration::SET_DEFAULT_UNATTENDED:
      shell_integration::SetAsDefaultBrowser();
      break;
    case shell_integration::SET_DEFAULT_INTERACTIVE:
      break;
  }
}
...
void BrowserCommandController::ExecuteCommandWithDisposition(
  ...
  switch (id) {
    ...
    case IDC_RELOAD: 
      ...
      break;
    case IDC_RELOAD_ALL: 
      ReloadAll(browser_, disposition);
      break;
    ...
    case IDC_SHOW_AS_TAB:
      ...
      break;
    case IDC_LOCK_OR_UNLOCK: //add by yana 160819
      if(profile()->GetPrefs()->GetBoolean(prefs::kLockKeyword)){
          profile()->GetPrefs()->SetBoolean(prefs::kLockKeyword, false);
      }else{
          profile()->GetPrefs()->SetBoolean(prefs::kLockKeyword, true);
      }
      break;
    case IDC_FULLSCREEN:
    ...
    // Zoom
    case IDC_ZOOM_PLUS://yana 160819
      if(profile()->GetPrefs()->GetBoolean(prefs::kLockKeyword)){
          return;
      }else{
    ...
    case IDC_ZOOM_NORMAL:
      ...
      break;
    case IDC_ZOOM_MINUS://yana 160819
      if(profile()->GetPrefs()->GetBoolean(prefs::kLockKeyword)){
          return;
      }else{
    ...
    case IDC_ABOUT:
      ...
      break;
    case IDC_FEEDBACK_TITLE:
      ShowFeedbackChrome(browser_);
      break;
    case IDC_ACCOUNT_DIALOG:
      ShowAccountDialog(browser_, std::string(), std::string());
      break;
    case IDC_CHECK_VERSION_DIALOG:
      ShowUpdateDialog(browser_, std::string(), std::string());
      break;
      case IDC_OFFICIAL_FORUM:
      ShowOfficialForum(browser_);
      break;
    ...
    case IDC_SET_AS_DEFAULT_BROWSER:
      BrowserThread::PostTask(
      BrowserThread::FILE, FROM_HERE,
      base::Bind(&BrowserCommandController::SetAsDefaultImpl, base::Unretained(this)));
      break;

    case IDC_SHOW_SEARCHBAR:
      ChangeVisibility(SearchBar);
      break;

    case IDC_SHOW_TOOLBAR:
      ChangeVisibility(ToolBar);
      break;

    case IDC_SHOW_BOOKMARKBAR:
      ChangeVisibility(BookMarkBar);
      break;

    case IDC_IMPORT_BOOKMARK:
      ImportExportBookmark(true);
      break;

    case IDC_EXPORT_BOOKMARK:
      ImportExportBookmark(false);
      break;

    case IDC_ADDRESSBAR_LIST:
      OpenAdressBarList(browser_);
      break;

    case IDC_SAVE_PAGE_AS_PIC:
      SavePageAsPic(browser_);
      break;

    case IDC_OPEN_SEARCH_URL:
      OpenSearchURL(browser_);
      break;

  case IDC_DETACH_WEB_VEDIO:
      DetachWebVedioToSimpleWebBrowser(browser_);
      break;

    case IDC_ATTACH_WEB_VEDIO:
      AttachWebVedio(browser_);
      break;

     case IDC_THEME_GALLERY:
      ShowThemeGallery(browser_);
      break;

    case IDC_COMPATIBLE_MODE_ON:
      SetCompatibleMode(browser_, true);
      break;

    case IDC_COMPATIBLE_MODE_OFF:
      SetCompatibleMode(browser_, false);
      break;
...
void BrowserCommandController::InitCommandState() {
  ...
  command_updater_.UpdateCommandEnabled(IDC_RELOAD_ALL, true);
  ...
  command_updater_.UpdateCommandEnabled(IDC_LOCK_OR_UNLOCK, true);
  // Show various bits of UI
  ...
  // added by nfs
  command_updater_.UpdateCommandEnabled(IDC_OPEN_SEARCH_URL, true);
  command_updater_.UpdateCommandEnabled(IDC_NAVIGATE, true);

  for (int i = IDC_SEARCH_ENGINE_START; i <= IDC_SEARCH_ENGINE_END; ++i)  {
    command_updater_.UpdateCommandEnabled(i, true);
  }

  command_updater_.UpdateCommandEnabled(IDC_REVOCATION, true);
  command_updater_.UpdateCommandEnabled(IDC_CLEAR_HISTORY, true);
  command_updater_.UpdateCommandEnabled(IDC_MY_APP, true);

  bool previous_disallowed = base::ThreadRestrictions::SetIOAllowed(true);
  bool is_default_browser = shell_integration::GetDefaultBrowser() == shell_integration::DefaultWebClientState::IS_DEFAULT;
  base::ThreadRestrictions::SetIOAllowed(previous_disallowed);

  command_updater_.UpdateCommandEnabled(IDC_SET_AS_DEFAULT_BROWSER, !is_default_browser);

  command_updater_.UpdateCommandEnabled(IDC_SHOW_SEARCHBAR, true);
  command_updater_.UpdateCommandEnabled(IDC_SHOW_TOOLBAR, true);
  command_updater_.UpdateCommandEnabled(IDC_SHOW_BOOKMARKBAR, true);

  command_updater_.UpdateCommandEnabled(IDC_IMPORT_BOOKMARK, true);
  command_updater_.UpdateCommandEnabled(IDC_EXPORT_BOOKMARK, true);

  command_updater_.UpdateCommandEnabled(IDC_ADDRESSBAR_LIST, true);

  command_updater_.UpdateCommandEnabled(IDC_DETACH_WEB_VEDIO, true);
  command_updater_.UpdateCommandEnabled(IDC_ATTACH_WEB_VEDIO, true);

  command_updater_.UpdateCommandEnabled(IDC_ABOUT, true);
  command_updater_.UpdateCommandEnabled(IDC_FEEDBACK_TITLE, true);
  command_updater_.UpdateCommandEnabled(IDC_CHECK_VERSION_DIALOG, true);
  command_updater_.UpdateCommandEnabled(IDC_OFFICIAL_FORUM, true);

  command_updater_.UpdateCommandEnabled(IDC_THEME_GALLERY, true);
  command_updater_.UpdateCommandEnabled(IDC_ACCOUNT_DIALOG, true);
  command_updater_.UpdateCommandEnabled(IDC_COMPATIBLE_MODE_ON, true);
  command_updater_.UpdateCommandEnabled(IDC_COMPATIBLE_MODE_OFF, true);
  // added end
...
void BrowserCommandController::UpdateSaveAsState() {
  ...
  command_updater_.UpdateCommandEnabled(IDC_SAVE_PAGE_AS_PIC, CanSavePageAsPic(browser_));
}
...
void BrowserCommandController::ChangeVisibility(ViewID id) {
  PrefService* prefs = browser_->profile()->GetPrefs();
  switch(id) {
    case SearchBar:
      prefs->SetBoolean(prefs::kShowSearchBar, !prefs->GetBoolean(prefs::kShowSearchBar));
      break;
    case BookMarkBar:
      prefs->SetBoolean(bookmarks::prefs::kShowBookmarkBar, !prefs->GetBoolean(bookmarks::prefs::kShowBookmarkBar));
      break;
    case ToolBar:
      prefs->SetBoolean(prefs::kShowToolbar, !prefs->GetBoolean(prefs::kShowToolbar));
      break;
    default:
      DCHECK(false);
    }
...
void BrowserCommandController::ImportExportBookmark(bool import) {
  std::string import_str = "bookmarks.NFSImport";
  std::string export_str = "bookmarks.export";
  if (import)  {
    extensions::BookmarksIOFunction* io_function =
          static_cast<extensions::BookmarksIOFunction*> (ExtensionFunctionRegistry::GetInstance()->NewFunction(import_str));
    io_function-> set_browser_context(browser_->profile());
    ShowImportSelectDialog(browser_->window()->GetNativeWindow());
  } else {
    base::ThreadRestrictions::SetIOAllowed(true);

    extensions::BookmarksIOFunction* io_function =
           static_cast<extensions::BookmarksExportFunction*> (ExtensionFunctionRegistry::GetInstance()->NewFunction(export_str));
    io_function-> set_browser_context(browser_->profile());
    io_function->SelectFile(ui::SelectFileDialog::SELECT_SAVEAS_FILE);
  }
}
