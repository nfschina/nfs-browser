...
content::WebContents* g_hide_new_tab_contents = NULL;
...
Browser* GetBrowserForDisposition(chrome::NavigateParams* params) {
  ...
  switch (params->disposition) {
    ...
    case WindowOpenDisposition::IGNORE_ACTION:
      return NULL;
    case WindowOpenDisposition::NEW_SIMPLE_WEB_WINDOW:
      // Make a browser window with no tabstrip, no toolbar, no bookmark bar
      return new Browser(Browser::CreateParams::CreateForSimpleWeb(profile));
    case WindowOpenDisposition::DOWNLOAD_MANAGER:
      return new Browser(Browser::CreateParams::CreateForDownloadManager(profile));
    case WindowOpenDisposition::NEW_DOWNLOAD_TASK:
      return new Browser(Browser::CreateParams::CreateForNewDownloadTask(profile));
    default:
      NOTREACHED();
  }
  return NULL;
...
void CreateHideNewTabContent(Profile* profile) {
  GURL url(chrome::kChromeUINewTabURL);
  g_hide_new_tab_contents = chrome::CreateHideWebContent(profile, url);
}
...
content::WebContents* CreateHideWebContent(content::BrowserContext* browser_context, const GURL& url) {
  ...
  Profile* profile = (Profile*) browser_context;
  ...
  content::WebContents* hidden_webcontent = WebContents::Create(create_params);
  DCHECK(hidden_webcontent);
  if (!hidden_webcontent) {
    return NULL;
  }

  BrowserNavigatorWebContentsAdoption::AttachTabHelpers(hidden_webcontent);

  LoadURLInContents(hidden_webcontent, url, &params);
  return hidden_webcontent;
}
...
void Navigate(NavigateParams* params) {
  ...
  if (source_browser != NULL &&  source_browser->is_type_simple_web()) {
  ...
  Profile* profile = params->initiating_profile;
  PrefService* prefs = profile->GetPrefs();
  if (prefs->GetBoolean(prefs::kOpenNewTabPage)  &&
        params->disposition == WindowOpenDisposition::CURRENT_TAB &&
       !params->is_renderer_initiated)
    params->disposition = WindowOpenDisposition::NEW_FOREGROUND_TAB;

   // 弹出窗口在新标签打开
  if (prefs->GetBoolean(prefs::kPomptInNewWindow)  &&
        params->disposition == WindowOpenDisposition::NEW_POPUP)
    params->disposition = WindowOpenDisposition::NEW_FOREGROUND_TAB;

  // 确保收藏管理器窗口在新窗口打开（保险）
  if (params->url == GURL(kChromeUIDownloadsURL) &&
        params->disposition == WindowOpenDisposition::CURRENT_TAB) {
    VLOG(0) << "chrome::ShowDownloads 2 ";
    chrome::ShowDownloads(params->browser);
    return;
  }

  // 屏蔽新建下载地址栏访问
  if (params->url == GURL(kChromeUIDownloadsNewURL) && params->disposition != WindowOpenDisposition::NEW_DOWNLOAD_TASK) {
    return;
  }
  ...
  const bool is_new_tab = GURL(chrome::kChromeUINewTabURL) == params->url;
  const bool is_incognito = profile->HasOffTheRecordProfile();
  const bool is_should_create_hide_newtab = is_new_tab && !is_incognito;
  if (is_should_create_hide_newtab) {
    params->target_contents = g_hide_new_tab_contents;
  ...
  if (singleton_index >= 0) {
    ...
    if(!chrome::CompareURLs(params->url, target->GetURL())) {
      ...
      if(chrome::IsSettingUrl(params->url, replacements)) {
  ...
  if (is_should_create_hide_newtab) {
    content::BrowserThread::PostDelayedTask(
        ...
        base::Bind(&CreateHideNewTabContent,
                   params->browser->profile()),
...
bool IsURLAllowedInIncognito(const GURL& url,
  ...
  if (url.scheme() == content::kChromeUIScheme &&
      (url.host() == chrome::kChromeUISettingsHost ||
       ...
       url.host() == chrome::kChromeUIExtensionsFrameHost ||
