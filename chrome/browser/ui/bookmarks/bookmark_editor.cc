...
bool BookmarkExist(const bookmarks::BookmarkNode* parent,
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (child_node->is_url() && child_node->url() == url) {
      return true;
    }
  }
  return false;
...
const bookmarks::BookmarkNode* GetBookmarkNodeByURL(const bookmarks::BookmarkNode* parent,
                   const GURL& url) {
  int count = parent->child_count();
  for (int i = 0; i < count; ++i) {
    const bookmarks::BookmarkNode* child_node = parent->GetChild(i);
...
int BookmarkEditor::EditDetails::GetWindowTitleId() const {
  ...
  switch (type) {
    ...
    case EditDetails::NEW_URL:
      dialog_title = IDS_BOOKMARK_BUBBLE_PAGE_BOOKMARK_CDOS;
      break;
...
const BookmarkNode* BookmarkEditor::ApplyEditsWithPossibleFolderChange(
    ...
    const GURL& new_url) {
  if ((details.type == EditDetails::NEW_URL &&
      !BookmarkExist(new_parent, new_url)) ||
      details.type == EditDetails::NEW_FOLDER) {
  ...
  const BookmarkNode* node = details.existing_node;
  if (details.type == EditDetails::NEW_URL) {
    node = GetBookmarkNodeByURL(new_parent, new_url);
  }
  DCHECK(node);
