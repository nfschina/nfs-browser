...
void PrefsTabHelper::RegisterProfilePrefs(
  ...
  for (size_t i = 0; i < kFontDefaultsLength; ++i) {
    ...
    const std::string system_name =
        base::ToLowerASCII(base::SysInfo::GetSystemName());
    if (pref.pref_name == prefs::kWebKitStandardFontFamily) {
      if (system_name.find("nfs desktop") != std::string::npos ||
          system_name.find("nfs") != std::string::npos) {
        pref.resource_id = IDS_STANDARD_FONT_FAMILY_CDOS;
      } else if (system_name == "ubuntu") {
        pref.resource_id = IDS_STANDARD_FONT_FAMILY_UBUNTU;
      } else if (system_name == "deepin") {
        pref.resource_id = IDS_STANDARD_FONT_FAMILY_DEEPIN;
      } else if (system_name == "linuxmint") {
        pref.resource_id = IDS_STANDARD_FONT_FAMILY_MINT;
      }
    }
#endif
