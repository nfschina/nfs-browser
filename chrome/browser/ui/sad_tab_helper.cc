...
const int kReloadDelayMilliseconds = 16;
const int kReloadBeforeFeedbackIsDisplayed = 3;
...
void SadTabHelper::Reload() {
  if (reload_times_ < kReloadBeforeFeedbackIsDisplayed) {
    sad_tab_->Reload();
    reload_times_++;
  } else {
    sad_tab_->Show();
...
void SadTabHelper::InstallSadTab(base::TerminationStatus status) {
  ...
  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      base::Bind(&SadTabHelper::Reload, base::Unretained(this)),
      base::TimeDelta::FromMilliseconds(kReloadDelayMilliseconds));
}
