...
Browser* OpenEmptyWindow(Profile* profile) {
  ...
  GURL url;
  if (chrome::IsSetHomePage(profile, &url)) {
...
void ReloadAll(Browser* browser, WindowOpenDisposition disposition) {
  content::RecordAction(UserMetricsAction("ReloadAll"));
  browser->tab_strip_model()->ExecuteContextMenuCommand(browser->tab_strip_model()->count(),TabStripModel::CommandReloadAllTabs);
}
...
void NewTab(Browser* browser) {
  ...
  GURL url, newtab_is_homepage;
  if (chrome::IsSetHomePage(browser->profile(), &url)) {
    newtab_is_homepage = url;
  } else {
    newtab_is_homepage = GURL();
  }

  if (browser->is_type_tabbed()) {
    AddTabAt(browser, newtab_is_homepage, -1, true);
    ...
    AddTabAt(b, newtab_is_homepage, -1, true);
    b->window()->Show();
...
void BookmarkCurrentPageIgnoringExtensionOverrides(Browser* browser) {
  ...
  //zhangyq: Add bookmark when user confirm(eg. user click ok button).
  browser->window()->ShowBookmarkBubble(url, title, was_bookmarked_by_user);
  #if 0
...
void QrCurrentPage(Browser* browser) {
  GURL url;
  base::string16 title;
  WebContents* web_contents =
      browser->tab_strip_model()->GetActiveWebContents();
  GetURLAndTitleToBookmark(web_contents, &url, &title);
  browser->window()->ShowQrBubble(url);
}
...
void BookmarkCurrentPage(Browser* browser) {
  content::RecordAction(UserMetricsAction("BookmarkCurrentPage"));
  chrome::ShowBookmarkCurrentPageDialog(browser);
}
...
void Find(Browser* browser) {
  content::RecordAction(UserMetricsAction("Copy"));
  browser->window()->CutCopyPaste(IDC_COPY);
  ...
  if (clipboard) {
    clipboard->ReadText(ui::CLIPBOARD_TYPE_SELECTION, &find_text);
    clipboard->Clear(ui::CLIPBOARD_TYPE_SELECTION);
  }
  find_bar->SetFindTextAndSelectedRange(find_text,
                                        find_helper->selected_range());
  ShowFindBar(browser);
  find_helper->StartFinding(find_text, true, false, true);
}
...
content::RenderFrameHost* GetScreenShotExtensionFrameHost(const Browser* browser) {
  const std::string screenshot_id = extensions::GetScreenShotID();

  extensions::ProcessManager* process_manager = extensions::ProcessManager::Get(browser->profile());
  if (!process_manager) {
    return NULL;
  }
  extensions::ExtensionHost* extension_host = process_manager->GetBackgroundHostForExtension(screenshot_id);
  if (!extension_host) {
    return NULL;
  }
...
void SavePageAsPic(Browser* browser) {
  content::RecordAction(UserMetricsAction("SavePageAsPic"));
  content::RenderFrameHost* host = GetScreenShotExtensionFrameHost(browser);

  if (host == NULL)  {
    return;
  }

  const std::string jsFunc = "screenshot.captureWebpage(\"\")";
  host->ExecuteJavaScript(base::ASCIIToUTF16(jsFunc.c_str()));
}

bool CanSavePageAsPic(const Browser* browser) {
  // LocalState can be NULL in tests.
  if (g_browser_process->local_state() &&
      !g_browser_process->local_state()->GetBoolean(
      prefs::kAllowFileSelectionDialogs)) {
  ...
  WebContents* current_tab = browser->tab_strip_model()->GetActiveWebContents();
  // http or https only
  if (!current_tab || !current_tab->GetURL().SchemeIsHTTPOrHTTPS() )  {
    return false;
  }

  // going to use the screenshot extension
  if (GetScreenShotExtensionFrameHost(browser) == NULL)  {
    return false;
  }
...
void OpenSearchURL(Browser* browser) {
  SearchBarView* search_bar = browser->window()->GetSearchBar();
  if (!search_bar) {
    return;
  }

  WindowOpenDisposition open_disposition = search_bar->GetWindowOpenDisposition();
  GURL url(search_bar->GetInputString());
  NavigateParams params(browser, url, search_bar->GetPageTransition());
  params.disposition = open_disposition;
  Navigate(&params);
}
...
void SetCompatibleMode(Browser* browser, bool mode) {
  TabStripModel* tab_strip = browser->tab_strip_model();
  WebContents* contents = tab_strip->GetWebContentsAt(tab_strip->active_index());
  content::RenderViewHost* host = contents->GetRenderViewHost();
  content::WebPreferences prefs = host->GetWebkitPreferences();
  prefs.compatible_mode = mode;
  host->UpdateWebkitPreferences(prefs);
  std::vector<content::RenderFrameHost*> frames = contents->GetAllFrames();
  for (auto frame : frames) {
    frame->SetCompatibleMode(mode);
  }
}

void DetachWebVedioToSimpleWebBrowser(Browser* browser) {
  TabStripModel* tab_strip = browser->tab_strip_model();
  if (tab_strip->count() == 1){
    NewTab(browser);
    SelectNextTab(browser);
  }
  WebContents* contents =
      tab_strip->DetachWebContentsAt(tab_strip->active_index());
  contents->GetMainFrame()->setVideoPopupWindowStatus(true);
  Browser* b = new Browser(Browser::CreateParams::CreateForWebVedio(browser));
  b->tab_strip_model()->AppendWebContents(contents, true);
  b->window()->Show();
}

void AttachWebVedio(Browser* browser) {
  TabStripModel* tab_strip = browser->tab_strip_model();
  WebContents* contents =
      tab_strip->DetachWebContentsAt(tab_strip->active_index());
  ...
  if (!source_valid) {
   ...
   if (!source_valid) {
      source = new Browser(Browser::CreateParams(browser->profile()));
   }

  source->tab_strip_model()->AppendWebContents(contents, true);
  if (!source_valid) {
    source->window()->Show();
  } else {
    source->window()->Activate();
  }

  contents->GetMainFrame()->setVideoPopupWindowStatus(false);
  contents->GetMainFrame()->RestoreVideoPopupWindow(extensions::GetCdosPopupID());

}

void OpenAdressBarList(Browser* browser) {
  LocationBarView* location_bar_view =
        BrowserView::GetBrowserViewForBrowser(browser)->GetLocationBarView();
  if (location_bar_view) {
    if (location_bar_view->IsOmniboxDropdownViewOpen()) {
      location_bar_view->CloseOmniboxDropdownView();
    } else {
      location_bar_view->ShowOmniboxDropdownView();
    }
  }
}
}  // namespace chrome
