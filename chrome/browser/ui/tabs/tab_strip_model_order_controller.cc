...
int TabStripModelOrderController::DetermineNewSelectedIndex(
  ...
  const int mode = chrome::FindBrowserWithWebContents(removed_contents)->
      profile()->GetPrefs()->GetInteger(prefs::kCloseTabSwitch);
  ...
  if (mode == 0) {
    ...
    if (removing_index < tab_count - 1) {
      return removing_index;
    }
    return tab_count - 2;
  } else {
    if (0 == removing_index && tab_count > 1) {
      return 0;
    }
