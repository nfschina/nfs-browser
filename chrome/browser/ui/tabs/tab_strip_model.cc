...
bool TabStripModel::CloseWebContentsAt(int index, uint32_t close_types) {
  DCHECK(ContainsIndex(index));
  if (kClickToCloseNotice() && order_controller_->DetermineNewSelectedIndex(index) == kNoTab)
    if (IsNewTabAtEndOfTabStrip(contents_data_[index]->web_contents()))
      return true;
  std::vector<int> closing_tabs;
  closing_tabs.push_back(index);
  const bool closing_all = closing_tabs.size() == contents_data_.size();
  if(closing_all && !kClickToCloseNotice()) {
    delegate_->CloseFrame();
    return true;
  }
...
void TabStripModel::AddWebContents(WebContents* contents,
  ...
  if (ui::PageTransitionTypeIncludingQualifiersIs(transition,
      ...
      (profile_->GetPrefs()->GetInteger(prefs::kOpenDirection) == 0) &&
  ...
  if (-2 == index) {
    index = count();
  } else if (profile_->GetPrefs()->GetInteger(prefs::kOpenDirection) == 0) {
...
bool TabStripModel::IsContextMenuCommandEnabled(
  ...
  switch (command_id) {
    ...
    case CommandSuspendOtherTabs://modify buy Yana 170103
    ...
    case CommandBookmarkCurrentPage:
      /*return browser_defaults::bookmarks_enabled &&
          delegate_->CanBookmarkCurrentPage();*/
...
void TabStripModel::ExecuteContextMenuCommand(
  ...
  switch (command_id) {
    ...
    case CommandReloadAllTabs: {
      content::RecordAction(UserMetricsAction("TabContextMenu_ReloadAllTabs"));
      for (int i = count() - 1; i >= 0; --i) {
        WebContents* tab = GetWebContentsAt(i);
        if (tab) {
          CoreTabHelperDelegate* core_delegate =
              CoreTabHelper::FromWebContents(tab)->delegate();
          if (!core_delegate || core_delegate->CanReloadContents(tab))
    ...
    case CommandCloseTab: {
      ...
      if (kClickToCloseNotice()){
        //CloseTracker close_tracker(GetWebContentsFromIndices(GetIndicesForCommand(context_index)));
        //if (GetIndicesForCommand(context_index).size() != 0 || close_tracker.HasNext())
        if (GetIndicesForCommand(context_index).size() != 0)
    ...
    case CommandSuspendOtherTabs: {
      delegate_->SuspendOtherTabs();
      break;
    }
    ...
    case CommandBookmarkCurrentPage: {
      content::RecordAction(
          UserMetricsAction("TabContextMenu_BookmarkCurrentPage"));

      delegate_->BookmarkCurrentPage();
      break;
    }
...
std::vector<int> TabStripModel::GetIndicesClosedByCommand(
  ...
  DCHECK(ContainsIndex(index));
  DCHECK(id == CommandCloseTabsToLeft || id == CommandCloseTabsToRight || id == CommandCloseOtherTabs
        || id == CommandSuspendOtherTabs);
  bool is_selected = IsTabSelected(index);
  int start;
  if (id == CommandCloseTabsToLeft || id == CommandCloseTabsToRight) {
    if (is_selected) {
...
bool TabStripModel::kClickToCloseNotice() {
  Browser* browser = chrome::FindLastActiveWithProfile(profile_);

  return profile_->GetPrefs()->GetBoolean(prefs::kClickToCloseNotice) && browser &&
...
bool TabStripModel::InternalCloseTabs(const std::vector<int>& indices,
  ...
  while (close_tracker.HasNext()) {
    ...
    if (index == 0 && closing_all&&(!closing_all_) && kClickToCloseNotice()) {
      content::RecordAction(UserMetricsAction("TabContextMenu_NewTab"));
      UMA_HISTOGRAM_ENUMERATION("Tab.NewTab",
                               TabStripModel::NEW_TAB_CONTEXT_MENU,
                                TabStripModel::NEW_TAB_ENUM_COUNT);
      delegate()->AddTabAt(GURL(), index + 1, true);
    }
  ...
  if (ref && closing_all && !retval && !kClickToCloseNotice()) {
    FOR_EACH_OBSERVER(TabStripModelObserver, observers_,
