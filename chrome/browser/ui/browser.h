...
class Browser : public TabStripModelObserver,
 ...
 public:
  ...
  enum Type {
    ...
    TYPE_TABBED = 1,
    TYPE_POPUP = 2,
    TYPE_SIMPLE_WEB = 3, // huk 新设计的一个类型，跟浏览器主窗口风格类似，但是只是外框加网页,效果参看下载管理器
  };
  ...
  struct CreateParams {
    ...
    static CreateParams CreateForSimpleWeb(Profile* profile);

    static CreateParams CreateForDownloadManager(Profile* profile);

    static CreateParams CreateForNewDownloadTask(Profile* profile);

    static CreateParams CreateForWebVedio(Browser* source);

    // The browser type.
    Type type;

    //simple web type
    SimpleWeb simple_web_type;
 ...
 void ClearBrowserData();
  ...
  Browser* source() const { return source_; }
  ...
  bool is_type_simple_web() const { return type_ == TYPE_SIMPLE_WEB; }

  bool is_simple_web_none() const {return simple_web_type_ == SIMPLE_NONE; }
  bool is_simple_web_normal() const {return simple_web_type_ == SIMPLE_NORMAL; }
  bool is_simple_web_download_manager() const {return simple_web_type_ == DOWNLOAD_MANAGER; }
  bool is_simple_web_new_download_task() const {return simple_web_type_ == DOWNLOAD_NEW_TASK; }
  bool is_simple_web_vedio() const {return simple_web_type_ == WEB_VEDIO; }
  ...
  int GetWithInProgressDownloads(); //add by luyue
 ...
 private:
  ...
  void OpenSearchContent(const base::string16& search_text) override;
  ...
  void AddFavorite(const base::string16& url, const base::string16& title) override;
  ...
  // huk added
  void RecordErrorPageInfo(const std::string& url, const std::string& detail) override;
  ...
  //if type is simpleweb this type
  const SimpleWeb simple_web_type_;
  ...
  // [zhangyq] Source browser to create this browser.
  Browser* source_;
