...
void WindowSizer::DetermineWindowBoundsAndShowState(
  ...
  *bounds = specified_bounds;
  ...
  if (bounds->IsEmpty() || GetWindowDefaultShowState() == ui::SHOW_STATE_MAXIMIZED)  {
...
bool WindowSizer::GetLastActiveWindowBounds(
  ...
  // huk 这里saved_work_area 记录的是normal状态的窗口大小，show_state记录normal/maximize状态
  if ( *show_state == ui::SHOW_STATE_MAXIMIZED && screen_ != NULL)  {
   GetDefaultWindowBounds(GetTargetDisplay(gfx::Rect()), bounds);
   // *bounds = screen_->GetPrimaryDisplay().bounds();
  }
...
void WindowSizer::GetDefaultWindowBounds(const display::Display& display,
  ...
  int default_width = std::min(work_area.width() * 3/4- 2 * kWindowTilePixels, 1450);
  int default_height = work_area.height() * 3/4 - 2 * kWindowTilePixels;
...
#else
  const int x_offset = (work_area.width() - default_width) / 2;
  const int y_offset = (work_area.height() - default_height) / 2;

  default_bounds->SetRect(x_offset,
                          y_offset,
                          default_width, default_height);
#endif
}
