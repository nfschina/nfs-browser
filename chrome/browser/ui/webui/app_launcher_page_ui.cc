...
std::string AppLauncherPageUI::HTMLSource::GetContentSecurityPolicyScriptSrc()
  ...
  return "script-src nfsbrowser://resources 'self' 'unsafe-eval' 'unsafe-inline';";
}

std::string AppLauncherPageUI::HTMLSource::GetContentSecurityPolicyStyleSrc()
    const {
  return "style-src 'self' nfsbrowser://resources nfsbrowser://theme 'unsafe-inline';";
}

std::string AppLauncherPageUI::HTMLSource::GetContentSecurityPolicyImgSrc()
    const {
  return "img-src nfsbrowser://extension-icon nfsbrowser://theme nfsbrowser://resources "
      "data:;";
}
