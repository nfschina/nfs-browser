...
content::WebUIDataSource* CreateDownloadsSpeedUIHTMLSource(Profile* profile) {
  ...
  source->AddLocalizedString("openDownload", IDS_DOWNLOAD_LINK_OPEN_DOWNLOAD);
  source->AddLocalizedString("newDownload", IDS_DOWNLOAD_LINK_NEW_DOWNLOAD);
  source->AddLocalizedString("openSetting", IDS_DOWNLOAD_LINK_OPEN_SETTING);
  source->AddLocalizedString("controlWait", IDS_DOWNLOAD_WAIT);
  source->AddLocalizedString("filealreadydelete", IDS_DOWNLOAD_FILE_DELETED);
  source->AddLocalizedString("downloadfailed", IDS_DOWNLOAD_FAILED);
...
MdDownloadsSpeedUI::MdDownloadsSpeedUI(content::WebUI* web_ui) : WebUIController(web_ui) {
  Profile* profile = Profile::FromWebUI(web_ui);
  DownloadManager* dlm = BrowserContext::GetDownloadManager(profile);

  web_ui->OverrideTitle(l10n_util::GetStringUTF16(IDS_DOWNLOAD_MANAGER_TITLE));

  handler_ = new MdDownloadsDOMHandler(dlm, web_ui);
  web_ui->AddMessageHandler(handler_);

  // Set up the chrome://downloads/ source.
  content::WebUIDataSource* source = CreateDownloadsSpeedUIHTMLSource(profile);
...
// static
base::RefCountedMemory* MdDownloadsSpeedUI::GetFaviconResourceBytes(
    ui::ScaleFactor scale_factor) {
  return ResourceBundle::GetSharedInstance().
      LoadDataResourceBytesForScale(IDR_DOWNLOADS_FAVICON, scale_factor);
}

void MdDownloadsSpeedUI::RenderViewReused(
