...
//#if !defined(OS_LINUX)
//  return CreateDownloadItemValueNormal(download_item);
//#else
  return CreateDownloadItemValueSpeed(download_item);
//#endif
}

std::unique_ptr<base::DictionaryValue>
DownloadsListTracker::CreateDownloadItemValueNormal(
      content::DownloadItem* download_item) const {
  ...
  switch (download_item->GetState()) {
    ...
    case content::DownloadItem::PAUSING: 
      state = "PAUSED";
      progress_status_text = download_model.GetTabProgressStatusText();
      percent = download_item->PercentComplete();
      break;
...
std::unique_ptr<base::DictionaryValue>
DownloadsListTracker::CreateDownloadItemValueSpeed(
      content::DownloadItem* download_item) const {
  // TODO(asanka): Move towards using download_model here for getting status and
  // progress. The difference currently only matters to Drive downloads and
  // those don't show up on the downloads page, but should.
  DownloadItemModelSpeed download_model(download_item);
...
//        state = "PAUSED";
  ...
  DCHECK(state);
  //if (percent < 0) {
  if (state && strcmp(state,"IN_PROGRESS") && percent < 0) {
    percent = 0;
  }
  ...
  file_value->SetBoolean("retry", retry);
  file_value->SetString("state", state);
  file_value->SetString("file_size", download_model.GetTabProgressFileSizeText());
  file_value->SetString("speed", download_model.GetTabProgressSpeedText());
...
bool DownloadsListTracker::ShouldShow(const DownloadItem& item) const {
  ...
  bool should_show = url.find(nfsDownloadVideo, url.length() - strlen(nfsDownloadVideo)) == std::string::npos ? true : false;
  return should_show && !download_crx_util::IsExtensionDownload(item) &&
...
void DownloadsListTracker::Init() {
  ...
  if (profile->IsOffTheRecord()) {
    ...
    if(profile->HasOffTheRecordProfile()) {
      ...
      original_notifier_.reset(new AllDownloadItemNotifier(
          BrowserContext::GetDownloadManager(off_the_record_profile), this));
    }
  }
