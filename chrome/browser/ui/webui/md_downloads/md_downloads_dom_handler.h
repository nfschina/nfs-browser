...
class MdDownloadsImpl {
 public:
  static void RemoveDownloads(const DownloadVector& to_remove);
};
...
class MdDownloadsDOMHandler : public content::WebContentsObserver,
 ...
 public:
  ...
  // Callback for the "retry" message - retry the file download from shelf
  // and list.
  ...
  // Callback for the "newItem" message - create new item download.
  void HandleNewItem(const base::ListValue* args);

  // Callback for the "openSetting" message - enter into setting page.
  void HandleOpenSetting(const base::ListValue* args);

  // Callback for the "openSetting" message - enter into setting page.
  void HandleRightMenu(const base::ListValue* args);

  // Callback for the "Remove" message - removes the file download.
  void RemoveConfirmed(nfsbrowser::MessageBoxResult result, bool checked);
 ...
 private:
  ...
  // Used to trigger progress check.
  std::unique_ptr<base::RepeatingTimer> check_timer_;

  // the download selected to delete
  content::DownloadItem* item_selected_remove_;
