...
#include "chrome/browser/ui/webui/md_downloads/md_downloads_speed_ui.h"
...
WebUIFactoryFunction GetWebUIFactoryFunction(WebUI* web_ui,
  ...
  //yana add 170411
  if (url.host() == chrome::kChromeUILastTabsListHost) {
    return &NewWebUI<LastTabsListUI>;
  }
  ...
  if (url.host() == chrome::kAboutNfsHost) {
    return &NewWebUI<AboutNFSUI>;
  }

  if (url.host() == chrome::kChromeUIAccountLoginHost) {
    return &NewWebUI<AccountLoginUI>;
  }

  if (url.host() == chrome::kFeedbackNfsHost) {
    return &NewWebUI<FeedbackNFSUI>;
  }

  if (url.host() == chrome::kUpdateNfsHost) {
    return &NewWebUI<UpdateNFSUI>;
  }
  ...
  if (url.host() == chrome::kChromeUIDownloadsHost)
    return &NewWebUI<MdDownloadsSpeedUI>;
  // Downloads new uses the built-in download manager.
  if (url.host() == chrome::kChromeUIDownloadsNewHost)
    return &NewWebUI<MdDownloadsSpeedNewUI>;
  ...
  if (url.host() ==chrome::kThemeGalleryHost)
    return &NewWebUI<ThemeGalleryUI>;

  if (url.host() ==chrome::kAppManagerHost)
    return &NewWebUI<AppManagerUI>;
...
base::RefCountedMemory* ChromeWebUIControllerFactory::GetFaviconResourceBytes(
  ...
  if (page_url.host() == chrome::kChromeUINewTabHost)
    return NewTabUI::GetFaviconResourceBytes(scale_factor);

  if (page_url.host() == chrome::kChromeUIHistoryFrameHost)//kChromeUIHistoryHost yana 161123
  ...
  if (page_url.host() == chrome::kChromeUIDownloadsHost)
    return MdDownloadsSpeedUI::GetFaviconResourceBytes(scale_factor);

  // Android uses the native download manager.
  if (page_url.host() == chrome::kChromeUIDownloadsNewHost)
    return MdDownloadsSpeedNewUI::GetFaviconResourceBytes(scale_factor);
