...
SearchEngineManagerHandler::CreateDictionaryForEngine(int index,
  ...
  //   dict->SetString("iconURL", icon_url.spec());
     GURL icon_url = template_url->favicon_url();
     base::string16  search_keyword= table_model->GetText(
    index, IDS_SEARCH_ENGINES_EDITOR_KEYWORD_COLUMN);
  std::string new_search_keyword(search_keyword.begin(), search_keyword.end());
  GURL  google_icon_url ("nfsbrowser://resources/images/search/search-google.png");
  GURL  bing_icon_url ("nfsbrowser://resources/images/search/search-bing.png");
  GURL  baidu_icon_url ("nfsbrowser://resources/images/search/search-baidu.png");
  GURL  sougou_icon_url ("nfsbrowser://resources/images/search/search-sogou.png");
 if (icon_url.is_valid()){
 if(new_search_keyword == "baidu.com") { // baidu
      dict->SetString("iconURL", baidu_icon_url.spec());
  } else if(new_search_keyword == "sogou.com") { // sougou
      dict->SetString("iconURL", sougou_icon_url.spec());
  } else if (new_search_keyword.find("google.com") != std::string::npos) { // google
      dict->SetString("iconURL", google_icon_url.spec());
  } else if(new_search_keyword == "bing.com") { // bing
      dict->SetString("iconURL", bing_icon_url.spec());
  } 
}
