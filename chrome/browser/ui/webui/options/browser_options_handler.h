...
#include "base/memory/scoped_vector.h"  //add by wdg
...
#include "ui/base/accelerators/accelerator.h"   //add by wdg
...
class BrowserOptionsHandler
 ...
 private:
  BrowsingDataRemover* remover_;   ///add by wdg
  ScopedVector< browsing_data::BrowsingDataCounter> counters_;
  base::DictionaryValue *roots_default;
  // content::NotificationObserver implementation.
  void InitShortCuts(bool force); //wdg
  void LoadShortCuts(const base::ListValue* value); //wdg add
  void  SaveShortCuts(const base::ListValue* value) ; //wdg
   void ResetShortCuts(const base::ListValue* value); //wdg add
  void OnPageOpened(const base::ListValue* value); //shortcuttable
  bool  GetAccelerator(int cmd_id, int index,ui::Accelerator* accelerator) const;  //shortcuttable
  int    GetAcceleratorCount(int cmd_id); //shortcuttable
   std::map<ui::Accelerator, int> accelerator_table_; //wdg
 //add by wdg
  ...
  void UpdateCounterText(std::unique_ptr<browsing_data::BrowsingDataCounter::Result> result);
  ...
  void HandleSelectCacheLocation(const base::ListValue* args);
  void HandleResetCacheLocation(const base::ListValue* args);
  ...
  //Callback  for  the "defaultTabDirection" message.
 // This is called if the
  // user changes the default close tab direction. |args| is an array that contains
  // one item, the font size as a numeric value.

void HandleDefaultOpenTabDirection(const base::ListValue* args);

void HandleSelectLanguage(const base::ListValue* args);

void HandleDefaultTabDirection(const base::ListValue* args);
...
//set up the close tab direction

  void  SetupTabPageSettings();
  void  SetupOpenTabPageDirection();
  // Setup the font size selector control.
  void SetupFontSizeSelector();

  void SetupLanguageSelection();
  ...
  scoped_refptr<ui::SelectFileDialog> cache_select_folder_dialog_;  //add by wdg
  base::FilePath   old_cache_path;  //add by wdg
