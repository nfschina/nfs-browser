...
class BrowsingHistoryHandler : public content::WebUIMessageHandler,
 ...
 public:
  ...
  // Handler for "addContextMenu" message.
  void HandleAddContextMenu(const base::ListValue* args) ;

  // Handler for "addBookmark" message.
  void HandleAddBookmark(const base::ListValue* args);

  // Handler for "removeBookmark" message.
  void HandleRemoveBookmark(const base::ListValue* args);

  // 查询所有活跃日期
  void HandleQueryActiveDates(const base::ListValue* args);

  // 查询之后调js的处理
  void QueryActiveDateComplete(std::vector<int>* results);
 ...
 private:
  ...
  // Sets the query options for a Day-wide query, |offset| day ago.
  void SetQueryTimeInDays(int offset, history::QueryOptions* options);
