...
using blink::WebContextMenuData;//yana add 161216
...
void BrowsingHistoryHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback("clearBrowsingData",
      base::Bind(&BrowsingHistoryHandler::HandleClearBrowsingData,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback("addContextMenu",//yana add 161215
      base::Bind(&BrowsingHistoryHandler::HandleAddContextMenu,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback("addBookmark",//yana add 161019
      base::Bind(&BrowsingHistoryHandler::HandleAddBookmark,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback("removeBookmark",
      base::Bind(&BrowsingHistoryHandler::HandleRemoveBookmark,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback("queryActiveDates",
      base::Bind(&BrowsingHistoryHandler::HandleQueryActiveDates,
                 base::Unretained(this)));
}
...
void BrowsingHistoryHandler::HandleQueryHistory(const base::ListValue* args) {
  ...
  else if (range == BrowsingHistoryHandler::DAY)//yana add 160907
    SetQueryTimeInDays(offset, &options);
...
//yana add 161215
void BrowsingHistoryHandler::HandleAddContextMenu(const base::ListValue* args) 
{
    base::string16 url, title, selectionText;
    int x,y;
    ...
    if (!args->GetInteger(2, &x)) {
      NOTREACHED();
      return;
    }
    if (!args->GetInteger(3, &y)) {
      NOTREACHED();
      return;
    }

    if (!args->GetString(4, &selectionText)) {
      NOTREACHED();
      return;
    }
    content::ContextMenuParams params;
    content::WebContentsImpl* web_contents = static_cast<content::WebContentsImpl*> (web_ui()->GetWebContents());
    content::RenderFrameHostImpl* main_frame =  web_contents->GetMainFrame();
    params.x =x,
    params.y =y;
    params.custom_context.is_pepper_menu = true;
    params.page_url = web_contents->GetURL();
    if(!selectionText.empty()){
      params.selection_text = selectionText;
    }else{
      params.link_url = GURL(url);
      params.unfiltered_link_url = GURL(url);
      params.link_text = title;
    }
    params.edit_flags = WebContextMenuData::CanCopy;
    main_frame->OnContextMenuPublic(params);
}
//yana add 161019
void BrowsingHistoryHandler::HandleAddBookmark(const base::ListValue* args) {
  base::string16 url,title;
  if (!args->GetString(0, &url)) {
    NOTREACHED();
    return;
  }
  ...
  bookmarks::AddToBookmarkBarIfNotBookmarked(model, GURL(url), title);
}
...
//yana add 160908
void BrowsingHistoryHandler::SetQueryTimeInDays(
  ...
  options->end_time = midnight -
      base::TimeDelta::FromDays(1 * offset);
  options->begin_time = midnight -
      base::TimeDelta::FromDays(1 * (offset + 1));
}
...
void BrowsingHistoryHandler::HandleQueryActiveDates(const base::ListValue* args) {
  Profile* profile = Profile::FromWebUI(web_ui());
  history::HistoryService* hs = HistoryServiceFactory::GetForProfile(
      profile, ServiceAccessType::EXPLICIT_ACCESS);
  ...
  hs->QueryActiveHistoryDate(base::Bind(&BrowsingHistoryHandler::QueryActiveDateComplete, base::Unretained(this)),
                     &query_task_tracker_);
}

void BrowsingHistoryHandler::QueryActiveDateComplete(std::vector<int>* results) {
  DCHECK(results);

  // call js to set the value
  std::unique_ptr<base::ListValue> active_date_list(new base::ListValue);

  for (std::vector<int>::reverse_iterator it = results->rbegin(); it != results->rend(); ++it) {
    char date_in_string[16];
    sprintf(date_in_string, "%d-%d-%d", (*it)/10000, ((*it) % 10000)/100, (*it)%100);
    active_date_list->AppendString(std::string(date_in_string));
  } 
 
   web_ui()->CallJavascriptFunctionUnsafe("historyDateResult", *active_date_list);  
}
