...
const int kMinWindowPadding = 30;
...
void ShowImportDialog(Browser* browser) {
  ShowImportSelectDialog(
      browser->window()->GetNativeWindow(),
      l10n_util::GetStringUTF16(IDS_CDOS_IMPORT_DIALOG_TITLE_FIRST),
      500);
  browser->profile()->GetPrefs()->SetBoolean(
      prefs::kShouldShowImportDialog, false);
}

gfx::Rect GetProperBounds(Profile* profile, gfx::Rect old_bounds) {
  ...
  if (browser) {
    ...
    int x = bounds.x() + std::max((bounds.width() - old_bounds.width()) / 2 , kMinWindowPadding);
    int y = bounds.y() + std::max((bounds.height() - old_bounds.height()) / 2, kMinWindowPadding);
    new_bounds.set_x(x);
    new_bounds.set_y(y);
  }
  return new_bounds;
...
// static
Browser::CreateParams Browser::CreateParams::CreateForSimpleWeb(
    Profile* profile) {
  // huk   test (后面应该还要改一下参数，如果有必要的话)
  CreateParams params(TYPE_SIMPLE_WEB, profile);
  params.simple_web_type = SIMPLE_NORMAL;
  params.trusted_source = true;
  return params;
}

Browser::CreateParams Browser::CreateParams::CreateForDownloadManager(
    Profile* profile) {
  CreateParams params(TYPE_SIMPLE_WEB, profile);
  params.simple_web_type = DOWNLOAD_MANAGER;
  gfx::Rect window_bounds = gfx::Rect(600, 300, 640, 480);
  params.initial_bounds = GetProperBounds(profile, window_bounds);
  params.trusted_source = true;
  return params;
}

Browser::CreateParams Browser::CreateParams::CreateForNewDownloadTask(
    Profile* profile) {
  CreateParams params(TYPE_SIMPLE_WEB, profile);
  params.simple_web_type = DOWNLOAD_NEW_TASK;
  gfx::Rect window_bounds = gfx::Rect(650, 350, 480, 250);
  params.initial_bounds = GetProperBounds(profile, window_bounds);
  params.trusted_source = true;
  return params;
}

Browser::CreateParams Browser::CreateParams::CreateForWebVedio(
    Browser* source) {
  CreateParams params(TYPE_SIMPLE_WEB, source->profile());
  params.simple_web_type = WEB_VEDIO;
  gfx::Rect window_bounds = gfx::Rect(600, 300, 640, 480);
  params.initial_bounds = GetProperBounds(source->profile(), window_bounds);
  params.trusted_source = true;
...
Browser::Browser(const CreateParams& params)
    : extension_registry_observer_(this),
      type_(params.type),
      simple_web_type_(params.simple_web_type),
...
Browser::~Browser() {
 ...
 if (profile_->GetProfileType() == Profile::REGULAR_PROFILE&&chrome::GetBrowserCount(profile_) == 0){
           ClearBrowserData();
    }
...
void Browser::ClearBrowserData()
{
  int remove_mask  = 0;
  int  origin_mask  = 0;
  int site_data_mask = BrowsingDataRemover::REMOVE_SITE_DATA;
  PrefService* prefs = profile_->GetPrefs();
   if( prefs->GetBoolean(prefs::kCloseAutoClearBrowserData))
   {
  ...
  profiles::RemoveBrowsingDataForProfile_RemoveMask(profile_->GetPath(),period_selected,remove_mask,origin_mask);
 }
}
///////////////////////////////////////////////////////////////////////////////
// Getters & Setters
...
//  if (!CanCloseWithInProgressDownloads()) //add by luyue
//    return false;   //add by luyue
...
//add by luyue
int Browser::GetWithInProgressDownloads() {
  ...
  if (!g_browser_process->profile_manager())
    return 0;
  return DownloadService::NonMaliciousDownloadCountAllProfiles();
}
//add end
...
void Browser::OnWindowDidShow() {
  ...
  content::NotificationService::current()->Notify(
      chrome::NOTIFICATION_BROWSER_WINDOW_SHOWN, content::Source<Browser>(this),
      content::NotificationService::NoDetails());
  ...
  // if (profile()->GetPrefs()->GetBoolean(prefs::kShouldShowImportDialog)) {
  //   content::BrowserThread::PostDelayedTask(
  //       content::BrowserThread::UI, FROM_HERE,
  //       base::Bind(&ShowImportDialog, this),
  ...
  NotificationServiceFactory::GetForProfile(profile());
...
void Browser::PurgeMemoryAndDiscardTab() {
  memory::TabManager::PurgeMemoryNfs();
}
///////////////////////////////////////////////////////////////////////////////
...
void Browser::AddFavorite(const base::string16& url, const base::string16& title) {
  chrome::BookmarkCurrentPageIgnoringExtensionOverrides(this);
}
...
bool Browser::SupportsWindowFeatureImpl(WindowFeature feature,
  ...
  unsigned int features = FEATURE_INFOBAR | FEATURE_DOWNLOADSHELF | FEATURE_BOOKMARKBAR;
  ...
  if (!hide_ui_for_fullscreen) {
    ...
    if (is_type_simple_web())
      features |= FEATURE_TABSTRIP;
  }

  // simple web
  if (is_type_simple_web())  {
    features &= ~FEATURE_INFOBAR;
...
void Browser::UpdateBookmarkBarState(BookmarkBarStateChangeReason reason) {
  ...
  // huk
  BrowserView* view = BrowserView::GetBrowserViewForBrowser(this);
...
void Browser::RecordErrorPageInfo(const std::string& url, const std::string& detail) {
}

void Browser::OpenSearchContent(const base::string16& search_text) {
  TemplateURLService* template_url_service = TemplateURLServiceFactory::GetForProfile(profile_);
  const TemplateURL* provider_url =  template_url_service->GetDefaultSearchProvider();

  const TemplateURLRef& search_url = provider_url->url_ref();
  DCHECK(search_url.SupportsReplacement(template_url_service->search_terms_data()));
  std::unique_ptr<TemplateURLRef::SearchTermsArgs> search_terms_args(new TemplateURLRef::SearchTermsArgs(search_text));
  ...
  params.disposition = WindowOpenDisposition::NEW_BACKGROUND_TAB;
  params.source_contents = tab_strip_model()->GetActiveWebContents();
