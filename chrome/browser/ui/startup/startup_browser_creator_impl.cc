...
#include "chrome/browser/ui/views/importer/import_dialog_view.h"//wdq
#include "chrome/browser/first_run/first_run.h" //wdq
...
const struct PreBookmarks{
  const char* url;
  const char* url_favicon;
  int favicon_id;
} kPreBookmarks[] = {
  {"http://www.qq.com/", "http://www.qq.com/favicon.ico", IDD_BOOKMARK_BAR_FAVICON_TENGXUN},
  {"http://www.jd.com/", "http://www.jd.com/favicon.ico", IDD_BOOKMARK_BAR_FAVICON_JINGDONG},
  {"http://www.taobao.com/", "http://www.taobao.com/favicon.ico", IDD_BOOKMARK_BAR_FAVICON_TAOBAO},
  {"http://www.sina.com.cn/", "http://www.sina.com.cn/favicon.ico", IDD_BOOKMARK_BAR_FAVICON_XINLANG},
  {"http://www.xiami.com/", "http://www.xiami.com/favicon.ico", IDD_BOOKMARK_BAR_FAVICON_XIAMI},
};
int64_t  Current_Cache_Size = 0;
int kSouldShowWelcomeVersion = 1;
#if defined(OS_WIN)
...
void DelayShow(Browser* browser) {
  browser->window()->Show();
...
void ComputeCacheSize(base::FilePath path){
  Current_Cache_Size  = 100;
  Current_Cache_Size = base::ComputeDirectorySize(path);
}

void DeleteTempUpdatePackage() {
...
#if defined(OS_WIN)
  base::FilePath update_package = tmp_path.Append(L"temp_update_package.exe");
#elif defined(OS_LINUX)
  base::FilePath update_package = tmp_path.Append("temp_update_package.deb");
#endif

  if (!base::PathExists(update_package)) {
    return;
  }

  base::DeleteFile(update_package, true);
}
...
void LoadPreFavicons(favicon::FaviconService* favicon_service) {
  for (size_t i = 0; i < arraysize(kPreBookmarks); ++i) {
    gfx::ImageSkia* favicon = GetImageSkiaNamed(kPreBookmarks[i].favicon_id);
    gfx::Image image(*favicon);

    favicon_service->SetFavicons(GURL(kPreBookmarks[i].url), GURL(kPreBookmarks[i].url_favicon), favicon_base::FAVICON, image);
  }
}
...
bool StartupBrowserCreatorImpl::Launch(Profile* profile,
  ...
  // Delete update software package
  content::BrowserThread::PostDelayedTask(
      content::BrowserThread::FILE,
      FROM_HERE, base::Bind(&DeleteTempUpdatePackage),
...
void StartupBrowserCreatorImpl::AddInfoBarsIfNecessary(
    ...
    //if(ImportDialogView::ShouldShowImportDialog()) {
   //       chrome::ImportFavoritesInfoBarDelegate::Create(InfoBarService::FromWebContents(
    //    browser->tab_strip_model()->GetActiveWebContents()), profile_);
   // }

  //std::string version = "2.2.2.2";
  //chrome::CheckUpdateInfoBarDelegate::Create(InfoBarService::FromWebContents(
  //      browser->tab_strip_model()->GetActiveWebContents()), profile_, std::string(), std::string(), std::string());

  if (!browser || !profile_ || browser->tab_strip_model()->count() == 0)
    return;

  if (profile_->GetPrefs()->GetBoolean(prefs::kShouldShowImportDialog))
    ImportDialogView::ShouldShowImportDialog(browser, profile_);

  if (HasPendingUncleanExit(browser->profile())) {
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableCrashedInfoBar)) {
      SessionRestore::RestoreSessionAfterCrash(browser);
    } else if(!SessionCrashedBubble::Show(browser)) {
  ...
  if (is_process_startup == chrome::startup::IS_PROCESS_STARTUP &&
    ...
    #ifdef huk  //we don't wanna the infobars
    ...
    if (!command_line_.HasSwitch(switches::kNoDefaultBrowserCheck)) {
      ...
      // changed by wangpp
      // 判断是否弹出“是否设置默认浏览器”的提示框
      // 1. 首次启动
      // 2. “启动时检查默认浏览器”选项被勾选
      PrefService* const local_state = g_browser_process->local_state();
      if (local_state &&
          (is_first_run_ ||
           local_state->GetBoolean(prefs::kDefaultBrowserCheck))) {
...
void StartupBrowserCreatorImpl::ProcessLaunchURLs(
    ...
    const std::vector<GURL>& urls_to_open) {
  if (profile_) {
    if(profile_->GetPrefs()->GetInteger(prefs::kBrowserShouldShowWelcome) < kSouldShowWelcomeVersion) {
      first_run::SetShouldShowWelcomePage();
      profile_->GetPrefs()->SetInteger(prefs::kBrowserShouldShowWelcome, kSouldShowWelcomeVersion);
    }
  }
...
Browser* StartupBrowserCreatorImpl::ProcessSpecifiedURLs(
  ...
  GURL url;
  if (chrome::IsSetHomePage(profile_, &url)) {
    std::vector<GURL> url_list;
    url_list.push_back(url);
    UrlsToTabs(url_list, &tabs);
  } else {
    if (!urls_to_open.empty()) {
      ...
      AddStartupURLs(&urls);
      UrlsToTabs(urls, &tabs);
    } else if(pref.type == SessionStartupPref::LAST_URLS_LIST){//yana add 170414
      std::vector<GURL> url_list;
      url_list.push_back(GURL(chrome::kChromeUILastTabsListURL));
      UrlsToTabs(url_list, &tabs);
