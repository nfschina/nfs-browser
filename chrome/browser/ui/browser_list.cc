...
Browser* BrowserList::GetLastActive() const {
  // 去除掉我们加入的simple web browser 类型的
  const_reverse_iterator r_iter = last_active_browsers_.rbegin();
  while  (r_iter != last_active_browsers_.rend()) {
    if (*r_iter != NULL && (*r_iter)->type() != Browser::Type::TYPE_SIMPLE_WEB)   {
      return *r_iter;
    }
