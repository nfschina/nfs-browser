...
FeedbackMenuModel::FeedbackMenuModel(ui::SimpleMenuModel::Delegate* delegate,
                               Browser* browser)
    : SimpleMenuModel(delegate) {
  Build(browser);
}

FeedbackMenuModel::~FeedbackMenuModel() {}
...
// - Developer tools.
// - Option to enable profiling.
void FeedbackMenuModel::Build(Browser* browser) {
  // huk
  // This only shows up in the public release version.

  AddItemWithStringId(IDC_CHECK_VERSION_DIALOG, IDS_CHECK_VERSION);

  AddItemWithStringId(IDC_FEEDBACK_TITLE, IDS_FEEDBACK_TITLE);
  AddItemWithStringId(IDC_OFFICIAL_FORUM, IDS_OFFICIAL_FORUM);//yana
...
void AppMenuModel::Build() {
  if (extensions::FeatureSwitch::extension_action_redesign()->IsEnabled() &&
        browser_->profile()->GetPrefs()->GetBoolean(prefs::kShowToolbar))
  ...
  if (AddGlobalErrorMenuItems() ||
   ...
   AddItemWithStringId(IDC_CLEAR_BROWSING_DATA, IDS_CLEAR_BROWSING_DATA);
    ...
    // AddItemWithStringId(IDC_CLEAR_HISTORY, IDS_CLEAR_HISTORY);
	  SetIcon(GetIndexOfCommandId(IDC_CLEAR_BROWSING_DATA),rb.GetNativeImageNamed(IDD_CLEAR_HISTORY));
    AddSeparator(ui::NORMAL_SEPARATOR);
   if (!browser_->profile()->IsGuestSession()) {
    bookmark_sub_menu_model_.reset(new BookmarkSubMenuModel(this, browser_));
    AddSubMenuWithStringId(IDC_BOOKMARKS_MENU, IDS_BOOKMARKS_MENU,
                           bookmark_sub_menu_model_.get());
    SetIcon(GetIndexOfCommandId(IDC_BOOKMARKS_MENU),rb.GetNativeImageNamed(IDD_BOOKMARKS_MENU));
  }
  AddItemWithStringId(IDC_SHOW_HISTORY, IDS_HISTORY_MENU);
  SetIcon(GetIndexOfCommandId(IDC_SHOW_HISTORY),rb.GetNativeImageNamed(IDD_SHOW_HISTORY));
   AddItemWithStringId(IDC_SHOW_DOWNLOADS, IDS_SHOW_DOWNLOADS);
  SetIcon(GetIndexOfCommandId(IDC_SHOW_DOWNLOADS),rb.GetNativeImageNamed(IDD_SHOW_DOWNLOADS));
  AddItemWithStringId(IDC_MY_APP, IDS_MY_APP);


  CreateZoomMenu();
  AddItemWithStringId(IDC_SAVE_PAGE, IDS_PAGE_SAVE_AS);

#if defined (OS_LINUX)
//  AddItemWithStringId(IDC_SAVE_PAGE_AS_PIC, IDS_PAGE_SAVE_AS_PIC);
#endif
  ...
  SetIcon(GetIndexOfCommandId(IDC_OPTIONS),rb.GetNativeImageNamed(IDD_OPTIONS));
  ...
  feedback_menu_model_.reset(new FeedbackMenuModel(this, browser_));
  AddSubMenuWithStringId(
      IDC_FEEDBACK_MENU, IDS_FEEDBACK_MENU, feedback_menu_model_.get());
...
// #endif
  ...
  if (!is_default_browser)  {
    AddItemWithStringId(IDC_SET_AS_DEFAULT_BROWSER, IDS_SET_AS_DEFAULT_BROWSER);
    SetIcon(GetIndexOfCommandId(IDC_SET_AS_DEFAULT_BROWSER),rb.GetNativeImageNamed(IDD_SET_AS_DEFAULT_BROWSER));
  } else {
    AddItemWithStringId(IDC_SET_AS_DEFAULT_BROWSER, IDS_ALREADY_IS_DEFAULT_BROWSER);
    SetIcon(GetIndexOfCommandId(IDC_SET_AS_DEFAULT_BROWSER),rb.GetNativeImageNamed(IDD_HAS_BEEN_SET_AS_DEFAULT));
  }

  browser_->command_controller()->command_updater()->
      UpdateCommandEnabled(IDC_SET_AS_DEFAULT_BROWSER, !is_default_browser);
  uma_action_recorded_ = false;
}
...
void AppMenuModel::CreateZoomMenu() {
  ...
  zoom_menu_item_model_->AddGroupItemWithStringId(IDC_ZOOM_PLUS,
                                                  IDS_ZOOM_PLUS2);
  //add by yana 160812
  zoom_menu_item_model_->AddItemWithImage(IDC_LOCK_OR_UNLOCK,
                                          IDD_UNLOCKED);
