...
#include "grit/webui_resources_nfs.h"
...
gfx::Path GetFillPath(float scale, const gfx::Size& size) {
  ...
  fill.moveTo(right-1, bottom);
  fill.lineTo(right-1, 1);
  fill.lineTo(1, 1);
  fill.lineTo(1, bottom);
  fill.close();
  return fill;
}
...
void Tab::ThrobberView::OnPaint(gfx::Canvas* canvas) {
  GURL url = owner_->data().url;
  if (owner_->data().url == GURL(chrome::kChromeUINewTabURL)) {
    canvas->DrawImageInt(ui::ResourceBundle::GetSharedInstance().
          GetImageNamed(IDR_CDOS_NTP_FAVICON_PNG).AsImageSkia(), 0, 0);
    return ;
  }
...
Tab::Tab(TabController* controller, gfx::AnimationContainer* container)
  ...
  close_button_->SetImage(views::Button::STATE_NORMAL,
                    rb.GetImageNamed(IDD_TAB_CLOSE_N).ToImageSkia());
  close_button_->SetImage(views::Button::STATE_HOVERED,
                    rb.GetImageNamed(IDD_TAB_CLOSE_H).ToImageSkia());
  close_button_->SetImage(views::Button::STATE_PRESSED,
                    rb.GetImageNamed(IDD_TAB_CLOSE_P).ToImageSkia());
#endif
  ...
  pref_service_ = g_browser_process->profile_manager()->GetPrimaryUserProfile()->GetPrefs();
}
...
void Tab::SetData(const TabRendererData& data) {
  ...
  base::string16 nfs = base::UTF8ToUTF16("nfsbrowser");
  if (!base::StartsWith(title, nfs, base::CompareCase::SENSITIVE))  {
    title_->SetText(title);
  }

  if (NeedToSetBold()) {
    title_->SetFontList(title_->font_list().Derive(0, gfx::Font::NORMAL, gfx::Font::Weight::BOLD));
  }
...
bool Tab::OnMousePressed(const ui::MouseEvent& event) {
  ...
  if (pref_service_->GetInteger(prefs::kEnableTabPreview) != 0)  {
    controller_->ShowPreViewOrNot(this, false);
  }
...
void Tab::OnMouseEntered(const ui::MouseEvent& event) {
  ...
  if (EnablePreview())  {
    controller_->ShowPreViewOrNot(this, true);
  }
}
...
void Tab::PaintTab(gfx::Canvas* canvas, const gfx::Path& clip) {
  ...
  const int y_offset = background_offset_.y();
  if (IsActive()) {
  ...
  // Draw the fill.
  {
    ...
    fill = GetFillPath(scale, size());
    {
      ...
      if (has_custom_image) {
        ...
        SkColor tab_blur_color =
                      tp->GetColor(ThemeProperties::COLOR_TAB_BLUR_INACTIVE);
        if (is_active) {
          tab_blur_color =
                      tp->GetColor(ThemeProperties::COLOR_TAB_BLUR_ACTIVE);
        }
        fill_canvas->DrawColor(tab_blur_color);
      } else {
        paint.setColor(
...
void Tab::PaintIcon(gfx::Canvas* canvas) {
  ...
  if (favicon_.isNull()) {
    ...
    if (!favicon_.isNull() &&
      ...
      bool noHSLShift = controller_->IsIncognito() &&
        data().url.SchemeIs(content::kChromeUIScheme) &&
        data().url.host() == chrome::kChromeUINewTabHost;
      if (!noHSLShift) {
...
void Tab::OnButtonColorMaybeChanged() {
  ...
  ui::ResourceBundle* rb = &ui::ResourceBundle::GetSharedInstance();
  bool dark = theme_provider->GetDisplayProperty(ThemeProperties::THEME_ICONS_DARK) ? true:false;
  int id_tab_close_n = dark ? IDD_TAB_CLOSE_N_DARK : IDD_TAB_CLOSE_N;
  int id_tab_close_h = dark ? IDD_TAB_CLOSE_H_DARK : IDD_TAB_CLOSE_H;
  int id_tab_close_p = dark ? IDD_TAB_CLOSE_P_DARK : IDD_TAB_CLOSE_P;
  close_button_->SetImage(views::Button::STATE_NORMAL,
                    rb->GetImageNamed(id_tab_close_n).ToImageSkia());
  close_button_->SetImage(views::Button::STATE_HOVERED,
                    rb->GetImageNamed(id_tab_close_h).ToImageSkia());
  close_button_->SetImage(views::Button::STATE_PRESSED,
                    rb->GetImageNamed(id_tab_close_p).ToImageSkia());
...
bool Tab::CloseOnDoubleClick() {
    // 考虑双击激活标签关闭
  if (pref_service_->GetBoolean(prefs::kDoubleClickToClose) && controller_->IsActiveTab(this))  {
    const int64_t kDoubleClickEcllipse = 300;
    static base::TimeTicks last_click = base::TimeTicks();
    int64_t elapsed_time = (base::TimeTicks::Now() - last_click).InMilliseconds();
    last_click = base::TimeTicks::Now();
    if (elapsed_time < kDoubleClickEcllipse)  {
      controller_->CloseTab(this, CLOSE_TAB_FROM_MOUSE);
      return true;
    }
  }
...
bool Tab::NeedToSetBold() const{
  // 考虑后台打开标签加粗显示的问题
  if (!pref_service_->GetBoolean(prefs::kBoldInBackground))  {
    return false;
  }
  return never_actived_;
}

void Tab::TabSelected() {
  never_actived_ =  false;
  title_->SetFontList(gfx::FontList());
}

bool Tab::EnablePreview() const{
  if (IsActive())  {
    return false;
  }
  const int indicator_left =
  ...
  if (pref_service_->GetInteger(prefs::kEnableTabPreview) == 1)  {
    return true;
  }

  if (pref_service_->GetInteger(prefs::kEnableTabPreview) == 2
        && indicator_left<=pref_service_->GetInteger(prefs::kTabMaxIndex)) {
    return true;
  }
  return false;
}
