...
const Browser* BrowserTabStripController::browser() {
  return browser_;
}
...
void BrowserTabStripController::CreateNewTab() {
  ...
  if (chrome::IsSetHomePage(profile(), &url)) {
    model_->delegate()->AddTabAt(url, -2, true);
    return ;
  }

  model_->delegate()->AddTabAt(GURL(), -2, true);
}
...
void BrowserTabStripController::ShowPreViewOrNot(int index, views::View* anchor, bool show) {
  WebContents* webcontents = model_->GetWebContentsAt(index);
  content::NotificationService::current()->Notify(
      show ? chrome::NOTIFICATION_SHOW_TAB_SCREEN_SHOT : chrome::NOTIFICATION_UNSHOW_TAB_SCREEN_SHOT,
      content::Source<WebContents>(webcontents),
      content::Details<views::View>(anchor));
}
