...
int GetNewTabButtonTopOffset() {
  ...
  const int kNewTabButtonBottomOffset = 0;
...
class NewTabButton : public views::ImageButton,
 ...
 private:
  ...
  void PaintTransparentLayer(views::Button::ButtonState state,
                             float scale,
                             const SkPath& fill,
                             gfx::Canvas* canvas) const;
...
void NewTabButton::OnPaint(gfx::Canvas* canvas) {
  ...
  const int visible_width = GetLayoutSize(NEW_TAB_BUTTON).width();
  ...
  fill.rCubicTo(0, 0.5 * scale, -0.25 * scale, scale, -scale, scale);
  #endif
  const float height = visible_height * scale;
  const float width = visible_width * scale;

  fill.moveTo(width -1, height);
  fill.lineTo(width -1, 1);
  fill.lineTo(1, 1);
  fill.lineTo(1, height);
  fill.close();
  PaintFill(pressed, scale, fill, canvas);

  if (tp && tp->HasCustomImage(IDR_THEME_TOOLBAR)) {
    PaintTransparentLayer(state(), scale, fill, canvas);
  }

  //cross fill
  gfx::Point cross_left(visible_width - 29, visible_height / 2);
  gfx::Point cross_right(visible_width - 11, visible_height / 2);
  gfx::Point cross_top(visible_width / 2, visible_height - 24);
  gfx::Point cross_bottom(visible_width / 2, visible_height - 6);
  //gfx::Point a(0, visible_height);
  //gfx::Point b(visible_width, visible_height);
  gfx::Point stroke_left(0, 1);
  gfx::Point stroke_right(width, 1);
  SkPaint paint_cross;
  const SkColor cross_color = tp->GetColor(ThemeProperties::COLOR_NEWTAB_BUTTON_CROSS);
  const float alpha_cross = SkColorGetA(cross_color);
  const SkAlpha path_alpha_cross = static_cast<SkAlpha>(
        std::round((pressed ? 0.875f : 0.609375f) * alpha_cross));
  paint_cross.setColor(SkColorSetA(cross_color, path_alpha_cross));
  canvas->DrawLine(cross_right, cross_left, paint_cross);
  canvas->DrawLine(cross_bottom, cross_top, paint_cross);
  //canvas->DrawLine(a, b, paint_cross);
  ...
  canvas->DrawLine(stroke_left, stroke_right, stroke_color);
...
void NewTabButton::GetBorderPath(float button_y,
  ...
  #endif
  //const int top = scale;
  const int bottom = GetLayoutSize(NEW_TAB_BUTTON).height()* scale;
  const int right = GetLayoutSize(NEW_TAB_BUTTON).width()* scale;
...
void NewTabButton::PaintFill(bool pressed,
  ...
  if (!pressed) {
    ...
    if (custom_image) {
      ...
      paint.setColor(tp->GetColor(ThemeProperties::COLOR_FRAME));
    }
    const SkColor stroke_color = tab_strip_->GetToolbarTopSeparatorColor();
...
void NewTabButton::PaintTransparentLayer(views::Button::ButtonState state,
                             float scale,
                             const SkPath& fill,
                             gfx::Canvas* canvas) const {
  gfx::ScopedCanvas scoped_canvas(canvas);
  ...
  const ui::ThemeProvider* tp = GetThemeProvider();
  const SkColor color_normal = tp->GetColor(
                  ThemeProperties::COLOR_NEW_TAB_BUTTON_BLUR_NORMAL);
  const SkColor color_hover = tp->GetColor(
                  ThemeProperties::COLOR_NEW_TAB_BUTTON_BLUR_HOVER);
  const SkColor color_pressed = tp->GetColor(
                  ThemeProperties::COLOR_NEW_TAB_BUTTON_BLUR_PRESS);
  if (state == views::Button::STATE_PRESSED) {
    paint.setColor(color_pressed);
  } else if (state == views::Button::STATE_HOVERED) {
    const SkAlpha hover_alpha = static_cast<SkAlpha>(
                    hover_animation().CurrentValueBetween(
                              static_cast<int>SkColorGetA(color_normal),
                              static_cast<int>SkColorGetA(color_hover)));
    paint.setColor(SkColorSetA(color_hover, hover_alpha));
  } else {
...
void TabStrip::SetTabData(int model_index, const TabRendererData& data) {
  ...
  // bool url_changed = tab->data().url != data.url;
  tab->SetData(data);
...
void TabStrip::SetSelection(const ui::ListSelectionModel& old_selection,
  ...
  // huk
  (tab_at(new_selection.active()))->TabSelected();
}
...
void TabStrip::PaintChildren(const ui::PaintContext& context) {
  if(!controller_->browser()->is_type_simple_web()) {
...
void TabStrip::ShowPreViewOrNot(Tab* tab, bool show) {
  DCHECK(controller_);
  int model_index = GetModelIndexOfTab(tab);
  if (IsValidModelIndex(model_index))
    controller_->ShowPreViewOrNot(model_index, tab, show);
}

void TabStrip::ShowContextMenuForView(views::View* source,
                            const gfx::Point& p,
                            ui::MenuSourceType source_type) {

  LOG(INFO) << "tabstrip  context menu";
}
