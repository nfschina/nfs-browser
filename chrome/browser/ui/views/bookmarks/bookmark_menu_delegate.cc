...
bool BookmarkExist(const bookmarks::BookmarkNode* parent,
                   const GURL& url) {
  int count = parent->child_count();
  for (int i = 0; i < count; ++i) {
    const bookmarks::BookmarkNode* child_node = parent->GetChild(i);
...
base::string16 BookmarkMenuDelegate::GetTooltipText(
  ...
  NonBookmarkNodeMap::const_iterator it = none_bookmark_node_map_.find(id);
  if (it != none_bookmark_node_map_.end()) {
...
void BookmarkMenuDelegate::ExecuteCommand(int id, int mouse_event_flags) {
  NonBookmarkNodeMap::const_iterator it = none_bookmark_node_map_.find(id);
  if (it != none_bookmark_node_map_.end()) {
    const BookmarkNode* parent = none_bookmark_node_map_[id];
    if(!parent) {
      return;
    }
    GURL url;
    ...
    if (!BookmarkExist(parent, url)) {
      GetBookmarkModel()->AddURL(parent, parent->child_count(), title, url);
    }
    return;
  }
  ...
  WindowOpenDisposition disposition_from_event_flags = ui::DispositionFromEventFlags(mouse_event_flags);
...
void BookmarkMenuDelegate::WillRemoveBookmarks(
  ...
  for (std::vector<const BookmarkNode*>::const_iterator i(bookmarks.begin());
    ...
    if (node_to_menu != node_to_menu_map_.end()) {
      ...
      none_bookmark_node_map_.erase(menu->GetCommand());
    }
  }
  ...
  for (NodeToMenuMap::iterator i(node_to_menu_map_.begin());
    ...
    if (ancestor_removed) {
      ...
      none_bookmark_node_map_.erase(i->second->GetCommand());
...
bool BookmarkMenuDelegate::ShouldCloseOnRemove(const BookmarkNode* node) const {
  ...
  return is_child_of_bookmark_bar;
}
...
void BookmarkMenuDelegate::BuildMenu(const BookmarkNode* parent,
  ...
  menu->AppendMenuItemWithLabel(id,
      l10n_util::GetStringUTF16(IDS_ADD_PAGE_IN_CURRENT_FOLDER));
  AddMenuToNonBookmarkNodeMap(id, parent);
...
void BookmarkMenuDelegate::AddMenuToNonBookmarkNodeMap(
    int id, const BookmarkNode* parent) {
  none_bookmark_node_map_[id] = parent;
}
