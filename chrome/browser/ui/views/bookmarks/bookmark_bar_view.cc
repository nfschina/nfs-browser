...
#include "chrome/grit/theme_resources_nfs.h"
...
//[zhangyq] Adjust menu button marging.
static const int kMargingAdjustment = 3;

// Number of pixels the attached bookmark bar overlaps with the toolbar.
static const int kToolbarAttachedBookmarkBarOverlap = 3 + kMargingAdjustment;

// Margins around the content.
static const int kDetachedTopMargin = 1;  // When attached, we use 0 and let the
                                          // toolbar above serve as the margin.
static const int kBottomMargin = 2 + kMargingAdjustment;
...
static const SkColor kBookmarkButtonBorderColor = SkColorSetRGB(0xad, 0xad, 0xad);
...
class BookmarkButtonHighlightLayerDelegate : public views::BasePaintedLayerDelegate {
 public:
  BookmarkButtonHighlightLayerDelegate(SkColor color, gfx::Size size)
    : BasePaintedLayerDelegate(color), size_(size) {}
  ~BookmarkButtonHighlightLayerDelegate() override {}
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(BookmarkButtonHighlightLayerDelegate);
};
...
class BookmarkButtonBase : public views::LabelButton {
 public:
  ...
  std::unique_ptr<views::InkDropHighlight> CreateInkDropHighlight()
    ...
    //[zhangyq] change bookmark button ink drop highlight.
    /*
    ...
    */
      return base::MakeUnique<views::InkDropHighlight>(
            ...
            base::WrapUnique(new BookmarkButtonHighlightLayerDelegate(
                kBookmarkButtonBorderColor, size())));
  }
...
BookmarkBarView::BookmarkBarView(Browser* browser, BrowserView* browser_view)
    : page_navigator_(nullptr),
      ...
      bookmark_manager_page_shortcut_(nullptr),
...
int BookmarkBarView::GetPreferredHeight() const {
  ...
  return height + kMargingAdjustment;
}
...
#else
  if (bookmark_manager_page_shortcut_->visible()) {
    gfx::Size size = bookmark_manager_page_shortcut_->GetPreferredSize();
    width += size.width() + kButtonPadding;
  }
#endif
...
void BookmarkBarView::Layout() {
  ...
  if (browser_view_->IsFullscreen()) {
    // huk plays such a trick
    for (int i = 0; i < child_count(); ++i) {
      views::View* view = child_at(i);
      if (view->visible())
        view->SetSize(gfx::Size(0, 0));
    }
    return;
  }
...
#else
  gfx::Size overflow_pref = overflow_button_->GetPreferredSize();
  gfx::Size bookmark_manager_page_shortcut_pref = bookmark_manager_page_shortcut_->visible() ?
      bookmark_manager_page_shortcut_->GetPreferredSize() : gfx::Size();

  int max_x = width - overflow_pref.width() - kButtonPadding;

  if (bookmark_manager_page_shortcut_->visible()) {
    bookmark_manager_page_shortcut_->SetBounds(x, y, bookmark_manager_page_shortcut_pref.width(),
                                               height);
    x += bookmark_manager_page_shortcut_pref.width() + kButtonPadding;
  }
#endif
...
void BookmarkBarView::AnimationProgressed(const gfx::Animation* animation) {
  ...
  if (GetThemeProvider()->HasCustomImage(IDR_THEME_TOOLBAR)) {
    views::NonClientView* non_client_view = browser_view_->GetWidget()->non_client_view();
    if (non_client_view) {
...
void BookmarkBarView::OnImportBookmarks() {
  ...
  browser_->command_controller()->command_updater()->ExecuteCommand(IDC_IMPORT_BOOKMARK);
}
...
void BookmarkBarView::OnMenuButtonClicked(views::MenuButton* view,
  ...
  if (event && (event->flags() & ui::EF_MIDDLE_MOUSE_BUTTON) != 0) {
    ...
    // huk
    bool is_ntp = browser_->tab_strip_model()->GetActiveWebContents()->GetURL().spec() ==  chrome::kChromeUINewTabURL;
    if (!is_ntp && disposition_from_event_flags == WindowOpenDisposition::CURRENT_TAB)  {
...
void BookmarkBarView::ShowContextMenuForView(views::View* source,
  ...
  } else if (source != this && source != bookmark_manager_page_shortcut_) {
...
#else
  bookmark_manager_page_shortcut_ = CreateBookmarkManagerPageShortcutButton();
  AddChildView(bookmark_manager_page_shortcut_);
#endif
...
#else
  bookmark_manager_page_shortcut_->SetVisible(
      chrome::ShouldShowAppsShortcutInBookmarkBar(browser_->profile()));
#endif
...
#else
views::LabelButton* BookmarkBarView::CreateBookmarkManagerPageShortcutButton() {
  views::LabelButton* button = new ShortcutButton(
      this, l10n_util::GetStringUTF16(IDS_BOOKMARK_BAR_BOOKMARK_MANAGER_SHORTCUT_NAME));
  button->SetTooltipText(l10n_util::GetStringUTF16(
      IDS_BOOKMARK_BAR_BOOKMARK_MANAGER_SHORTCUT_TOOLTIP));
  button->set_id(VIEW_ID_BOOKMARK_BAR_ELEMENT);
  button->SetImage(views::Button::STATE_NORMAL,
                   *GetImageSkiaNamed(IDR_BOOKMARK_BAR_BOOKMARK_MANAGER_SHORTCUT));
  button->set_context_menu_controller(this);
  button->set_tag(kAppsShortcutButtonTag);
  return button;
}
  ...
  *location = DropLocation();
  ...
  if (!found) {
    if (overflow_button_->visible()) {
      ...
      location->index = GetFirstHiddenNodeIndex();
      //return;  //[zhangyq]Fix bug:0031354 无法在书签栏中通过拖拽将文件的排序变为最后一个
    }
  }
...
#else
    const BookmarkNode* parent = model_->bookmark_bar_node()->GetChild(location->index);
#endif
...
#else
  if (bookmark_manager_page_shortcut_->visible())
    bookmark_manager_page_shortcut_->SetEnabledTextColors(color);
#endif
...
bool BookmarkBarView::UpdateOtherAndManagedButtonsVisibility() {
  ...
  return update_managed || update_supervised;
#endif
}
...
void BookmarkBarView::OnAppsPageShortcutVisibilityPrefChanged() {
  ...
  LayoutAndPaint();
#else
  DCHECK(bookmark_manager_page_shortcut_);
  // Only perform layout if required.
  bool visible =
      chrome::ShouldShowAppsShortcutInBookmarkBar(browser_->profile());
  if (bookmark_manager_page_shortcut_->visible() == visible)
    return;
  bookmark_manager_page_shortcut_->SetVisible(visible);
  UpdateBookmarksSeparatorVisibility();
  LayoutAndPaint();
#endif
}
...
void BookmarkBarView::OnMouseEntered(const ui::MouseEvent& event) {
  if (browser_view_->IsFullscreen())  {
    browser_view_->UpdateExclusiveAccessExitBubbleContent(GURL(),
