...
const int kVertical_padding = 7;
const int kVertical_padding_reload = 0;
const SkColor kBackgroundColorDefault = SkColorSetA(SK_ColorWHITE, 204);
const SkColor kBackgroundColorInputState = SK_ColorWHITE;
...
LocationBarView::LocationBarView(Browser* browser,
    ...
    : LocationBar(profile),
      ...
      background_color_(kBackgroundColorDefault) {
  ...
  if (profile->GetPrefs()->FindPreference(prefs::kAddressCompatibleMode)) {
    registrar_.Add(
        prefs::kAddressCompatibleMode,
        base::Bind(&LocationBarView::OnPreferenceChanged,
                   base::Unretained(this),
                   registrar_.prefs()));
  }
}
...
LocationBarView::~LocationBarView() {
  ...
  if (profile()->GetPrefs()->FindPreference(prefs::kAddressCompatibleMode)) {
    registrar_.Remove(prefs::kAddressCompatibleMode);
  }
}
...
void LocationBarView::Init() {
  ...
  compatible_mode_button_ = new CompatibleModeButton(browser_);
  bool mode = profile()->GetPrefs()->GetBoolean(prefs::kAddressCompatibleMode);
  compatible_mode_button_->SetVisible(mode);
  AddChildView(compatible_mode_button_);

  star_view_ = new StarView(command_updater(), browser_);
  star_view_->SetVisible(false);
  AddChildView(star_view_);

  qr_ = new QrButton(browser_, delegate_);
  qr_->SetVisible(false);
  AddChildView(qr_);

  dropdown_view_ = new DropDownView(browser_);
  dropdown_view_->SetVisible(true);
  AddChildView(dropdown_view_);
  
  reload_ = new ReloadButton(profile(), command_updater());
...
SkColor LocationBarView::GetColor(
  ...
  switch (kind) {
    ...
    case SECURITY_CHIP_TEXT:
      return GetSecureTextColor(GetToolbarModel()->GetSecurityLevel(true));
  }
  NOTREACHED();
...
SkColor LocationBarView::GetSecureTextColor(
  ...
  if (!color_utils::IsDark(GetColor(BACKGROUND))) {
    if ((security_level == security_state::SecurityStateModel::EV_SECURE) ||
      ...
      text_color = gfx::kGoogleGreen300;
...
void LocationBarView::Layout() {
  ...
  if (is_popup_mode_)  {
    ...
    dropdown_view_->SetVisible(false);
    qr_->SetVisible(false);
  }
  ...
  if (reload_->visible()) {
    // trailing_decorations.AddDecoration(kVertical_padding_reload, location_height,
    //                                     false , 0, 4, 0, reload_);
    trailing_decorations.AddDecoration(kVertical_padding_reload, location_height + 4,
                                            reload_);
  }

  if (dropdown_view_->visible()) {
    trailing_decorations.AddDecoration(kVertical_padding, location_height,
                                        false , 0, -1, 0, dropdown_view_);
  }
  ...
  if (compatible_mode_button_->visible()) {
    trailing_decorations.AddDecoration(kVertical_padding, location_height,
                                        compatible_mode_button_);
  }
  ...
  // huk disable the hint
  if (false && !keyword.empty() && omnibox_view_->model()->is_keyword_hint() &&
...
void LocationBarView::Update(const WebContents* contents) {
  ...
  if (reload_) {
    ...
    // set the reload button to navigate, if user is typing
    if (GetToolbarModel()->input_in_progress())  {
      reload_->ChangeMode(ReloadButton::Mode::MODE_NAVIGATE, true);
    }
  }
...
void LocationBarView::UpdateQrButtonVisibility() {
  if(qr_) {
    qr_->SetVisible(qr_->UrlIsHttp());
  }
}
...
void LocationBarView::PaintChildren(const ui::PaintContext& context) {
  ...
  const float scale = GetWidget()->GetCompositor()->device_scale_factor();
  if(scale == 1) {
    bounds.Inset(0, 0, 1, 1); // left, top, right, bottom (shrink)
  } else {
    bounds.Inset(1, 1, 0, 1); // left, top, right, bottom (shrink)
  }

  static SkColor edge = SkColorSetRGB(0xdd, 0xdd, 0xdd);
  recorder.canvas()->DrawRect(bounds,  edge);  
}
...
void LocationBarView::OnChanged() {
  ...
  if (GetThemeProvider()->HasCustomImage(IDR_THEME_TOOLBAR)) {
    SkColor background_color = (user_input_in_progress() && !IsTextEmpty()) ?
                    kBackgroundColorInputState : kBackgroundColorDefault;
...
void LocationBarView::ShowOmniboxDropdownView() {
  OmniboxPopupView* popup = omnibox_view_->model()->popup_model()->view();
  if (popup->IsOpen()) {
    popup->ClosePopup();
  }
  ...
  omnibox_view_->SetFocus();
  omnibox_view_->SelectAll(true);
  OmniboxPopupView* dropdown = omnibox_view_->model()->dropdown_model()->view();
  dropdown->UpdatePopupAppearance();
}

void LocationBarView::CloseOmniboxDropdownView() {
  OmniboxPopupView* dropdown = omnibox_view_->model()->dropdown_model()->view();
  if (dropdown->IsOpen()) {
      dropdown->ClosePopup();
  }
...
bool LocationBarView::IsOmniboxDropdownViewOpen() {
  OmniboxPopupView* dropdown = omnibox_view_->model()->dropdown_model()->view();
  return dropdown->IsOpen();
}

bool LocationBarView::IsTextEmpty() const {
  if (omnibox_view_) {
    return omnibox_view_->GetText().empty();
  }
  return true;
}

bool LocationBarView::user_input_in_progress() const{
  if (omnibox_view_) {
    return omnibox_view_->model()->user_input_in_progress();
  }
  return false;
}

void LocationBarView::OnPreferenceChanged(PrefService* service,
        const std::string& pref_name) {
  if (pref_name == prefs::kAddressCompatibleMode) {
    bool mode = profile()->GetPrefs()->GetBoolean(prefs::kAddressCompatibleMode);
    compatible_mode_button_->SetVisible(mode);
    Layout();
    SchedulePaint();
