...
BubbleIconView::BubbleIconView(CommandUpdater* command_updater, int command_id)
  ...
  //[zhangyq] switch off ink drop mode.
...
bool BubbleIconView::IsBubbleShowing() const {
  ...
  return GetCommonBubble() != nullptr || GetBubble() != nullptr;
}
...
void BubbleIconView::OnMouseEntered(const ui::MouseEvent& event) {
  if (!hovered_) {
    hovered_ = true;
    UpdateIcon();
  }
...
void BubbleIconView::OnMouseExited(const ui::MouseEvent& event) {
  hovered_ = false;
  UpdateIcon();
}
...
views::BubbleDelegateView* BubbleIconView::GetCommonBubble() const {
  return nullptr;
}
...
void BubbleIconView::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  if (views::BubbleDelegateView* bubble = GetCommonBubble()) {
