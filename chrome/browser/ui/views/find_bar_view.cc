...
const int kDefaultCharWidthInPx = 307;
...
FindBarView::FindBarView(FindBarHost* host)
  ...
  find_text_->set_default_width_in_chars(kDefaultCharWidth);
   ...
   //[zhangyq] Font in nfs is pretty small, it causes find_text width  not long enough to contain text.
  int width = find_text_->GetPreferredSize().width();
  if (width < kDefaultCharWidthInPx) {
    find_text_->set_default_width_in_chars(
          static_cast<int>(ceil((1.0 * kDefaultCharWidthInPx / width) * kDefaultCharWidth)));
  }
  //[zhangyq]  end
