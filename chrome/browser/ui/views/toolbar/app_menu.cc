...
//const int kZoomLabelHorizontalPadding = kHorizontalPadding;//yana mask
#else
//modify by yana 160812
const int kFullscreenPadding = 16;//38
...
//yana 160812
// Subclass of ImageButton whose preferred size includes the size of the border.
class LockButton : public ImageButton {
 public:
  explicit LockButton(views::ButtonListener* listener)
      : ImageButton(listener) { }
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(LockButton);
};
...
class AppMenu::ZoomView : public AppMenuView {
 public:
  ZoomView(AppMenu* menu,
    ...
    gfx::ImageSkia* full_screen_image =
        ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(
            IDD_FULL_SCREEN);/*IDR_FULLSCREEN_MENU_BUTTON*/
    ...
    //yana 160812
    lock_button_ = new LockButton(this);
    // all buttons on menu should must be a custom button in order for
    // the keyboard nativigation work.
    DCHECK(CustomButton::AsCustomButton(lock_button_));
    gfx::ImageSkia* lock_on_image =
        ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(
            IDD_UNLOCKED);
    if (menu->browser_->profile()->GetPrefs()->GetBoolean(prefs::kLockKeyword)){
        lock_on_image = ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDD_LOCKED);
    }
    lock_button_->SetImage(ImageButton::STATE_NORMAL, lock_on_image);

    lock_button_->SetFocusBehavior(FocusBehavior::ALWAYS);
    lock_button_->set_request_focus_on_press(false);
    lock_button_->set_tag(lock_index);
    lock_button_->SetImageAlignment(
        ImageButton::ALIGN_CENTER, ImageButton::ALIGN_MIDDLE);
    lock_button_->set_background(
        new InMenuButtonBackground(InMenuButtonBackground::LEADING_BORDER));
  ...
  gfx::Size GetPreferredSize() const override {
    ...
    //yana 160812
    int lock_width =
        lock_button_->GetPreferredSize().width() + kFullscreenPadding;
    ...
    return gfx::Size(
        button_width + ZoomLabelMaxWidth() + button_width + lock_width + fullscreen_width,
        0);
  }
  ...
  void Layout() override {
    ...
    x += bounds.width();
    bounds.set_x(x);
    bounds.set_width(lock_button_->GetPreferredSize().width() +
                     kFullscreenPadding);
    lock_button_->SetBoundsRect(bounds);
 ...
 private:
  ...
  // Index of the lock menu item in the model.
  const int lock_index_;//yana 160812
  ...
  ImageButton* lock_button_;//yana 160812
...
bool AppMenu::IsCommandEnabled(int command_id) const {
  ...
  if (command_id == IDC_FEEDBACK_MENU)
    return true;
  ...
  for (int i = 0, max = model->GetItemCount(); i < max; ++i) {
    ...
    switch (model->GetCommandIdAt(i)) {
      ...
      case IDC_ZOOM_MENU: {
        ...
        DCHECK_EQ(IDC_LOCK_OR_UNLOCK, submodel->GetCommandIdAt(2));//yana 160812
        DCHECK_EQ(IDC_FULLSCREEN, submodel->GetCommandIdAt(3));
        item->SetTitle(l10n_util::GetStringUTF16(IDS_ZOOM_MENU2));
        item->AddChildView(new ZoomView(this, submodel, 0, 1, 2,3));
        break;
      }
...
MenuItemView* AppMenu::AddMenuItem(MenuItemView* parent,
  ...
  if (command_id > -1) {  // Don't add separators to |command_id_to_entry_|.
    ...
    if (command_id != IDC_SHOW_HISTORY && command_id != IDC_SAVE_PAGE && command_id != IDC_CLEAR_BROWSING_DATA) {//yana 170110
