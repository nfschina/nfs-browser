...
ToolbarView::ToolbarView(Browser* browser)
  ...
  chrome::AddCommandObserver(browser_, IDC_RELOAD_ALL, this);
...
void ToolbarView::ShowBookmarkBubble(
  ...
  views::Widget* bubble_widget = BookmarkBubbleView::ShowBubble(
      ...
      browser_->profile(), url, title, already_bookmarked);
  if (star_view)
    bubble_widget->AddObserver(star_view);
}

QrButton* ToolbarView::GetQrBubbleAnchor() {
  QrButton* qr_button = location_bar()->qr_button();
  return qr_button;
}
