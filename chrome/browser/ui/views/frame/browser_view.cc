...
#include "base/strings/utf_string_conversions.h"  //add by luyue
#include "chrome/browser/ui/views/search_bar/search_bar_view.h"
#include "chrome/browser/ui/views/search_bar/searchbox_view_views.h"
...
void PaintBackgroundAttachedMode(gfx::Canvas* canvas,
  ...
  if (theme_provider->HasCustomImage(IDR_THEME_TOOLBAR) ||
    ...
    canvas->TileImageInt(*theme_provider->GetImageSkiaNamed(IDR_THEME_TOOLBAR),
                         ...
                         bounds.width(),
                         bounds.height());
    //[zhangyq]paint transparent layer.
    canvas->DrawColor(theme_provider->GetColor(ThemeProperties::COLOR_TOOLBAR_BLUR));
  }
...
BrowserView::BrowserView()
    : views::ClientView(nullptr, nullptr),
      ...
      close_directly_(false),
      close_window_state_(0), //add by luyue
...
gfx::Rect BrowserView::GetBookmarkBounds() const {
  if (!bookmark_bar_view_.get()) {
    return gfx::Rect();
  }

  gfx::Rect bookmark_bounds(bookmark_bar_view_->bounds());
  if (bookmark_bounds.IsEmpty())
    return bookmark_bounds;
  // The apparent bookmark edges are outside the "real" toolbar edges.
  bookmark_bounds.Inset(-views::NonClientFrameView::kClientEdgeThickness, 0);
  return bookmark_bounds;
}

int BrowserView::GetBookmarkbarNoOverlapHeight() const {
  int height = 0;
  if (bookmark_bar_view_) {
    height = std::max(0, bookmark_bar_view_->height() -
...
gfx::Point BrowserView::OffsetPointForNTPBackgroundImage() const {
  return contents_container_->bounds().origin();
}
...
bool BrowserView::DownloadManagerIsExist() {
...
bool BrowserView::DownloadManagerIsActive() {
  const BrowserList* browser_list = BrowserList::GetInstance();
  for(size_t i = 0; i < browser_list->size(); i ++) {
    Browser* browser = browser_list->get(i);
    if(browser && browser->is_simple_web_download_manager()) {
      if(browser->window() == NULL)
        return false;

      BrowserView* browserview = static_cast<BrowserView*>(browser->window());
      if (browserview == NULL || browserview->GetWidget() == NULL)
        return false;

      return browserview->GetWidget()->IsVisible() &&
        !browserview->GetWidget()->IsMinimized();
    }
  }
...
bool BrowserView::DownloadNewTaskIsExist() {
  const BrowserList* browser_list = BrowserList::GetInstance();
  Browser* browser = NULL;
  for(size_t i = 0; i < browser_list->size(); i ++) {
    browser = browser_list->get(i);
    if(browser->is_simple_web_new_download_task()) {
      if(browser->window() == NULL)
        return false;
      BrowserView* browserview = static_cast<BrowserView*>(browser->window());
...
void BrowserView::OnActiveTabChanged(content::WebContents* old_contents,
  ...
  GetLocationBarView()->compatibleMode_button()->
      UpdateImageForContents(new_contents);
}
...
void BrowserView::UpdateReloadStopState(bool is_loading, bool force) {
  if (toolbar_->location_bar()->reload_button()) {
    ReloadButton::Mode mode = is_loading ? ReloadButton::MODE_STOP : ReloadButton::MODE_RELOAD;
    if (toolbar_->location_bar()->GetToolbarModel()->input_in_progress())  {
      mode = ReloadButton::MODE_NAVIGATE;
    }
    toolbar_->location_bar()->reload_button()->ChangeMode(
        mode,
        force);
  }
}
...
void BrowserView::ShowBookmarkBubble(const GURL& url, const base::string16 title,
                                                                            bool already_bookmarked) {
  toolbar_->ShowBookmarkBubble(url, title, already_bookmarked,
                              bookmark_bar_view_.get());
}

void BrowserView::ShowQrBubble(GURL& url) {
  QrButton* anchor_view = toolbar_->GetQrBubbleAnchor();
  views::Widget* bubble_widget = QrBubbleView::ShowBubble(
      anchor_view, gfx::Rect(), nullptr, browser_->profile(), url);
...
void    BrowserView ::DeleteAcceleratorById(int cmd_id)
{
   for (std::map<ui::Accelerator, int>::const_iterator it =
           accelerator_table_.begin(); it != accelerator_table_.end(); ++it) {
    if (it->second == cmd_id) {
...
int  BrowserView::ConvertModifersValue(std::string &modifiers)
{
 int  convert_value = 0;
 if(modifiers == "Ctrl")
        convert_value = 4;
      ...
      convert_value = 2;
  else if(modifiers == "Home")
      convert_value  = 0;
    return convert_value;
 }

 bool BrowserView::DecodeShortCutPrefs(const base::Value& value)   //wdg add  for  parser the shortcuts  preferences
 {//40019 "20020"  "20019"  40251
   const int short_items= 26;
   const  int  command_id[26] = {IDC_ADDRESSBAR_LIST,IDC_RELOAD,IDC_HOME,IDC_STOP,IDC_RELOAD_BYPASSING_CACHE,IDC_NEW_WINDOW,IDC_NEW_INCOGNITO_WINDOW,IDC_NEW_TAB,IDC_CLOSE_TAB,IDC_SELECT_NEXT_TAB,IDC_SELECT_PREVIOUS_TAB,IDC_RESTORE_TAB,IDC_FULLSCREEN,IDC_BOOKMARK_PAGE,IDC_PRINT,IDC_SAVE_PAGE,IDC_FIND,IDC_ZOOM_PLUS,IDC_ZOOM_NORMAL,IDC_ZOOM_MINUS,IDC_FOCUS_TOOLBAR,IDC_OPEN_FILE,IDC_SHOW_BOOKMARK_BAR,IDC_SHOW_HISTORY,IDC_SHOW_DOWNLOADS,IDC_CLEAR_BROWSING_DATA};

  // const std::string command_id[26] = {"20018","33002","33003","33006","33007","34000","34001","34014","34015","34016","34017","34028","34030","35000","35003","35004","37000","38001","38002","38003","39001","40000","40009","40010","40012","40013"};
  const base::DictionaryValue* d_value = nullptr;
  if (!value.GetAsDictionary(&d_value)||d_value->size() ==0)
    return false;  // Unexpected type
 for  (int  command_id_index = 0; command_id_index <short_items;command_id_index++){
     int  id = command_id[command_id_index];
    // base::StringToInt(command_id[command_id_index], &id);
   DeleteAcceleratorById(id);
}
 for  (int  command_id_index = 0; command_id_index <short_items;command_id_index++){
    const base::ListValue* list_value = nullptr;
    d_value->GetList(base::IntToString(command_id[command_id_index]), &list_value);
    int size = list_value->GetSize();
    for(int k = 0 ;k<size;k++)
    {
          const base::Value* element = NULL;
          if (list_value->Get(k, &element))
          {
           const base::DictionaryValue* list_value_1 = nullptr;
             element->GetAsDictionary(&list_value_1);
              const base::ListValue* list_value_2 = nullptr;
              list_value_1->GetList("Keys", &list_value_2);
              int key_code_value = 0;
              int  modifiers_value = 0;
              int   arg_size  = list_value_2->GetSize();
               std::string key_code_string;
              list_value_2->GetString(arg_size-1,&key_code_string);
              //printf("the key_code_string is %s\n",key_code_string.c_str() );
               base::StringToInt(key_code_string, &key_code_value);
              for(int j = 0;j<arg_size-1;j++)
              {
                 std::string  modifiers;
                 list_value_2->GetString(j,&modifiers);
                 int temp_value = ConvertModifersValue(modifiers);
                  modifiers_value += temp_value;
              }
              int command_id_value = command_id[command_id_index];
     //          base::StringToInt(command_id[command_id_index], &command_id_value);
              ui::Accelerator accelerator((ui::KeyboardCode)key_code_value, modifiers_value);
              std::map<ui::Accelerator, int>::const_iterator iter =
              accelerator_table_.find(accelerator);
              if(iter == accelerator_table_.end())
              {
                 AddAccelerator(ui::Accelerator((ui::KeyboardCode)key_code_value, modifiers_value));
                accelerator_table_[accelerator] =    command_id_value;
              }
          }
    }
  }
...
//add  by wdg for the 快捷键设置
  PrefService* prefs = browser_->profile()->GetPrefs();
  const  base::DictionaryValue* prefs_value =
      prefs->GetDictionary(prefs::kShortCutsTable);
  DecodeShortCutPrefs(*prefs_value);
//end
  *is_keyboard_shortcut = false;
  ...
  if (tab_strip_model) {
    ...
    DCHECK(web_contents);
    if (web_contents) {
      if (web_contents->GetURL().spec() == "nfsbrowser://settings/#3") {
        return false;
      }
    }
  }
...
SearchBarView* BrowserView::GetSearchBar() const {
  return toolbar_ ? static_cast<ToolbarViewNfs*> (toolbar_)->search_bar() : nullptr;
}
...
bool BrowserView::ShouldShowWindowIcon() const {
  ...
  return browser_->SupportsWindowFeature(Browser::FEATURE_TITLEBAR) && !browser_->is_type_simple_web();
}

bool BrowserView::ShouldShowTabStripTnsertsTop() const {
  return !browser_->is_type_simple_web();
}
...
void BrowserView::CloseDownloadManager() {
  const BrowserList* browser_list = BrowserList::GetInstance();
  Browser* browser = NULL;
  for(size_t i = 0; i < browser_list->size(); i ++) {
    browser = browser_list->get(i);
    if(browser->is_simple_web_download_manager() ||browser->is_simple_web_new_download_task()) {
      if(browser->window() == NULL)
        return;
      BrowserView* browserview = static_cast<BrowserView*>(browser->window());
      browserview->GetWidget()->Close();
    }
  }
}

void BrowserView::AttachWebVedio() {
  chrome::ExecuteCommand(browser_.get(), IDC_ATTACH_WEB_VEDIO);
}
///////////////////////////////////////////////////////////////////////////////
...
bool BrowserView::CanClose() {
  // 考虑多标签情况
  if (!close_directly_ && !CanCloseWithMultipleTabs())  {
    return false;
  }
...
gfx::Size BrowserView::GetMinimumSize() const {
  if(browser_->is_type_simple_web()) {
    if(browser_->is_simple_web_download_manager())
      return gfx::Size(640, 480);
    if(browser_->is_simple_web_vedio())
      return gfx::Size(480, 320);
    if(browser_->is_simple_web_new_download_task())
      return gfx::Size(480, 250);
  }
...
gfx::Size BrowserView::GetMaximumSize() const {
  ...
  if(browser_->is_type_simple_web())
    if(browser_->is_simple_web_new_download_task())
      return gfx::Size(480, 250);
  //return GetBrowserViewLayout()->GetMaximumSize();

  return views::ClientView::GetMaximumSize();
}
...
bool BrowserView::AcceleratorPressed(const ui::Accelerator& accelerator) {
  PrefService* prefs = browser_->profile()->GetPrefs();
  if(!prefs->GetBoolean(prefs::kEnableShortCuts))
    return  false;

  // 非tabbed正常窗口，不需要响应快捷键
  if (!browser_->is_type_tabbed())  {
   return false;
  }
  ...
  std::map<ui::Accelerator, int>::const_iterator iter =
      accelerator_table_.find(accelerator);
  // DCHECK(iter != accelerator_table_.end());  //disabe the dcheck for the table can be modified
...
void BrowserView::OnOmniboxPopupShownOrHidden() {
  ...
  LocationBarView* location_bar_view = GetLocationBarView();
  if (location_bar_view) {
    location_bar_view->CloseOmniboxDropdownView();
    location_bar_view->GetOmniboxView()->CloseUrlFloatingView();
  }
}
...
void BrowserView::InitViews() {
  ...
  toolbar_ = new ToolbarViewNfs(browser_.get());
...
void BrowserView::LoadAccelerators() {
  ...
  for (std::vector<chrome::AcceleratorMapping>::const_iterator it =
    ...
    //merge form directory to make the new accelerator effect
   PrefService* prefs = browser_->profile()->GetPrefs();
   const  base::DictionaryValue* prefs_value  = prefs->GetDictionary(prefs::kShortCutsTable);
  int dict_size = prefs_value->size();
  if(dict_size!=0)
   DecodeShortCutPrefs(*prefs_value);
 //end
...
int BrowserView::GetCommandIDForAppCommandID(int app_command_id) const {
  ...
  switch (app_command_id) {
    ...
    //case APPCOMMAND_BROWSER_REFRESH_ALL: return IDC_RELOAD_ALL;
...
void BrowserView::UpdateAcceleratorMetrics(const ui::Accelerator& accelerator,
  ...
  switch (command_id) {
    ...
    case IDC_RELOAD_ALL:
      if (key_code == ui::VKEY_R)
	content::RecordAction(UserMetricsAction("Accel_Reload_R"));
      //else if (key_code == ui::VKEY_BROWSER_REFRESH_ALL)
      //	content::RecordAction(UserMetricsAction("Accel_Reload_F7"));
     break;
...
void BrowserView::ClosePopups() {
  // Close the omnibox popup, if any.
  LocationBarView* location_bar_view = GetLocationBarView();
  if (location_bar_view)
    location_bar_view->GetOmniboxView()->CloseOmniboxPopup();

  // close seachbox popup, if any.
  SearchBarView* search_bar_view = GetSearchBar();
  if (search_bar_view)
    search_bar_view->searchbox_view()->CloseSearchboxPopup();

  // close Omnibox Dropdown and UrlFloatingView, if any
  if (location_bar_view) {
     location_bar_view->CloseOmniboxDropdownView();
     location_bar_view->GetOmniboxView()->CloseUrlFloatingView();
  }
...
void BrowserView::OnWidgetBoundsChanged(views::Widget* widget, const gfx::Rect& new_bounds) {
  // 通知地址栏、搜索栏 把一些框去掉
  if (widget != GetWidget()) {
    return;
  }
...
bool BrowserView::CanCloseWithMultipleTabs() {
  bool closeMultiTabsState = browser()->profile()->GetPrefs()->GetBoolean(prefs::kCloseMultiTabs);
  bool noteDownloadQuitState = browser()->profile()->GetPrefs()->GetBoolean(prefs::kNoteDownloadQuit);
  if (!closeMultiTabsState && !noteDownloadQuitState) {
    return true;
  }

  TabStripModel* tab_strip_model = browser()->tab_strip_model();
  int num_downloads_blocking = browser()->GetWithInProgressDownloads();
  if (tab_strip_model->count() <= 1 && num_downloads_blocking <1) {
    return true;
  }
  ...
  base::string16 content_text;
  if(tab_strip_model->count() > 1 && closeMultiTabsState && (num_downloads_blocking <1 || !noteDownloadQuitState)) {
     content_text += l10n_util::GetStringUTF16(IDS_CLOSE_MULTI_TABS_CONTENT);
     close_window_state_ = 1;
  }else if (num_downloads_blocking >=1 && noteDownloadQuitState && (tab_strip_model->count() <= 1 || !closeMultiTabsState)) {
     content_text += l10n_util::GetPluralStringFUTF16(IDS_DOWNLOAD_REMOVE_CONFIRM_EXPLANATION,num_downloads_blocking);
     close_window_state_ = 2;
  }else if (tab_strip_model->count() > 1 && num_downloads_blocking >=1 && closeMultiTabsState && noteDownloadQuitState) {
     content_text += l10n_util::GetStringUTF16(IDS_CLOSE_MULTI_TABS_CONTENT);
     close_window_state_ = 3;
  } else {
     return true;
  }

  int title_id = IDS_CLOSE_MULTI_TABS_TITLE;

  nfsbrowser::SimpleMessageBoxViews::InitParams params(GetNativeWindow(),
  ...
  params.checkbox_text = l10n_util::GetStringUTF16(IDS_CLOSE_MULTI_TABS_NO_REMAINDER);
  params.callback = base::Bind(&BrowserView::CloseConfirmed, base::Unretained(this));
  nfsbrowser::SimpleMessageBoxViews::ShowNfsMessageBox(params);

   // the message box will take over
  return false;
}

void BrowserView::CloseConfirmed(nfsbrowser::MessageBoxResult result, bool checked) {
  if (result == nfsbrowser::MESSAGE_BOX_RESULT_YES) {
    if (checked && (close_window_state_ == 1 || close_window_state_ == 3)) {
      browser_->profile()->GetPrefs()->SetBoolean(prefs::kCloseMultiTabs, false);
    }
    else if (checked && close_window_state_ == 2) {
       browser_->profile()->GetPrefs()->SetBoolean(prefs::kNoteDownloadQuit, false);
    }
    close_directly_ = true;
    if(close_window_state_ == 1 || close_window_state_ == 2) {
       Close();
       return;
    }
    int num_downloads_blocking = browser()->GetWithInProgressDownloads();
    if (num_downloads_blocking <1) {
      return ;
    }
    ...
    nfsbrowser::SimpleMessageBoxViews::InitParams params(GetNativeWindow(),
                        nfsbrowser::MESSAGE_BOX_TYPE_QUESTION,
                        l10n_util::GetStringUTF16(title_id),
                        l10n_util::GetPluralStringFUTF16(IDS_DOWNLOAD_REMOVE_CONFIRM_EXPLANATION,num_downloads_blocking)
                        );
    params.yes_text = l10n_util::GetStringUTF16(IDS_CLOSE_MULTI_TABS_OK);
    params.no_text = l10n_util::GetStringUTF16(IDS_CLOSE_MULTI_TABS_CANCEL);
    params.checkbox_text = l10n_util::GetStringUTF16(IDS_CLOSE_MULTI_TABS_NO_REMAINDER);
    params.callback = base::Bind(&BrowserView::CloseDownload, base::Unretained(this));
    nfsbrowser::SimpleMessageBoxViews::ShowNfsMessageBox(params);
  } else {
    frame_->ResetWindowControls();
  }
}

void BrowserView::CloseDownload(nfsbrowser::MessageBoxResult result, bool checked) {
  if (result == nfsbrowser::MESSAGE_BOX_RESULT_YES) {
     if (checked)
        browser_->profile()->GetPrefs()->SetBoolean(prefs::kNoteDownloadQuit, false);
     Close();
  }else if (result == nfsbrowser::MESSAGE_BOX_RESULT_NO) {
     close_directly_ = false;
  }
}