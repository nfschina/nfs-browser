...
void BrowserFrame::InitBrowserFrame() {
  ...
  new views::ShadowView(this, NULL, views::ShadowViewBorder::SHADOW, 300, false);
...
const ui::NativeTheme* BrowserFrame::GetNativeTheme() const {
  if (browser_view_->browser()->profile()->GetProfileType() ==
      ...
      ThemeServiceFactory::GetForProfile(browser_view_->browser()->profile())
          ->UsingDefaultTheme()) {
#if defined(OS_WIN)
    //[zhangyq] use views::Widget::GetNativeTheme() on Windows.
...
void BrowserFrame::ResetWindowControls() {
 browser_frame_view_->ResetWindowControls();
}
