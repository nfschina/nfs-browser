...
class BrowserView : public BrowserWindow,
 ...
 public:
  ...
  int GetBookmarkbarNoOverlapHeight() const;
  ...
  gfx::Point OffsetPointForNTPBackgroundImage() const;
  ...
  gfx::Rect GetBookmarkBounds() const;
  ...
  bool IsBrowserTypeNormal() const {
    return browser_->is_type_tabbed() || browser_->is_type_simple_web();
  }
  ...
  SearchBarView* GetSearchBar() const override;
  ...
  void ShowBookmarkBubble(const GURL& url,
      const base::string16 title, bool already_bookmarked) override;
  void ShowQrBubble(GURL& url) override;
  ...
  void  DeleteAcceleratorById(int cmd_id);  //wdg add for the short_cut_table
  int   ConvertModifersValue(std::string &modifiers); //wdg add for the short_cut_table
  bool  DecodeShortCutPrefs(const base::Value& value) ;  //wdg  add for the short_cut_table
  ...
  bool ShouldShowTabStripTnsertsTop() const override;
  ...
  bool DownloadManagerIsExist() override;
  bool DownloadManagerIsActive() override;
  bool DownloadNewTaskIsExist() override;
  ...
  void CloseConfirmed(nfsbrowser::MessageBoxResult result, bool checked);
  void CloseDownload(nfsbrowser::MessageBoxResult result, bool checked); //add by luyue
 ...
 private:
  ...
  bool close_directly_;

  int close_window_state_; // 1:close window  2:close download 3:close window and download add by luyue
