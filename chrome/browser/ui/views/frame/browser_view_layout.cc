...
int BrowserViewLayout::LayoutBookmarkBar(int top) {
  ...
  // huk set the tab strip to be 1px height in fullscreen mode
  bool is_fullscreen_mode = static_cast<BrowserView*>(browser_view_)->IsFullscreen();

  if (!is_fullscreen_mode && !delegate_->IsBookmarkBarVisible()) {
...
void BrowserViewLayout::LayoutContentsContainerView(int top, int bottom) {
  ...
  const ui::ThemeProvider* theme_provider = browser_view_->GetThemeProvider();
  if (theme_provider && theme_provider->HasCustomImage(IDR_THEME_TOOLBAR) &&
                      browser_->is_type_tabbed() && ntp_background_offset_ != top) {
    ntp_background_offset_ = top;
    NotifyNTPBgOffsetChanged(&ntp_background_offset_);
  }
}
...
void BrowserViewLayout::NotifyNTPBgOffsetChanged(int* offset) const{
  content::NotificationService* service =
      content::NotificationService::current();
      service->Notify(chrome::NOTIFICATION_NTP_BACKGROUND_OFFSET,
                      content::Source<BrowserViewLayout>(this),
                      content::Details<int>(offset));
}
