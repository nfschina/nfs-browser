...
const int OpaqueBrowserFrameViewLayout::kFrameBorderThickness = 2; //huk
...
const int OpaqueBrowserFrameViewLayout::KNonClientTopHeight = 0;
...
OpaqueBrowserFrameViewLayout::OpaqueBrowserFrameViewLayout(
  ...
  trailing_buttons_.push_back(views::FRAME_BUTTON_WEB_VIDEO_PIN);
  trailing_buttons_.push_back(views::FRAME_BUTTON_WEB_VIDEO_RETURN);
  trailing_buttons_.push_back(views::FRAME_BUTTON_THEME);
  trailing_buttons_.push_back(views::FRAME_BUTTON_ACCOUNT);
...
int OpaqueBrowserFrameViewLayout::NonClientTopHeight(bool restored) const {
  return KNonClientTopHeight;//wdq
...
int OpaqueBrowserFrameViewLayout::GetTabStripInsetsTop(bool restored) const {
  if(!delegate_->ShouldShowTabStripTnsertsTop())
    return 0;
...
void OpaqueBrowserFrameViewLayout::LayoutWindowControls(views::View* host) {
  ...
  if(pin_button_)
    pin_button_->SetState(views::CustomButton::STATE_NORMAL);
  if(unpin_button_)
    unpin_button_->SetState(views::CustomButton::STATE_NORMAL);
  if(return_button_)
    return_button_->SetState(views::CustomButton::STATE_NORMAL);
  if (account_button_)
    account_button_->SetState(views::CustomButton::STATE_NORMAL);
  ...
  buttons_not_shown.push_back(views::FRAME_BUTTON_THEME);
  if(account_button_)
    buttons_not_shown.push_back(views::FRAME_BUTTON_ACCOUNT);
  if(pin_button_)
    buttons_not_shown.push_back(views::FRAME_BUTTON_WEB_VIDEO_PIN);
  if(return_button_)
    buttons_not_shown.push_back(views::FRAME_BUTTON_WEB_VIDEO_RETURN);
  ...
  if (delegate_->ShouldShowCaptionButtons()) {
    ...
    for (const auto& button : base::Reversed(trailing_buttons_)) {
      if(button== views::FRAME_BUTTON_WEB_VIDEO_PIN) {
        if(pin_button_) {
          ConfigureButton(host, button, ALIGN_TRAILING, caption_y);
          buttons_not_shown.erase(
              std::remove(buttons_not_shown.begin(), buttons_not_shown.end(), button),
                      buttons_not_shown.end());
        }
      } else if(button == views::FRAME_BUTTON_WEB_VIDEO_RETURN) {
        if(return_button_) {
          ConfigureButton(host, button, ALIGN_TRAILING, caption_y);
          buttons_not_shown.erase(
              std::remove(buttons_not_shown.begin(), buttons_not_shown.end(), button),
...
void OpaqueBrowserFrameViewLayout::LayoutTitleBar(views::View* host) {
  ...
  if(delegate_->ShouldShowTabStripTnsertsTop()) {
    ...
    window_icon_bounds_ = gfx::Rect(0 , 8, size, size);
...
void OpaqueBrowserFrameViewLayout::ConfigureButton(
    ...
    int caption_y) {
  switch (button_id) {
    case views::FRAME_BUTTON_ACCOUNT: {
      if (account_button_) {
        account_button_->SetVisible(true);
        SetBoundsForButton(host, account_button_, alignment, caption_y);
      }
      break;
    }
    case views::FRAME_BUTTON_THEME: {
      if (theme_button_) {
        theme_button_->SetVisible(true);
        SetBoundsForButton(host, theme_button_, alignment, caption_y);
      }
      break;
    }
    ...
    case views::FRAME_BUTTON_WEB_VIDEO_PIN: {
      if(pin_button_) {
        bool is_alwaysontop = delegate_->IsAlwaysOnTop();
        views::ImageButton* invisible_button = is_alwaysontop ?
           unpin_button_ : pin_button_;
       invisible_button->SetVisible(false);

        views::ImageButton* visible_button = is_alwaysontop ?
    ...
    case views::FRAME_BUTTON_WEB_VIDEO_RETURN: {
      if(return_button_) {
        return_button_->SetVisible(true);
        SetBoundsForButton(host, return_button_, alignment, caption_y);
      }
      break;
    }
  }
...
void OpaqueBrowserFrameViewLayout::HideButton(views::FrameButton button_id) {
  switch (button_id) {
    ...
    case views::FRAME_BUTTON_CLOSE:
      ...
      break;
    case views::FRAME_BUTTON_WEB_VIDEO_PIN:
      pin_button_->SetVisible(false);
      unpin_button_->SetVisible(false);
      break;
    case views::FRAME_BUTTON_WEB_VIDEO_RETURN:
    ...
    case views::FRAME_BUTTON_THEME:
      if (theme_button_) {
        theme_button_->SetVisible(false);
      }
      break;
    case views::FRAME_BUTTON_ACCOUNT:
      if (account_button_) {
        account_button_->SetVisible(false);
      }
      break;
  }
}
...
void OpaqueBrowserFrameViewLayout::SetView(int id, views::View* view) {
  ...
  switch (id) {
    ...
    case VIEW_ID_CLOSE_BUTTON:
      ...
      break;
    case VIEW_ID_WEB_VIDEO_PIN_BUTTON:
      if (view) {
        /*DCHECK_EQ(std::string(views::ImageButton::kViewClassName),
                  view->GetClassName());*/
      }
      pin_button_ = static_cast<views::ImageButton*>(view);
      break;
    case VIEW_ID_WEB_VIDEO_UNPIN_BUTTON:
      if (view) {
        /*DCHECK_EQ(std::string(views::ImageButton::kViewClassName),
                  view->GetClassName());*/
      }
      unpin_button_ = static_cast<views::ImageButton*>(view);
      break;
    case VIEW_ID_WEB_VIDEO_RETURN_BUTTON:
      if (view) {
        /*DCHECK_EQ(std::string(views::ImageButton::kViewClassName),
                  view->GetClassName());*/
      }
      return_button_ = static_cast<views::ImageButton*>(view);
      break;
    ...
    case VIEW_ID_AVATAR_BUTTON:
      ...
      break;
    case VIEW_ID_THEME_BUTTON:
      if (view) {
        DCHECK_EQ(std::string(views::ImageButton::kViewClassName),
                  view->GetClassName());
      }
      theme_button_ = static_cast<views::ImageButton*>(view);
      break;
    case VIEW_ID_ACCOUNT_BUTTON:
      if (view) {
        DCHECK_EQ(std::string(views::ImageButton::kViewClassName),
                  view->GetClassName());
      }
      account_button_ = static_cast<views::ImageButton*>(view);
      break;
    default:
