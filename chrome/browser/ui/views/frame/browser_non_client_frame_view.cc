...
SkColor BrowserNonClientFrameView::GetFrameColor(bool active) const {
  ...
  if(browser_view_->browser()->is_simple_web_download_manager())
    color_id = active ? ThemeProperties::COLOR_DOWNLOAD_FRAME
              : ThemeProperties::COLOR_DOWNLOAD_FRAME_INACTIVE;

  if(browser_view_->browser()->is_simple_web_vedio())
    color_id = active ? ThemeProperties::COLOR_WEB_VIDEO_FRAME
              : ThemeProperties::COLOR_WEB_VIDEO_FRAME_INACTIVE;
...
gfx::ImageSkia BrowserNonClientFrameView::GetFrameImage(bool active) const {
  ...
  //[zhangyq] show active frame image only.
  if(tp->HasCustomImage(IDR_THEME_FRAME)) {
    frame_image_id = IDR_THEME_FRAME;
  }
  ...
  if (default_uses_color) {
    bool is_download = browser_view_->browser()->is_simple_web_download_manager()
                  || browser_view_->browser()->is_simple_web_new_download_task();
    if (is_download) {
      frame_image_id = active ? IDR_THEME_FRAME_DOWNLOAD : IDR_THEME_FRAME_INACTIVE;
    }
