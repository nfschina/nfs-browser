...
bool BrowserRootView::OnMousePressed(const ui::MouseEvent& event) {
  // add a notification of mouse pressed (here i set left mouse only)
  if(event.IsOnlyLeftMouseButton()) {
      content::NotificationService::current()->Notify(chrome::NOTIFICATION_MOUSE_PRESSED,
             content::NotificationService::AllSources(), 
             content::NotificationService::NoDetails());
   }

  return views::internal::RootView::OnMousePressed(event);
}
