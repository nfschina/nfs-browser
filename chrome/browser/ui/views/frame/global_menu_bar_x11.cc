...
GlobalMenuBarCommand view_menu[] = {
  ...
  { IDS_SHOW_SEARCHBAR, IDC_SHOW_SEARCHBAR },
  { IDS_SHOW_TOOLBAR, IDC_SHOW_TOOLBAR },
...
GlobalMenuBarCommand help_menu[] = {
  ...
  { IDS_ABOUT_TITLE , IDC_ABOUT },
  { IDS_FEEDBACK_TITLE, IDC_FEEDBACK_TITLE },
  { IDS_CHECK_VERSION, IDC_CHECK_VERSION_DIALOG },
...
void GlobalMenuBarX11::InitServer(unsigned long xid) {
  ...
  pref_change_registrar_.Add(
      prefs::kShowToolbar,
      base::Bind(&GlobalMenuBarX11::OnToolbarVisibilityChanged,
                 base::Unretained(this)));
  pref_change_registrar_.Add(
      prefs::kShowSearchBar,
      base::Bind(&GlobalMenuBarX11::OnSearchBarVisibilityChanged,
                 base::Unretained(this)));
  OnBookmarkBarVisibilityChanged();
  OnToolbarVisibilityChanged();
  OnSearchBarVisibilityChanged();
  ...
  for (int i = 0; commands[i].str_id != MENU_END; ++i) {
    ...
    if (commands[i].str_id == MENU_SEPARATOR) {
      ...
      if (command_id == MENU_DISABLED_ID) {
        ...
        if (command_id == IDC_SHOW_SEARCHBAR)
          menuitem_property_set(menu_item, kPropertyToggleType, kTypeCheckmark);
        if (command_id == IDC_SHOW_TOOLBAR)
          menuitem_property_set(menu_item, kPropertyToggleType, kTypeCheckmark);
...
void GlobalMenuBarX11::OnToolbarVisibilityChanged() {
  CommandIDMenuItemMap::iterator it =
      id_to_menu_item_.find(IDC_SHOW_TOOLBAR);
  if (it != id_to_menu_item_.end()) {
    PrefService* prefs = browser_->profile()->GetPrefs();
    // Note: Unlike the GTK version, we don't appear to need to do tricks where
    // we block activation while setting the toggle.
    menuitem_property_set_int(
        it->second,
        kPropertyToggleState,
        prefs->GetBoolean(prefs::kShowToolbar));
  }
}
void GlobalMenuBarX11::OnSearchBarVisibilityChanged() {
  CommandIDMenuItemMap::iterator it =
      id_to_menu_item_.find(IDC_SHOW_SEARCHBAR);
  if (it != id_to_menu_item_.end()) {
    PrefService* prefs = browser_->profile()->GetPrefs();
    // Note: Unlike the GTK version, we don't appear to need to do tricks where
    // we block activation while setting the toggle.
    menuitem_property_set_int(
        it->second,
        kPropertyToggleState,
        prefs->GetBoolean(prefs::kShowSearchBar));
  }
}
