...
#ifndef CHROME_BROWSER_UI_VIEWS_SEARCHBAR_SEARCHBOX_POP_CONTENT_VIEW_H_
#define CHROME_BROWSER_UI_VIEWS_SEARCHBAR_SEARCHBOX_POP_CONTENT_VIEW_H_
...
#include "chrome/browser/ui/search_bar/searchbox_popup_model.h"
#include "chrome/browser/ui/views/search_bar/searchbox_popup_view.h"
...
class SearchboxResultView;
class SearchboxView;
class Profile;

// A view representing the contents of the autocomplete popup.
class SearchboxPopupContentsView : public views::View,
                                 public SearchboxPopupView,
                                 public views::ViewTargeterDelegate,
                                 public gfx::AnimationDelegate {
 public:
  // Factory method for creating the AutocompletePopupView.
  static SearchboxPopupView* Create(const gfx::FontList& font_list,
                                  SearchboxView* searchbox_view,
                                  SearchboxEditModel* edit_model,
                                  SearchBarView* search_bar_view);
 ...
 protected:
  SearchboxPopupContentsView(const gfx::FontList& font_list,
                           SearchboxView* searchbox_view,
                           SearchboxEditModel* edit_model,
                           SearchBarView* search_bar_view);
  ~SearchboxPopupContentsView() override;

  SearchBarView* search_bar_view() { return search_bar_view_; }

  // Calculates the height needed to show all the results in the model.
  virtual int CalculatePopupHeight();
  virtual SearchboxResultView* CreateResultView(int model_index,
 ...
 private:
  ...
  SearchboxResultView* result_view_at(size_t i);

  // Returns the target bounds given the specified content height.
  gfx::Rect CalculateTargetBounds(int height);

  std::unique_ptr<SearchboxPopupModel> model_;
  ...
  SearchboxView* searchbox_view_;

  SearchBarView* search_bar_view_;
  ...
  DISALLOW_COPY_AND_ASSIGN(SearchboxPopupContentsView);
};

#endif  // CHROME_BROWSER_UI_VIEWS_SEARCHBAR_SEARCHBOX_POP_CONTENT_VIEW_H_
