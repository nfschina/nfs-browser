...
void OmniboxPopupContentsView::LayoutChildren() {
  ...
  // contents_rect.Inset(0, g_top_shadow.Get().height(), 0,
  //                     0);
...
void OmniboxPopupContentsView::ClosePopup() {
  if (popup_ != NULL) {
...
void OmniboxPopupContentsView::UpdatePopupAppearance() {
  ...
  //<<-- zhangyq add to change popup view's x coordinate and width.
  top_left_screen_coord.set_x(top_left_screen_coord.x() + start_margin_ + 1);
  ...
  if (!popup_) {
    ...
    if (!popup_) {
      ...
      // add the shadow
    new views::ShadowView(static_cast<views::Widget*>(popup_.get()), NULL, views::ShadowViewBorder::LOCATIONBAR_SHADOW);
...
void OmniboxPopupContentsView::Observe(int type,
