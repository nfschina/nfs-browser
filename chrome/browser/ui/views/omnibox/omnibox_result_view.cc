...
void OmniboxResultView::PaintMatch(const AutocompleteMatch& match,
  ...
  contents_max_width = text_bounds_.width() / 2;
...
void OmniboxResultView::LoadIcon() {
  ...
  favicon::FaviconService* favicon_service =
      FaviconServiceFactory::GetForProfile(
          location_bar_view_->browser()->profile(),
  ...
  favicon_service->GetFaviconImageForPageURL(
      url,
      base::Bind(&OmniboxResultView::OnFaviconDataAvailable,
                 base::Unretained(this), url), &cancelable_task_tracker_);
}

void OmniboxResultView::OnFaviconDataAvailable(const GURL& url,
    const favicon_base::FaviconImageResult& image_result) {
  if (image_result.image.IsEmpty()) {
    for (size_t i = 0; i < arraysize(kPrepopulatePages); i++) {
      //If it's prepopulate page
      if (!(url.spec().compare(kPrepopulatePages[i].url))) {
            icon_ = *(ui::ResourceBundle::GetSharedInstance().
                             GetImageSkiaNamed(kPrepopulatePages[i].favicon_id));
         break;
      }
    }
  } else {
      icon_ = image_result.image.AsImageSkia();
  }
  Invalidate();
  if (GetWidget()) {
    Layout();
...
gfx::ImageSkia OmniboxResultView::GetKeywordIcon() const {
  //huk disable the icon , using empty instead
...
void OmniboxResultView::Layout() {
  ...
  if (match_.associated_keyword.get()) {
    ...
    //[zhangyq] We dont't show keyword text.
