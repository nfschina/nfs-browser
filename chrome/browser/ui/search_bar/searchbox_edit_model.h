...
#ifndef CHROME_BROWSER_UI_SEARCHBAR_SEARCHBOX_EDIT_MODEL_H_
#define CHROME_BROWSER_UI_SEARCHBAR_SEARCHBOX_EDIT_MODEL_H_
...
#include "chrome/browser/ui/search_bar/searchbox_controller.h"
...
class SearchboxEditModel {
 public:
  ...
  SearchboxEditModel(SearchboxView* view,
                   Profile* profile,
                   SearchboxEditController* controller,
                   std::unique_ptr<OmniboxClient> client);
  virtual ~SearchboxEditModel();

  // TODO(beaudoin): Remove this accessor when the AutocompleteController has
  //     completely moved to OmniboxController.
  AutocompleteController* autocomplete_controller() const {
    return searchbox_controller_->autocomplete_controller();
  }

  void set_popup_model(SearchboxPopupModel* popup_model) {
    searchbox_controller_->set_popup_model(popup_model);
  }
  ...
  SearchboxPopupModel* popup_model() const {
    return searchbox_controller_->popup_model();
  }

  SearchboxEditController* controller() const { return controller_; }
  ...
  bool AcceptKeyword(KeywordModeEntryMethod entered_method);
  ...
  const AutocompleteResult& result() const {
    return searchbox_controller_->result();
  }
 ...
 private:
  ...
  std::unique_ptr<SearchboxController> searchbox_controller_;

  SearchboxView* view_;

  Profile* profile_;

  SearchboxEditController* controller_;
  ...
  DISALLOW_COPY_AND_ASSIGN(SearchboxEditModel);
};

#endif  // CHROME_BROWSER_UI_SEARCHBAR_SEARCHBOX_EDIT_MODEL_H_
