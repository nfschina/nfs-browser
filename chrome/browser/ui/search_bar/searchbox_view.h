...
#ifndef CHROME_BROWSER_UI_SEARCHBAR_SEARCHBOX_VIEW_H_
#define CHROME_BROWSER_UI_SEARCHBAR_SEARCHBOX_VIEW_H_
...
#include "chrome/browser/ui/search_bar/searchbox_edit_model.h"
...
class SearchboxView {
 public:
  ...
  SearchboxEditModel* model() { return model_.get(); }
  const SearchboxEditModel* model() const { return model_.get(); }
  ...
  virtual void CloseSearchboxPopup();
 ...
 protected:
  SearchboxView(Profile* profile, SearchboxEditController* controller,
  ...
  virtual int GetSearchboxTextLength() const = 0;

  // Try to parse the current text as a URL and colorize the components.
  virtual void EmphasizeURLComponents() = 0;

  SearchboxEditController* controller() { return controller_; }
  const SearchboxEditController* controller() const { return controller_; }

 private:
  // |model_| can be NULL in tests.
  std::unique_ptr<SearchboxEditModel> model_;
  SearchboxEditController* controller_;

  DISALLOW_COPY_AND_ASSIGN(SearchboxView);
};

#endif  // CHROME_BROWSER_UI_SEARCHBAR_SEARCHBOX_VIEW_H_
