...
base::string16 GetBrowserProgId(const base::string16& suffix) {
  ...
  if (ShellUtil::GetUserSpecificRegistrySuffix(&new_style_suffix) &&
      suffix == new_style_suffix && chrome_html.length() > 42) {
    NOTREACHED();
    chrome_html.erase(42);
  }
  return chrome_html;
}
...
bool ElevateAndRegisterChrome(BrowserDistribution* dist,
  ...
  base::FilePath exe_path = chrome_exe;
...
bool ShellUtil::MakeChromeDefault(BrowserDistribution* dist,
  ...
  if (!RegisterChromeBrowser(dist, chrome_exe, base::string16(), elevate_if_not_admin) &&
      !RegisterChromeBrowser(dist, chrome_exe, base::string16(), true)) {
    return false;
  }
