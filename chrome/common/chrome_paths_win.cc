...
bool GetUserDownloadsDirectory(base::FilePath* result) {
  ...
  if (GetDiskFreeSpaceEx(path.c_str(), &uiFreeBytesAvailableToCaller,
  ...
  if ( access("D:\\",6) == 0 && dwFreeDiskSpace > 1)
  {	   
      *result = base::FilePath(L"D:\\");
       // *result = result->Append(l10n_util::GetStringUTF16(IDS_CDOS_DOWNLOAD_PATH));
      *result = result->Append(L"方德基础浏览器下载");
      return true;
  }
  else
  {
