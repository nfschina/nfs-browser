...
function DownloadItem(data) {
  ...
  item.getElement('by-ext').onclick = function() {
    chrome.tabs.create({url: 'nfsbrowser://extensions#' + item.byExtensionId});
    return false;
  }
  ...
  function setByExtension(show) {
    if (show) {
      ...
      item.getElement('by-ext').href =
        'nfsbrowser://extensions#' + item.byExtensionId;
      item.getElement('by-ext img').src =
        'nfsbrowser://extension-icon/' + item.byExtensionId + '/48/1';
    } else {
