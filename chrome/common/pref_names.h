...
//yana add 160818
extern const char kLockKeyword[];
...
extern const char kWebKitClickPrerenderEnabled[];
extern const char kWebKitMouseOverPrerenderEnabled[];
...
extern const char kOriginalDownloadDirectoty[]; //wdg
extern const char kDownloadDefaultDirectory[];
extern const char kDownloadDirectorySelected[];
...
extern const char kDiskDefaultCacheDir[]; //wdg
extern const char kDiskMaxCacheSize[]; //wdg
extern const char kDiskCahceAutoclear[]; // wdg add
...
// the  ui show
extern const char kShowSearchBar[];//搜索栏
extern const char kShowMarkedBar[]; //收藏栏
extern const char kShowToolbar[]; //工具栏
...
extern const char kBrowseringMode[]; //浏览模式

// the address bar prefs
extern const char kAddressAutofill[];//启用网站地址栏自动补全
extern const char kAddressHistory[];//推荐列表中 显示历史记录
extern const char kAddressSaved[]; //推荐列表中显示收藏地址
extern const char kAddressClip[];//启用地址粘贴复制功能
extern const char kAddressCompatibleMode[];//地址栏显示切换兼容模式图标
//end  addressbar

  /////////////////////
//add  for opening tag page prefs
extern const char kNewTabPage[];//新建 标签 -新空白页/主页
extern const char kCloseMultiTabs[]; //有多个标签页关闭时提示
extern const char kCloseAutoClearBrowserData[]; //关闭时自动清除数据
extern const char kDoubleClickToClose[];//双击关闭
extern const char kClickToCloseNotice[]; //关闭后重新打开最后一个窗口
extern const char kCloseTabSwitch[];//关闭后切换模式
extern const char kOpenNewTabPage[];//点击标签连接时，跳转到新的标签页
extern const char kBoldInBackground[];//后台打开加粗显示
extern const char kPomptInNewWindow[];//弹出窗口总在新标签页打开
extern const char kOpenDirection[];//打开新标签页 左边打开 右边打开
//end  for  the tab pages


//add for checkurl db update prefs
extern const char kMurlUpdate[];//更新Murl DB

//add  the download  options
extern const char kDownloadMaxSpeed[];
extern const char kDownloadNeedLimitSpeed[];
extern const char kNoteDownloadQuit[];//退出时有任务提示
extern const char kPromptDownload[];//下载完成时弹窗提示
//extern const char kVoiceNoteDownload[];//下载完成时声音提示
extern const char kContinueLastDownload[];//浏览器启动后自动开始上次未完成任务
//end
extern const char kDefaultBrowserCheck[];//启动时检测默认浏览器

extern const char kEnableTabPreview[]; // 标签预览功能开关
extern const char kTabMaxIndex[]; // 标签预览功能

extern const char kEnableClickPrerender[];
extern const char kEnableMouseOverPrerender[];
extern const char kEnableShortCuts[]; // 禁用快捷键开关
extern const char kShortCutsTable[]; //快捷键设置表

// extern const char kIsNewFolder[]; // 导入书签时是否新建文件夹

// extern const char kUserID[];  // wangpp 用户ID
extern const char kShouldShowImportDialog[];  // wangpp 判断是否应该显示导入对话框
extern const char kBrowserLastUpdateVersion[];  // wangdq 记录上一次提示升级的版本号
extern const char kBrowserShouldShowWelcome[];  // wangdq

extern const char kWebKitUsesUniversalDetector[];  // wangpp
extern const char kStaticEncodings[];
extern const char kRecentlySelectedEncoding[];

extern const char kEnablePrivacyProtocal[];

extern const char kEnableSyncBookmark[];
extern const char kEnableSyncSkin[];
extern const char kEnableSyncPassword[];
extern const char kFirstSyncPassword[];

extern const char kAutoUpdateToLatest[];

extern const char kShowVideoToolbar[];
extern const char kShowCopySearchToolbar[];
