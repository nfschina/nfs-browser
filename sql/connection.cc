...
#include "base/command_line.h"  //add by wdg for encrypt
#include "sql/sql_switches.h"  //wdg
...
const int kBusyTimeoutSeconds = 3;
...
bool Connection::Raze() {
  ...
  if (rc == SQLITE_NOTADB || rc == SQLITE_IOERR_SHORT_READ) {
    ...
    if (rc != SQLITE_DONE) {
          // DLOG(FATAL) << "Failed retrying Raze().";  //wdg
           DLOG(WARNING) << "Failed retrying Raze().";
    }
  }
  ...
  if (rc != SQLITE_DONE) {
    ...
    DLOG(WARNING) << "Unable to copy entire null database.";  //wdg
    return false;
  }
...
int Connection::ExecuteAndReturnErrorCode(const char* sql) {
  ...
  while ((rc == SQLITE_OK) && *sql) {
    ...
    base::CommandLine::ForCurrentProcess();
       std::string  database_name = DbPath().BaseName().AsUTF8Unsafe();
      const  std::string  firefox_path("places.sqlite");
      int   encrypt_sqlitedatabase  = database_name.find(firefox_path,0);
      if (!command_line->HasSwitch(switches::kDisableSqlKey)&&encrypt_sqlitedatabase<0){
        sqlite3_key(db_,  "cdossqlitekey",  strlen("cdossqlitekey"));   //do andy excute need input the key
        }
...
bool Connection::OpenInternal(const std::string& file_name,
  ...
  const  std::string  user_path(".config/cdos-browser2/");
  int   encrypt_sqlitedatabase  = file_name.find(user_path,0);
  base::CommandLine* command_line =
 base::CommandLine::ForCurrentProcess();
 if (!command_line->HasSwitch(switches::kDisableSqlKey)&&encrypt_sqlitedatabase>=0){
         sqlite3_key(db_,  "cdossqlitekey",  strlen("cdossqlitekey"));   //do andy excute need input the key
...
int Connection::OnSqliteError(
  ...
  std::string id = histogram_tag_;
   ...
   const std::string  firefox("places.sqlite");
   int  is_firefox  = id.find(firefox,0);
  if(!id.empty()&&is_firefox<0){
    id = DbPath().BaseName().AsUTF8Unsafe();
  if (err != 2067)
  ...
  if (!command_line->HasSwitch(switches::kDisableSqlKey)&&err ==SQLITE_NOTADB &&
        err == SQLITE_NOTADB){
      Delete(DbPath());
     }
  }
  if (!error_callback_.is_null()&& (!id.empty())) {
  ...
  if (!IsExpectedSqliteError(err))
    ...
    DLOG(WARNING) << GetErrorMessage();   //wdg
  return err;
}
