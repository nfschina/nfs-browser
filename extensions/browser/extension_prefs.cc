...
const char kPreInstallVersion[] = "browser.preinstallversion";
...
bool ExtensionPrefs::ReadPrefAsURLPatternSet(const std::string& extension_id,
  ...
  if (extension_id == extensions::GetMouseGestureID()) {
    valid_schemes |= URLPattern::SCHEME_CHROMEUI;
  }
...
void ExtensionPrefs::RegisterProfilePrefs(
  ...
  registry->RegisterIntegerPref(kPreInstallVersion, 0);
...
int ExtensionPrefs::GetPreinstallVersion()  {
  return prefs_->GetInteger(kPreInstallVersion);
}

void ExtensionPrefs::SetPreinstallVersion(int version)  {
  prefs_->SetInteger(kPreInstallVersion, version);
}
