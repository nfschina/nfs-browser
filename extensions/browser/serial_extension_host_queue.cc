...
int GetDelayMs(bool has_browser_window_shown) {
  ...
  // here we try to use lazy load(huk)
  static int extension_lasy_load_nfs_first = 10 * 1000;
  static int extension_lasy_load_nfs_non_first = 1 * 1000;

  static bool is_first_ = true;
  base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
  if (!command_line->HasSwitch(switches::kDisableExtensionLazyLoad)  && delay_ms <= 0) {
    if (is_first_ && !has_browser_window_shown) {
      delay_ms = extension_lasy_load_nfs_first;
    } else {
      delay_ms = extension_lasy_load_nfs_non_first;
    }
    is_first_ = false;
  }
...
SerialExtensionHostQueue::SerialExtensionHostQueue()
    : pending_create_(false),
      has_browser_window_shown_(false),
      ptr_factory_(this) {
  registrar_.Add(this, chrome::NOTIFICATION_BROWSER_WINDOW_SHOWN,
...
void SerialExtensionHostQueue::Observe(int type,
                                 const content::NotificationSource& source,
                                 const content::NotificationDetails& details) {
  static bool is_first = true;
  switch (type) {
    case chrome::NOTIFICATION_BROWSER_WINDOW_SHOWN:
      if (is_first) {
        is_first = false;
        has_browser_window_shown_ = true;
        ProcessOneHost();
      }
      break;
    default:
...
void SerialExtensionHostQueue::PostTask() {
  if (!pending_create_) {
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
        ...
        base::TimeDelta::FromMilliseconds(GetDelayMs(has_browser_window_shown_)));
