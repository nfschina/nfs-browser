...
void ManagementEventRouter::OnExtensionCancelled(
    content::BrowserContext* browser_context,
    const Extension* extension) {
  BroadcastEvent(extension, events::MANAGEMENT_ON_CANCELLED,
                 management::OnCancelled::kEventName);
}
...
void ManagementEventRouter::BroadcastEvent(
  ...
  if (event_name == management::OnUninstalled::kEventName ||
      event_name == management::OnCancelled::kEventName) {
...
ManagementAPI::ManagementAPI(content::BrowserContext* context)
  ...
  event_router->RegisterObserver(this, management::OnCancelled::kEventName);
